...
// finance-test@x.com + SO-9101..9104 aging-bucket orders).
//
...
//   • Filtered AR page shows the corresponding seeded SO row
test("phase-5 A3: AR aging buckets — dashboard renders + AR filter pills work", async ({ browser }) => {
  ...
  // ----- 2. FinanceAR — switch to All status filter so seeded SO-9101..9104 -
  ...
  //          "0-30", "31-60", "61-90", "90+". Filter by 90+ → only SO-9104.
  ...
  // 0-30 (SO-9101) should NOT be visible under 90+ filter.
  await expect(fpage.getByText(/INV-\d{4}-9101\b/)).toHaveCount(0);

  // Filter by 0-30 → only SO-9101. (Use exact match because 0-30 is also a
