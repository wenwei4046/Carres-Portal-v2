...
| API bundle | **LIVE = Worker version `13c77a71` from main tip `a12e8ff6`** (deployed with PR #472 Portal-Core C9, which DOES change api files — the booking gate, the dispatch gate, the release-decision route and the Orders list select. Safe on the union tip because **`git diff --diff-filter=A bc92e4cc..HEAD -- supabase/migrations/` is EMPTY — no migration has been added since the confirmed-applied tail 0300**. Wrangler's receipt read back: `PUBLIC_WEB_URL: https://pos.carresofficial.com` + the `api.carresofficial.com` custom domain + the 09:00-MYT cron; `GET /health` through the custom domain returns 200.) Prior: `739d4be8` from `bc92e4cc` (PR #466 C2). **The api deploy is `wrangler deploy --env production`, NEVER bare `wrangler deploy`** — both stanzas share `name = "carres-portal-v2-api"`, so a bare deploy overwrites production with the default env: `PUBLIC_WEB_URL=http://localhost:5173` and NO `api.carresofficial.com` route. Done by accident once and corrected within a minute; **wrangler echoes the bindings it deployed — read them, they are the receipt.** Same union-tip rule as web. |
