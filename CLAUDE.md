...
| Web bundle | **built + deployed 2026-07-18 from main `f5265bf` (New Order billing MY-cascade, PR #187; Pages deploy `0d64bc6a`, bundle `index-jeIwmpcU.js` — carries everything through change-request edit [PR #186 / 0234]); `SERVICE_ROLE` scan on dist = 0; canonical https://carres-portal.pages.dev live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-18 from main `f5265bf` (Worker version `402525e4-7637-4480-a084-c79de1e46f17` — raw door posts paid>0 into the 0193 `order_payments` ledger [kind deposit, best-effort] + carries POS entry fixes / Add-product P1–P3 / staff PIN / change-request edit [migrations 0230–0234 on prod]; GET /api/catalog → 401 live-verified).** |
