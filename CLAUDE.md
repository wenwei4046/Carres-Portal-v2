...
| Web bundle | **built + deployed 2026-07-18 from main `4c1911a` (New Order Pay online/Stripe, PR #185; Pages deploy `588fd267`, bundle `index-CTxnSgm6.js` — carries everything through POS entry fixes + Add-product P1–P3 + staff PIN [PRs #173–#184]); `SERVICE_ROLE` scan on dist = 0; canonical https://carres-portal.pages.dev live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-18 from main `4c1911a` (Worker version `6572903e-8a94-4036-8c54-914c129af23a` — raw door posts paid>0 into the 0193 `order_payments` ledger [kind deposit, best-effort] + carries POS entry fixes / Add-product P1–P3 / staff PIN [migrations 0230–0233 on prod]; GET /api/catalog → 401 live-verified).** |
