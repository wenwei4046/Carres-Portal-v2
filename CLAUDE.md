...
| Latest migration | **Applied tail = 0267 — THREE files share the number** (all applied 2026-07-26 within 2 minutes of each other by three parallel sessions; the tracker keys on TIMESTAMP so this is cosmetic and, per the standing rule, NONE were renumbered): `0267_auth_hook_wont_mint_a_role_for_disabled` (054334) · **`0267_guarantee_id`** (054413 — the `ABCD123456` handle: 4 letters + 6 digits minted with the Sales Order, written back onto the line's `attrs.remark`, retired into `claimed_guarantee_id` on claim) · `0267_rental_agreement_templates` (054452). Prior: 0266 · 0265 · 0264 · **0261-0263 guarantee packages**. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8; collisions renumber at apply. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) Full per-migration history → archive doc + worklog. |
