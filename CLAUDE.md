...
| Test count | api **795/798** (3 pre-existing fails, untouched by P3) · web **720/725** (5 pre-existing fails) · shared **344/344** — measured 2026-06-21 after Sofa engine Phase 3 (PR #33); all 8 fails pre-existing per §17.7 (OhanaSofaTab ×4 + NiceFutureMattressTab ×1 web; supplier/pos ×2 + pickups ×1 api), zero new regressions. Phase 3 added: shared sofa-geometry (**57 tests** — footprint/snap/groupSofas/analyzeSofa) + web CompartmentSilhouette/ModulePaletteItem/SofaBuildCanvas/sofa-build-draft tests. P3 is web+shared only — no API change, api stays 795/798. |
| Web bundle | built + deployed from `main` 2026-06-21 (Sofa engine Phase 3, Pages deploy `a267c6a1`); `SERVICE_ROLE` scan on dist = 0. bundle-size regression CF still open, see §17.5. |
| API bundle | **Redeployed 2026-06-21 from `main` (Worker `1b3f8bd7`, PR #31 — catalog GET returns `sofaCombos` + principal-gated sofa-combo CRUD; backward-compatible).** |
...
- `sofa-engine-roadmap-remaining-phases` (watch) — Phase 1 (compartments) + Phase 2 (pricing engine + `sofa_combo_pricing`) + Phase 3 (visual drag builder — `sofa-geometry` + `SofaBuildCanvas` + integration) are SHIPPED; the ~5-phase **2990s sofa custom-cell engine** roadmap (`docs/superpowers/plans/2026-06-21-sofa-engine-roadmap.md`) still has **P4 (the trust gate — Hono server-recompute on the SofaBuildCanvas single line: re-run `computeSofaPrice` with fresh DB prices, 0.5% drift → 400 reject, then `explodeSofaBuild` the build into per-compartment `order_lines` via the existing `create_order`; touches NO schema if config stays in `order_lines.attrs` free jsonb)** · P5 (downstream regroup-by-`sofa_build_key` + cutover + deploy). Each phase = its own plan + SDD. Promo (PWP/GWP) + bedframe remain deferred. **Polish follow-ons deferred from P3**: group-drag/rotate (move/rotate a whole connected sofa as a unit — 2990s has it; per-cell drag+snap reconnects pieces fine) + recliner per-seat toggle+pricing (needs a migration `product_models.recliner_upgrade_price`). **Two P2 review minors to confirm byte-aligned at P4's Hono recompute**: 0-priced-combo gate (already fixed to match 2990s) + `pickSofaCombo` `tier=null` wildcard (Carres-only superset, unreachable from the real `computeSofaPrice` path).
