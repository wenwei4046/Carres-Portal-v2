...
Next decision pending:
  ...
  2. Phase 5 Finance — Chunk A IN FLIGHT 2026-05-08 evening (server complete + 3 of 5 Chunk A web pages live: Dashboard / AR / Payments; AP + Invoices remain). Spec at `docs/superpowers/specs/2026-05-08-phase-5-finance-spec.md`. 9 Q's locked at 21:42. Nine commits pushed to origin/main this session: `8ecb62b` spec, `c459db9` migrations 0061+0062, `02efa78` server foundation, `61bd6b2` +18 route tests, `50468e3` invoices+refunds routers +24 tests, `0337d06` web queries.ts (qk.finance.* + 10 hooks), `a9f4614` FinanceApp shell + Sidebar + Dashboard +3 web tests + App.tsx /finance/* mount, `9d75f0e` FinanceAR + ARDrawer +4 web tests, plus pending FinancePayments read-only view (+4 web tests). Acceptance progress: A1+A2+A3 server-side complete; web Dashboard live + AR page with full mutation drawer (record receipt + issue invoice gated) + Payments page read-only with progress bars. Next: FinanceAP + APDrawer (3-way match needs new schema/state for payStatus matched→scheduled→paid — possibly migration 0063 add purchase_orders.pay_status column or finance-side pay_schedule table; defer schema design to next session); FinanceInvoices page (Chunk C scope); shared finance helpers hoist (rm/rmCompact duplicated 3 times); 3 Playwright E2E specs (A1+A2+A3 acceptance flows). Chunk B: recon tables already exist via 0061; need finance_recon_suggest_matches RPC + reconciliation.ts router + reports.ts ap-aging/cashflow/monthly-pl/top-skus + FinanceRecon + FinanceReports pages. Chunk C: refunds.ts already partial; need /apply route + FinanceRefunds + FinanceInvoices full pages + invoice PDF render via @react-pdf/renderer (Q7=A server-side).
