...
| Test count | api **784/787** (3 pre-existing fails) · web **676/681** (5 pre-existing fails) · shared **233/233** — measured 2026-06-21 after Sofa engine Phase 1 (PR #29); all 8 fails pre-existing per §17.7 (OhanaSofaTab ×4 + NiceFutureMattressTab ×1 web; supplier/pos ×2 + pickups ×1 api), zero new regressions. Phase 1 added compartment tests (shared compartment/model-compartment schemas + adapters; catalog API compartment-pool GET + principal-gated CRUD branches; P&M Sofa Compartments maintenance UI + per-model panel). |
| Web bundle | built + deployed from `main` 2026-06-21 (Sofa engine Phase 1, Pages deploy `5ce9a835`); `SERVICE_ROLE` scan on dist = 0. bundle-size regression CF still open, see §17.5. |
| API bundle | **Redeployed 2026-06-21 from `main` (Worker `6053e1a0`, PR #29 — catalog GET returns the sofa-compartment pool + per-model offered rows + principal-gated compartment CRUD; backward-compatible).** |
...
- `sofa-engine-roadmap-remaining-phases` (watch) — Phase 1 (compartment foundation) is shipped; the ~5-phase **2990s sofa custom-cell engine** roadmap (`docs/superpowers/plans/2026-06-21-sofa-engine-roadmap.md`) still has P2 (pricing engine + `sofa_combo_pricing` Kuhn subset-match + `computeSofaPrice`/`explodeSofaBuild`) · P3 (visual drag plan-view builder, web-only) · P4 (Hono server-recompute + 0.5% drift-reject + explode into per-compartment lines via existing `create_order`) · P5 (downstream regroup + cutover). Each phase = its own plan + SDD; migrations go through the §7 gate (P2 `sofa_combo_pricing` additive; P4 touches NO schema if config stays in `order_lines.attrs` free jsonb). Promo (PWP/GWP) + bedframe remain deferred.
