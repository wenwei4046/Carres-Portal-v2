...
E2E NEW carry-forwards (added 2026-05-09 morning recon):
  • phase-7-lp-whitelist-trigger-service-role-bypass (medium) — migration 0046 `enforce_partner_po_column_whitelist` BEFORE UPDATE trigger uses `app_role() <> 'partner'` early-return guard. When a service_role connection (no auth.uid) does an UPDATE on purchase_orders, `app_role()` returns NULL and `NULL <> 'partner'` evaluates to NULL (not TRUE), so the trigger falls through to the column whitelist check and rejects with `lp_column_whitelist_violation`. Blocks `pnpm reset:e2e-state` and any other service_role admin work that touches purchase_orders. Fix: change to `coalesce((select public.app_role()), '') <> 'partner'` (or `IS DISTINCT FROM`). 1-line migration. Production impact: any cron / backup / batch script.
