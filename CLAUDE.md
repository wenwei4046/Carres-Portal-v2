...
| Web bundle | **LIVE = `index-C4Gvas6i.js` from main tip `474e67bd`** (carres-portal `744baafd`, deployed by the parallel line minutes after PR #359's own deploy). **That tip CONTAINS PR #359** — proven with `git merge-base --is-ancestor 2ad24013 474e67bd` — and the live bundle greps `hr/kpi` + "Manager view" (4,193,257 bytes, `SERVICE_ROLE` 0), so HR-P6 IS live; this superseded #359's bundle rather than clobbering it. Prior in this chain: `index-DtlbnO4w.js` from `2ad24013` (PR #359 HR-P6 targets + scoreboard; carres-portal `77da79e6` + carres-pos `a9df1568`; all 4 canonicals converged, downloaded 4,192,734 bytes, `SERVICE_ROLE` grep 0, `hr/kpi` + "Manager view" both present; `carres-pos.pages.dev` was the one that lagged this time — the deployment list confirmed `a9df1568` from source `2ad2401` was already the newest, so it was cache, not a clobber). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after and POLL until they converge (three deploys running: one or two of four lag ~20s, `pos.carresofficial.com` most often). **A canonical serving a DIFFERENT hash is usually the OLDER bundle still cached — `wrangler pages deployment list` names the true last writer, then `git merge-base --is-ancestor <theirs> <yours>` proves whether your tip contains their code. Contained = you superseded them, keep polling; NOT contained = you clobbered them and must redeploy from the union.** A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker `abff79eb` from main tip `2ad24013`** (PR #359 — `/api/hr/kpi` GET + PUT target + DELETE target + PUT manual + PUT store-manager; deployed AFTER 0276 was applied. Unauth 401 live-verified on `/api/hr/kpi`, `/api/hr/runs`, `/api/hr/people`, `/api/guarantees` via the custom domain). Same union-tip rule as web. |
