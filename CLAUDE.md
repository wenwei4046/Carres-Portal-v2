...
| Web bundle | **built + deployed 2026-07-19 (v2, latest) from merged main `505d756e` (bundle `index--8-5zdIF.js` — the day's FULL superset: PR #216 staff Edit form + #215 bundle V2 + #214/#212 logout + #211 postcodes + #210 POS-only + #209 staff profile/English-only + #208 bundles; deployed to BOTH Pages projects — carres-portal `fe894847` + carres-pos `66717fde`, `--branch=main`; supersedes `index-ItuJHcJ4.js`); `SERVICE_ROLE` scan on dist = 0; both pages.dev canonicals + pos./erp.carresofficial.com all live-verified (cache-busted). ⚠️ Same-day parallel-deploy race hit twice — before ANY Pages deploy: `git fetch` + `git log HEAD..origin/main`, rebuild from the union tip.** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 (v2, latest) from merged main `505d756e` (Worker version `3af3e6b1-d9cc-4667-947c-479cc0e2f9e7` — staff PATCH gains `phone` [Edit form, PR #216]; strict superset of same-day `226ae939` [0241 profile fields] + `88fa0ac1` [bundles] + `ca4faad8` [/api/account]; carries everything through BOTH 0241s; GET /api/catalog → 401 live-verified).** |
