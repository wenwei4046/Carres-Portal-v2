...
| Latest migration | **Applied tail = 0298_service_case_deadline** (2026-07-27): S4 — `service_cases.sla_events`, one jsonb column with two CHECKs (the stamp · extend ONCE). The DEADLINE itself is deliberately not a column: it is `opened_at + 14 working days`, derived in shared. Prior 0297: `commission_run_state` stops computing an `unattributed` count nothing reads (STABLE marker kept — CLAUDE.md §8 fix 3). Prior 0296: attribution stops being a worklist and becomes a rule. Drops `hr_assign_salesperson` (the repair RPC nothing calls any more), removes the now-unclearable `unattributed_orders` gate from `commission_close_month`, and adds `orders_salesperson_required` — every order the portal writes must name who sold it; only `source_system = 'autocount'` archive rows are exempt. Prior: 0295 rental payment failed · 0294 takeout-from-free · 0293 service case follow-ups · 0292 ready-stock pool usage · 0291 supplier claim lifecycle. **Rules that survive**: (1) `list_migrations` the tracker tail immediately before numbering AND again before applying. (2) Migrations apply manually via MCP; never renumber APPLIED files, but DO renumber an unapplied draft. (3) Reconcile the FILE back to live after apply by comparing `md5(prosrc)` + length per function — do not eyeball it. (4) **An assertion that RAISEs inside its own `EXCEPTION WHEN sqlstate` handler catches itself and proves nothing** — set a flag, check it outside the handler. (5) **The dry-run payload must contain EVERY object the migration changes** — omitting one silently tests the OLD body (cost a round on 0279 and 0281). (6) `revoke execute … from anon` alone does NOTHING and `from public` alone does nothing either — the pair PLUS an explicit grant back, asserted both directions. (7) Inside a transaction, read `audit_log` rows back by `ref`, never by time. (8) **A CHECK constraint ACCEPTS a NULL result** — `col = 'x'` against a NULL column yields NULL, `false OR NULL` is NULL, and the row passes. Wrap the comparison in `coalesce` or the constraint guards nothing (0296, caught by its dry run). (9) **A sanity block that greps the new function body can match the migration’s own comment** — assert on the RAISE form, not the bare word (0296, caught by its first apply). |
...
| Web bundle | **LIVE = `index-CKQhEuFh.js` from main tip `d61a3867`** (PR #451 Ready Stock K5; carres-portal `e8397fe2` + carres-pos `70fd225d`, both `--branch=main`; **all 4 canonicals converged on the first poll**; downloaded to a file then grepped — 4,410,666 bytes, `SERVICE_ROLE` 0, four markers from MOUNTED components present: `Set a number` · `Not selling` · `Did the plan work` · `we ordered too little`. **`Nothing is watched yet` greps 0 and that is CORRECT** — the headline sentence is computed SERVER-side by `stockHealthHeadline`, so the web tree-shakes it; picking it as a proof marker would have read as a failed deploy). **This deploy raced a parallel line and won on containment, not on time**: PR #449's `index-Ds-5K6Ke.js` from `ffa5d623` was live minutes earlier, and `git merge-base --is-ancestor ffa5d623 d61a3867` proves this bundle CARRIES it — check that before assuming a newer hash dropped somebody's work. **NOTE main is AHEAD of live**: PR #452 (service case S4) merged after this deploy — its migration 0298 was already applied, but its CODE is not on the canonicals; the next deploy carries it. **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty, deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals and POLL until they converge. **A canonical serving a DIFFERENT hash is usually the OLDER bundle still cached** — `wrangler pages deployment list` names the true last writer, then `git merge-base --is-ancestor <theirs> <yours>` proves containment. A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file first. **Pick proof markers from MOUNTED components**: a string from a retired page is correctly tree-shaken and greps 0. **`wrangler` lives in `apps/api`, not `apps/web`** — run Pages deploys from `apps/api` with `CLOUDFLARE_ACCOUNT_ID=e2494242a0fd563cacee5a301cf95dd3` (two accounts are authorised; without it wrangler stops and asks). bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker version `6a4a9b5c` from main tip `d61a3867`** (PR #451 — the read-only `GET /api/ops/stock/health`; all 312 migration files on the tip confirmed applied first, tail `0298`; receipt read back: `PUBLIC_WEB_URL: https://pos.carresofficial.com` + the custom domain, and `wrangler deployments list` shows `6a4a9b5c` at 100%. **Everything behind this Worker's auth middleware answers 401 BEFORE routing**, so an unauthenticated curl cannot tell a live route from a missing one — a nonsense path answers 401 too. The deploy receipt is the proof; a 401 is not.) Same containment + AHEAD-of-live notes as the web row. **The api deploy is `wrangler deploy --env production`, NEVER bare `wrangler deploy`** — both stanzas share `name = "carres-portal-v2-api"`, so a bare deploy overwrites production with the default env: `PUBLIC_WEB_URL=http://localhost:5173` and NO `api.carresofficial.com` route. Done by accident once and corrected within a minute; **wrangler echoes the bindings it deployed — read them, they are the receipt** (this deploy's receipt: `PUBLIC_WEB_URL: https://pos.carresofficial.com` + the custom domain). Same union-tip rule as web. |
...
- `row-dots-of-is-dead-code` — `rowDotsOf` (the three-dot row signal) is exported and unit-tested but nothing renders it; its strings grep 0 in the live bundle. Decide at C1 (which re-specifies that column): revive it with the per-dot icons Law 6 wants, or delete it with its tests.
...
- `rental-payment-failed-event-not-subscribed` — **0295 is inert until someone looks**: the Stripe endpoint must be subscribed to `invoice.payment_failed` or the new branch never fires. Not checkable from a session (the MCP key is refused on `GetWebhookEndpoints`) — one look in the Stripe dashboard, no code.
...
- `case-sla-no-closed-at` — **S5 must fix this in its own migration**: `service_cases` has no `closed_at`, so "avg days to close" and "SLA hit rate" (both S5's own asks) cannot be computed for any case already closed. 0293's trigger already detects the transition into a closed status and can carry the stamp.
