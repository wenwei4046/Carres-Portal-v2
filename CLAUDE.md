...
| Latest migration | **Applied tail = 0280_delivery_photos** (2026-07-26, PR #384 — delivery queue T6): ONE additive column, `ops_order_control.delivery_photos` jsonb `'[]'` ledger of `{path, at, by}` + an is-array CHECK. **`delivered_at` deliberately NOT added** although the card asked for it — `orders.delivered_at` exists since 0019 with real writers (partner deliver RPC, bulk-complete, 0106 auto-status); a second copy on the overlay is the samantha-hole shape. Bucket REUSED, not created: the private `proof-of-delivery` bucket (0069) under `order/{order_id}/` (partner flow keys `{thread_id}/` — no collision), zero storage-policy changes because the Worker signs both directions with the service client after its own role gate (0069's read policy still names the pre-0121 'logistics' role, so user-JWT storage was never a working HQ path). Dry-run in a rolled-back txn proved the CHECK rejects a non-array with the flag-outside-the-handler pattern (rule 4). Prior: 0279 rental signed at birth · 0278 HR-P7 staff_comp · 0277 delivery two-stage booking. **Rules that survive**: (1) `list_migrations` the remote tracker tail immediately before numbering AND again before applying — parallel lines take numbers within minutes. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber APPLIED files, but an unapplied draft SHOULD be renumbered. (3) Reconcile the FILE back to live after apply — compare `md5(prosrc)` + length per function, do not eyeball it. (4) **An assertion that RAISEs inside its own `EXCEPTION WHEN sqlstate 'P0001'` handler catches itself and proves nothing** — set a flag and check it outside the handler (0279's first dry run silently un-tested four assertions this way). (5) `revoke execute … from anon` alone does NOTHING, and `from public` alone does nothing either — the pair PLUS an explicit `grant … to authenticated, service_role`, asserted both directions in the migration's own sanity block. (6) Inside a transaction, read `audit_log` rows back by `ref`, never by time. |
...
| Web bundle | **LIVE = `index-ChJLxdiq.js` from main tip `984d9d0b`** (PR #386 delivery T7 queue split + auto-overdue, union with the parallel line's PR #385 J1 Documents; carres-portal + carres-pos both deployed `--branch=main`; **the POS pair served the J1 line's older bundle on the first poll and converged on the next** — the documented cache behaviour, and `wrangler pages deployment list` + `git merge-base --is-ancestor` proved the last writer `8020510` was contained before deploying; live bundle downloaded 4,228,707 bytes, `SERVICE_ROLE` grep 0, three T7 markers + the J1 marker present). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after and POLL until they converge. **A canonical serving a DIFFERENT hash is usually the OLDER bundle still cached** — this deploy saw exactly that (carres-portal served PR #376's `index-BDR0dVav.js` for ~1 min, and fetching that asset returned a 1,724-byte 404 because it no longer existed; it converged on the next poll). `wrangler pages deployment list` names the true last writer, then `git merge-base --is-ancestor <theirs> <yours>` proves containment. A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. **Pick proof markers from MOUNTED components**: a string from a retired/unmounted page is correctly tree-shaken and greps 0, which reads as a failed deploy when nothing is wrong. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker `f96b3e19` from main tip `984d9d0b`** (PR #386 — T7 adds `delivery_photos` to the operation orders LIST select so the "Upload delivery photo" queue can count delivered orders with no proof; one additive select line, no migration. Deployed with `--env production`; the bindings receipt read back correct — `PUBLIC_WEB_URL=pos.carresofficial.com` + the custom-domain route — and unauth 401 verified on BOTH `api.carresofficial.com` and workers.dev). **The api deploy is `wrangler deploy --env production`, NEVER bare `wrangler deploy`** — both stanzas share `name = "carres-portal-v2-api"`, so a bare deploy overwrites production with the default env: `PUBLIC_WEB_URL=http://localhost:5173` (where Stripe Checkout would return customers) and NO `api.carresofficial.com` route. Done by accident on this deploy and corrected within a minute; **wrangler echoes the bindings it deployed — read them, they are the receipt.** Same union-tip rule as web. |
