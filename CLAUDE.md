...
| Latest migration | **Applied tail = 0263** (verified at apply 2026-07-26): **0261-0263 guarantee packages** (0261 = `product_category` + 'guarantee' ALONE — PG forbids USING a new enum value in the txn that adds it, same split as 0169-0172; 0262 = `guarantee_terms` + `guarantee_entitlements` + the order_lines mint/void triggers + the orders sync trigger + the `guarantee_claim` / `guarantee_attach` DEFINER RPCs; 0263 = the RM150 / 15y / one-for-one Mattress Guarantee seed, NOT dormant). Prior: 0260_hr_duty_keys (parallel line) · 0259_org_departments · 0258 · 0257 · 0254-0256. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8; collisions renumber at apply. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) Full per-migration history → archive doc + worklog. |
...
| **Guarantee packages** (6th SKU category — RM150 Mattress Guarantee, 15y, one-for-one swap; entitlement ledger + POS covered-item picker + invoice block + claim desk) | ✅ shipped, undeployed | 2026-07-26 | 0261-0263 · `docs/guarantee-package-spec.md` |
