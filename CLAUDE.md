...
| Latest migration | **0166** `ops_bulk_complete_orders` (next free: **0167**). New since 0158: 0159 = `ops_order_control` overlay table · 0160 = q6 test-data cleanup · 0161 = control `delivery_time_slot` · 0162 = ops cockpit `ops_notes`+`ops_tasks` · 0163 = notes.archived · 0164 = tasks assign = operation-role only + shared account → "Operations" · 0165 = control payments cols (`balance`/`storage_from`/`storage_fee_override`) · 0166 = bulk Mark-completed RPC (SECURITY DEFINER, AutoCount-scoped). 0159-0166 applied via project MCP, live verified. **TRACKER GAP** (older): `supabase_migrations.schema_migrations` rows for 0155-0158 missing though schema fully live (verified 2026-06-08). Backfill only if a clean `list_migrations` matters. |
...
| Orders state | **155 live orders** in the Orders control list (verified 2026-06-12 preview; post-0160 q6-test cleanup, cancelled excluded server-side); bulk Mark-completed sweep of the ~105 overdue AutoCount legacy rows still pending (Jess action). 0 have `delivery_stops` set — multi-leg chain never live-exercised (see §17.3 Pending). |
| Test count | api 730/733 (3 pre-existing fails) · web 535/540 (5 pre-existing fails) · shared 193/193 — measured 2026-06-12 post P1-P3+P8; all 8 fails pre-existing per §17.7, zero new regressions |
| Web bundle | live = `index-CEfqcYcZ.js` (Pages hash deployment `dffc2daf`, deployed 2026-06-12 evening — carries Payments panel + Orders V2 + bulk complete). **main is ahead of live**: P1-P3 + P8 (2026-06-12 late session) committed, NOT yet deployed. Bundle-size regression CF still open, see §17.5 |
| API bundle | 1302.31 KiB raw / 245.32 KiB gzipped — live Worker version `e05f1a76-c787-4a31-a94d-b78a41cdd5e4` deployed 2026-06-12 evening (carries `/operation/payments` + `/operation/orders/bulk-complete`, verified 155 rows with an operation JWT) |
