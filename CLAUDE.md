...
| Latest migration | **Applied tail = 0269_hr_employee_master** (2026-07-26, HR-P4) — the people spine: `hr_employees` stores ONLY fields with no home on the identity tables (no `full_name`/`staff_code`/`status` copies — identity is read THROUGH the join), because `app_users.status` became load-bearing for auth in 0266/0267 and a second cosmetic `status` is how HR marks someone resigned, believes access is gone, and is wrong. Plus `hr_employment_events` (lifecycle only — promotions stay in `org_position_history`), `hr_employee_documents`, `hr_checklist_items` (ticks only; the LIST is a `@carres/shared` constant), the hr-only private `hr-docs` bucket, 10 DEFINER RPCs on the post-0266 fail-closed gate, and a 9-row backfill filtered by CR code. Prior: 0268_rental_approver_gate (agreement born `pending_approval`, materialises nothing until finance approves) · **0267 — THREE of them** (parallel lines minutes apart): 0267_rental_agreement_templates · 0267_guarantee_id · 0267_auth_hook_wont_mint_a_role_for_disabled. Then: 0266_disabled_accounts_actually_lose_access · 0265_hr_source_exclude_imported_archive · 0264_rental_offers. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8; collisions renumber at apply. (2) Migrations apply manually via MCP; the tracker keys on TIMESTAMP, so dual/triple-numbered FILES are cosmetic — never renumber applied files. (3) If you flatten any character in the apply payload, reconcile the FILE back to live — file-equals-live beats a tidier file. (4) Full per-migration history → archive doc + worklog. |
...
| Web bundle | **LIVE = `index-CgJfvG3c.js` from main tip `b25fa2bc`** (PR #341 HR-P4 People; carres-portal `ab2b36cc` + carres-pos `9b76d8bf`; all 4 canonicals verified after convergence; downloaded 4,145,721 bytes, `SERVICE_ROLE` grep 0). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after. **The two projects converge INDEPENDENTLY — carres-pos routinely lags carres-portal by ~1 min. Poll each until it flips; do not re-deploy.** A canonical serving a DIFFERENT hash right after your deploy is usually the OLDER bundle still cached, not a clobber — check `wrangler pages deployment list` before reacting. A piped `curl` into `grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker `aa03fe76` from main tip `b25fa2bc`** (PR #341 — `/api/hr/people` + the new HR access door; unauth `/api/hr/people` and `/api/hr/people/:id/access` both 401 live-verified on the workers.dev AND custom domain). Same union-tip rule as web. |
| Catalog state | 11 suppliers · 171 product_models · ~1150 product_skus (1013 AutoCount + SVC + minted compartment skus; 123 compartment skus flipped ON 2026-07-24, mostly RM0 pending pricing). Bundles: 1 active (King Bedroom Set RM2500). PWP/free-gift/delivery-fee/rental config tables exist, dormant until authored. |
| Orders state | **55 orders** (counted live 2026-07-26 — the old "~190 / 153 AutoCount" figure predated the 2026-06-24 catalog/data reset and was stale): **18 native POS orders, ALL 18 attributed**, + **37 `source_system='autocount'` archive rows** imported 2026-07-23, all unattributed and all carrying RM 0 of line value. That split is why HR-P3 was cut and why 0265 excludes archive from the attribution worklist. `dealers.channel` is THE showroom-vs-dealer authority (PR #226). |
| Test count | Baselines + pre-existing fails → §17.7. Run full suites before merge; ZERO new failures is the bar. |
