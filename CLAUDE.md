...
Next decision pending:
  ...
  2. Phase 5 Finance — Chunk A IN FLIGHT 2026-05-08 evening (server complete + web shell + Dashboard skeleton). Spec at `docs/superpowers/specs/2026-05-08-phase-5-finance-spec.md`. 9 Q's locked at 21:42. Seven commits pushed to origin/main this session: `8ecb62b` spec, `c459db9` migrations 0061+0062, `02efa78` server foundation, `61bd6b2` +18 route tests, `50468e3` invoices+refunds routers +24 tests, `0337d06` web queries.ts (qk.finance.* + 10 hooks), and pending FinanceApp shell + Sidebar + Dashboard skeleton (+3 web tests, App.tsx /finance/* mount + HomeRedirect finance role). Acceptance progress: A1+A2+A3 server-side complete; web Dashboard renders 4 KPIs + AgingCard via useFinanceDashboardSummary + useFinanceArAging hooks. Next: AR/AP/Payments/Invoices pages (Chunk A 5 pages remain); ~22 component tests; 3 Playwright E2E specs (A1+A2+A3 acceptance flows). Chunk B: recon tables already exist via 0061; need finance_recon_suggest_matches RPC + reconciliation.ts router + reports.ts ap-aging/cashflow/monthly-pl/top-skus + FinanceRecon + FinanceReports pages. Chunk C: refunds.ts already partial; need /apply route + FinanceRefunds + FinanceInvoices full pages + invoice PDF render via @react-pdf/renderer (Q7=A server-side).
