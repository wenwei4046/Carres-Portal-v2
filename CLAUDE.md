...
| Web bundle | **built + deployed 2026-07-18 from main `7f4769d` (New Order POS-parity, PR #175; Pages deploy `6f699845`, bundle `index-Dk_ut6QS.js`); `SERVICE_ROLE` scan on dist = 0; canonical https://carres-portal.pages.dev live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
...
- **2026-07-18** · **New Order POS-structure parity** (PR **#175**, merge `7f4769d`, NO migration) — MAINTAIN → New Order rebuilt as the POS 3-step flow: 01 Items = SKU-search-first over the FULL admin catalog (a hit opens the SAME POS configure surface with the variant preselected — spec fields follow the product; prices stay editable, custom lines kept) · 02 Customer = the POS `CustomerStep` VERBATIM with `rawDates` (no lead-time floor, past dates OK, empty = TBD) · 03 Confirm = POS payment+signature layout, everything OPTIONAL (`rawMode`, no Stripe). Raw door extended (full customer block incl. 0230 structured address, delivery extras, addons, line spec attrs with `pwp`/`free_gift`/`free_item` marker-strip + client `DELIVERY*` addon drop, optional payment/signature/entryData); minimal body byte-identical; NO POS engine runs. Shared-component changes additive (`initialSkuId`, `rawDates`, optional `minIso`, `rawMode`, keyed draft storage — POS byte-identical when unset). Deploy api `8c75d0e0` / web `6f699845` (`index-Dk_ut6QS.js`). NOTE: the raw path's OrderSummaryRail preview would show delivery-fee/gift previews the raw door never charges if those engines ever go live (dormant today) — revisit then.
