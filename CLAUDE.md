...
| **Portal Core C4 — Purchase and Payments stop saying Chase** (both pages read the shared word module; the card's own `Call to collect` refused because the money action already has ONE spelling; `Collect RM RM 2,455.00` and a guard blind to interpolated labels fixed on the way past) | ✅ LIVE | 2026-07-27 | PR #484 · no migration · `docs/portal-core-execution-queue.md` |
...
- **2026-07-27** · **Portal Core C4 — Purchase and Payments stop saying Chase** (PR #484, no migration, **web only — the tip changes no file under `apps/api`**) — the last two surfaces carrying the banned word now read their action words from C1's shared module instead of typing them. Purchase's stage ② is `Confirm ready date` on the facet, the list header and the calendar lens C1 had already renamed; its detail pane's title becomes the action with the factory in it (`Call Ohana — confirm ready date`) and the footer button drops to **`Open WhatsApp`**, because COPY-STANDARD's verb table rules that **the CHANNEL is not the action** — the header says WHAT, the button says HOW. Payments' queue is `Collect`, one order reads `Collect RM 2,455 from John Tan`, the last-touched stamp becomes `Last message …` (what is stamped is a message being sent) and the two tones become `Remind` / `Call`. **The card's own wording lost to the law, which is the interesting part**: C4 asked for `Call to collect` / `Call {customer} — collect balance RM X`, but COPY-STANDARD's audit table, Law 4 rung 5 and the shipped module already spell that action — building the card's words would have **forked the money action across two pages**, the precise failure the card exists to end, arrived at by following the card. The four laws outrank a card, so the law's words shipped and the card's are reported for Jess. **Two defects found ON the lines being rewritten and fixed rather than filed**: the desk printed **`Collect RM RM 2,455.00`** (every call site passed `rm()`, which already carries the currency word, into a module that adds `RM ` itself — so one order read two ways), and **`Last chased {date}` survived in the order drawer** under a guard whose banned list already held `/chase[ds]?/` — C1's JSX matcher skipped any text node containing an interpolation, which is exactly where party-named labels live. The scanner moved to `apps/web/src/test/banned-words.ts` with that hole closed, and **Purchase + Payments, which had no test file at all** (which is why nine visible `Chase` strings lived on them), ship the same guard. Negative control: restoring the two strings fails 4 tests. **Six findings reported**: UI-KIT §A0's Action-law table + new-panel checklist still name `Chase supplier` / `Chase logistic` / `Confirm` / `Collect $`, so **a new panel built from the checklist re-seeds every word C1-C4 removed** · COPY-STANDARD still tells a chat to build `Confirm delivery with {customer}` (C3's, unchanged) · the card-vs-law money wording needs Jess's pen · `GRN` is on screen in the same grid row C4 rewrote while rule 9 bans it · Purchase says "factory" and Payments says "supplier" for one party · `Confirm ready date` is now both a Purchase stage and an Orders queue holding different populations, and nothing says why the counts differ. Suites at baseline (web 16 pre-existing · 2062 passing); typecheck 0, build + v4 guard + lint clean.
