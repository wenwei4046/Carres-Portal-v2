...
| Latest migration | **Applied tail = 0295_rental_payment_failed** (2026-07-27, rental): a refused card becomes a recorded EVENT. `rental_billing_events.kind` gains `payment_failed` + a `stripe_event_id` unique index (a decline changes no state, so it has nothing else to be idempotent by — and Smart Retries make each ATTEMPT a genuinely new event); `rental_record_payment_failure` is its only writer and deliberately **never touches `rental_billings`**, keeping 0281's single-writer law and refusing to create a second source of truth for lateness; `rental_agreement_card_trouble` is a `security_invoker` view so the agreements LIST can show a card problem without opening a drawer. NOTE a parallel line has **0296** applied whose code is not yet in main. Prior: 0294_takeout_from_free_says_why (Ready Stock K4): the shared ready pool stops being a black box. `ops_stock_pool_usage` is a DATED ledger (one row per draw: sku · qty · reason · who · when), written ONLY by `ops_stock_pool_draw`, which takes the unit and records the reason in ONE transaction and serves both reserve doors; `ops_stock_reserve_levels` + `ops_set_reserve_level` hold the COO's floor per SKU (`stock_planner`, zero new duty keys). 0294 closes the THIRD door — `Takeout` off a FREE row demands a reason, off a RESERVED one records none (already ledgered). 0213's `ops_stock_items.reserve_reason` is superseded and now has no writer (CF). Prior: 0292 K4 base · 0290 urgent restock · 0287 ready-stock plan · 0286 reorder points · 0281 rental collection ledger. **Rules that survive**: (1) `list_migrations` the tracker tail immediately before numbering AND again before applying — **it fired TWICE on K4 alone** (0291 and 0293 were both taken by parallel lines mid-dry-run). (2) Migrations apply manually via MCP; never renumber APPLIED files, but DO renumber an unapplied draft. (3) Reconcile the FILE back to live after apply by comparing `md5(prosrc)` + length per function — do not eyeball it (0292 diverged by ONE re-wrapped comment line at identical length; only the hash caught it). (4) **An assertion that RAISEs inside its own `EXCEPTION WHEN sqlstate` handler catches itself and proves nothing** — set a flag, check it outside the handler, and run a negative control so you know a failure would actually surface. (5) **The dry-run payload must contain EVERY object the migration changes** — omitting one silently tests the OLD body. (6) `revoke execute … from anon` alone does NOTHING and `from public` alone does nothing either — the pair PLUS an explicit grant back, asserted both directions in the migration's own sanity block. (7) Inside a transaction, read `audit_log` rows back by `ref`, never by time. (8) Adding DEFAULTED parameters to a live RPC creates a SECOND signature, not a new version — drop the old one explicitly and assert exactly one survives. |
...
**HIGH**:
- `payhold-blind-to-the-payment-ledger` — **verified live 2026-07-27, scheduled as C5**: `ops_order_control.balance` NULL ×55 AND `order_payments`/`payments` hold **0 rows with no writer** — `orders.paid` is the only money number (written by `top_up_order`). So the ladder's 🔒 never fires AND `bookingConfirmGate` refuses every priced order's booking (SO-1209 is paid in full and blocked today). 18 orders genuinely owe RM 56,859. Fix reads `orders.paid`, never the empty ledger.
