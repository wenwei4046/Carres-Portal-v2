...
- `case-evidence-prose-path-ungated` — the S2 gate binds to `issue_type`, not to the endpoint, so a create with no issue type demands no evidence (deliberate — the edit modal's prose-only path). Also: PATCHing an issue type onto a legacy evidence-less case now violates 0287's floor CHECK; no live path does it.
