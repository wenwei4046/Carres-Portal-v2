...
| Latest migration | **Applied tail = 0255** (verified at apply 2026-07-25): 0244-0246 HR · 0247-0249+0253 Rental base · 0250-0252 BD commission · 0254_hr_team_hierarchy (hr line) · **0255_rental_sell_lane** (renumbered from 0254 at apply — the hr line took 0254 mid-session; guardrail #8 pre-check caught it). **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8; collisions renumber at apply (0239/0241 lessons). (2) Migrations apply manually via MCP; tracker keys on timestamp, so dual-numbered FILES (0165/0166/0233/0241…) are cosmetic — never renumber applied files. (3) Full per-migration history → archive doc + worklog. |
...
| Web bundle | **LIVE = `index-C6DPMO9M.js` from main tip `84e1397`** (PR #280 Rental-plans table overlap fix on top of the #275/#276 union; carres-portal `4cdb5ab2` + carres-pos `72b9a442`; all 4 canonicals verified; `SERVICE_ROLE` grep 0). Prior union bundle `index-Cjl110n-.js` (`8503184` (union: PR #275 rental sell lane + PR #276 hr hierarchy + #273; carres-portal `2666f531` + carres-pos `e205c12c`; all 4 canonicals verified; live bundle downloaded IN FULL — 3,999,649 bytes, Rent-to-Own markers present, `SERVICE_ROLE` grep 0; the hr line's interim deploy `77a6d490`/`c6838eda` from `609f22a` was superseded minutes later by this union build — re-verified 4/4 canonicals serve the union). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after (~15s edge lag); a piped `curl \| grep` on the ~3.9 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). |
| API bundle | **LIVE = Worker `c00ae4fc` from main tip `8503184`** (PR #275 rental sell lane `/api/rental/pos-plans|agreements|stripe` + #276; unauth 401 + /health live-verified). Same union-tip rule as web. |
