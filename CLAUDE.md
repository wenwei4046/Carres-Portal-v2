...
| Web bundle | **built + deployed 2026-07-19 from main `69d153c` (showroom minimal account creation, PR #206; bundle `index-CQDonvs7.js`, deployed to BOTH Pages projects — carres-portal `fcbca173` + carres-pos `eb0f78ba`, `--branch=main`); `SERVICE_ROLE` scan on dist = 0; both pages.dev canonicals live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 from main `69d153c` (Worker version `32ee6b3c-8351-4030-b754-237d2d89bd88` — showroom create input relaxed [SSM/PIC dealer-only in zod] + dealers-insert null-guards; carries everything through 0230–0234; GET /api/catalog → 401 live-verified).** |
