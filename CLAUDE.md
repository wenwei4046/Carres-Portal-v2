...
| Web bundle | **built + deployed 2026-07-18 from main `8e836c7` (New Order inline row variants, PR #178; Pages deploy `e75d9c37`, bundle `index-sUFkMr_A.js` — also carries the ops journey-card work `15e75a8`); `SERVICE_ROLE` scan on dist = 0; canonical https://carres-portal.pages.dev live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-18 from main `039d33d` (Worker version `2e6c6717-d21a-4a11-addd-b1e0648e1e4d` — carries the extended raw door [PR #175] + add-product P1 [PR #174, migration 0231 verified on prod]; GET /api/catalog → 401 live-verified).** |
