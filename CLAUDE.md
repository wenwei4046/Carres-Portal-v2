...
| Web bundle | **built + deployed 2026-07-19 (v3, latest) from main `51f01dba` (PR #213 merge — bundle `index-DZeSAUEH.js`: POS customer-step fixes [birthday wheel opens UP `.dki-pop--up` · structured billing MY-cascade `billing*` fields, composed at submit, gates mirror delivery, autofill re-parses · Building type select → `entry_data.fields.building_type`, shown on CONFIRM + PosOrderDetail + ops drawer CustomerIdentityCard · ThankYou View-orders button REMOVED, onClose prop retired] — strict superset of the day's `index--8-5zdIF.js` [built pre-#213 from `505d756e`: #208–#216] and `index-ItuJHcJ4.js`; deployed to BOTH Pages projects — carres-portal `89b0851a` + carres-pos `eff1d9af`, `--branch=main`; `SERVICE_ROLE` scan on dist = 0; carres-portal.pages.dev + pos.carresofficial.com RE-verified cache-busted AFTER the parallel v2 deploys — `index-DZeSAUEH.js` is what serves). ⚠️ Same-day parallel-deploy race hit 3×: before ANY Pages deploy `git fetch` + `git log HEAD..origin/main`, rebuild from the union tip, and RE-curl the canonicals afterwards. ⚠️ design-standard lint currently fails on main: PR #215's `BundleConfigurePage.tsx` carries 5 un-baselined hex literals (pre-existing, not #213).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 (v3, latest) — Worker version `acac8ec6-063b-46ce-bcf6-33f6f3329130`, built from the post-#213 union tree (GET /api/operation/orders/:id now also selects `entry_data` [ops drawer building type, PR #213] + staff PATCH `phone` [PR #216] + BOTH 0241s; deployed at 07:15Z to guarantee the last-writer Worker carries the union — content-equivalent to the parallel session's `48640203` (also a union build per its v3 record) and supersedes the same-hour `ff0994f2` / `3af3e6b1`; GET /api/catalog → 401 live-verified).** |
