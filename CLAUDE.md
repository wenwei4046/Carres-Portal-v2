...
Next decision pending:
  ...
  2. Phase 5 Finance — Chunk A NEAR-COMPLETE 2026-05-09 00:25 GMT+8 (server complete + 4 of 5 Chunk A web pages live: Dashboard / AR / Payments / AP; FinanceInvoices is the only Chunk A stub left — moves to Chunk C scope per spec). Spec at `docs/superpowers/specs/2026-05-08-phase-5-finance-spec.md`. 9 Q's locked at 21:42. Fifteen commits pushed to origin/main across 2 sessions: 2026-05-08 evening (9 commits): `8ecb62b` spec, `c459db9` migrations 0061+0062, `02efa78` server foundation, `61bd6b2` +18 route tests, `50468e3` invoices+refunds routers +24 tests, `0337d06` web queries.ts (qk.finance.* + 10 hooks), `a9f4614` FinanceApp shell + Sidebar + Dashboard +3 web tests + App.tsx /finance/* mount, `9d75f0e` FinanceAR + ARDrawer +4 web tests, `b112d8d` FinancePayments read-only view (+4 web tests). 2026-05-09 FinanceAP session (6 commits): `8f1fb8b` chore web hoist rm/rmCompact (no behavior change, pre-emptive cleanup before 5th+6th copy), `4bc3397` migration 0063 (3 RPCs: ap_aging STABLE + po_pay + po_schedule), `6a0031b` API routes (po-pay/po-schedule/ap-aging) + zod schemas, `76faaca` API tests (+12), `c23081f` web hooks (apAging + usePoPay + usePoSchedule + 6 type interfaces), `7e46326` FinanceAP page + APDrawer + tests (+7). Acceptance progress: A1+A2+A3 server-side complete; web Dashboard live + AR page with full mutation drawer (record receipt + issue invoice gated) + Payments page read-only with progress bars + AP page with 3-way match drawer (Schedule + Mark paid actions gated by 5-bucket pay_status_ui derived from po.status/sup_status/pay_status). Schema NON-finding (key insight from FinanceAP session): `purchase_orders.pay_status` (3-value enum unpaid|scheduled|paid) was already in 0001 since day 1 — no new column or pay_schedule table needed; the proto's 5-bucket payStatus is fully derivable from existing columns. Avoided what was thought to be a schema fork. Next: FinanceInvoices page (now Chunk C scope per spec §8 Q8=A); 3 Playwright E2E specs (A1+A2+A3 acceptance flows). Chunk B: recon tables already exist via 0061; need finance_recon_suggest_matches RPC + reconciliation.ts router + reports.ts cashflow/monthly-pl/top-skus + FinanceRecon + FinanceReports pages (ap-aging route already shipped 2026-05-09). Chunk C: refunds.ts already partial; need /apply route + FinanceRefunds + FinanceInvoices full pages + invoice PDF render via @react-pdf/renderer (Q7=A server-side).
