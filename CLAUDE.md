...
| Latest migration | **Applied tail = 0264** (verified at apply 2026-07-26): **0264_rental_offers** (the Rental SETTING closed loop — `rental_offers` per model + `rental_buy_prices` + `rental_offer_services` + `rental_plans` offer/combo/line_kind/gifts + `service_packages.category` + agreement snapshot cols). Prior: **0261-0263** (guarantee category/entitlements/seed, parallel line) · **0260_hr_duty_keys** (HR-P2 — `org_duties` + `org_position_duties` + self-only `my_org_duties()` + list-read `org_duty_holders()` + audited `hr_set_position_duty()`; folds the missing audit into `hr_set_reports_to`) · 0259_org_departments · 0258 (edit_order_addon) · 0257 (replace_lines + p_addons_append) · 0254-0256. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8; collisions renumber at apply. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) Full per-migration history → archive doc + worklog. |
...
| Web bundle | **LIVE = `index-5c_O6Gw0.js` from main tip `b62c2b61`** (PR #321 SKU-Master row/header grid-template alignment; the UNION also carries the parallel line's #319 HR-O1 Overview + #318 + #314 guarantee packages — live bundle greps BOTH lines' markers, nothing lost; carres-portal `eeb15f16` + carres-pos `114bee82`; all 4 canonicals verified; downloaded 4,082,408 bytes, `SERVICE_ROLE` grep 0). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after. **When a parallel session deploys around the same time, a canonical can briefly serve THEIR bundle — check `wrangler pages deployment list` for the true last writer and confirm your commit CONTAINS their merge before assuming a clobber; `erp.carresofficial.com` lags ~1-2 min.** A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). |
| API bundle | **LIVE = Worker `cdc6b800` from main tip `5a9c5380`** (PR #319 `legacyUnattributed` + `monthSold` on /api/hr/report; unauth 401 live-verified; deployed AFTER 0265 was applied). Same union-tip rule as web. |
...
- `hr-rpc-null-role-gate-shape` — every `hr_*` DEFINER gate is `app_role() not in (...)`, which is NULL (→ passes) for a user with no `app_users` row; open-on-unknown. Fix = one `coalesce(...)` sweep across all of them.
