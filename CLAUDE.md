...
| Web bundle | **LIVE = `index-CAb0V6zA.js` from main tip `0ff8f1bc`** (PR #315 Rental offers, union with HR-P2 #312/#313; carres-portal `8bd30efe` + carres-pos `efebec76`; 4 canonicals verified; full bundle 4,062,610 bytes, `SERVICE_ROLE` grep 0; NOTE: right after a deploy the asset URL can briefly serve the SPA fallback index.html — a <3MB "bundle" is the fallback, retry). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after (~15s edge lag); a piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). |
| API bundle | **LIVE = Worker `7fb8505c` from main tip `0ff8f1bc`** (PR #315 `/api/rental/offers*`; unauth `/api/rental/config` 401 live-verified). Same union-tip rule as web. |
