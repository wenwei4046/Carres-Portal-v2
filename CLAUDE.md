...
| API bundle | **LIVE = Worker version `2a0b5707` from main tip `9b73fb17`** (PR #457 — `subscriptionIdOf` reads BOTH invoice shapes: the endpoint is pinned to `2025-02-24.acacia` while our SDK is Basil-era, and without the fallback 0281's `invoice.paid` and 0295's `invoice.payment_failed` both silently dropped every rental invoice. Deployed api-only from the union tip, tracker checked first — tail 0298 and main's last migration file 0298, so nothing was ahead of the database. Verified live through the custom domain: unsigned webhook POST **400**, bad signature **400 `invalid_signature`**, `/api/rental/agreements` unauth **401**. Supersedes `6a4a9b5c`/`d61a3867`, which `wrangler deployments list` confirms was the PREVIOUS writer and `git merge-base --is-ancestor` confirms is contained in this tip.) **The api deploy is `wrangler deploy --env production`, NEVER bare `wrangler deploy`** — both stanzas share `name = "carres-portal-v2-api"`, so a bare deploy overwrites production with the default env: `PUBLIC_WEB_URL=http://localhost:5173` and NO `api.carresofficial.com` route. Done by accident once and corrected within a minute; **wrangler echoes the bindings it deployed — read them, they are the receipt.** Same union-tip rule as web. |
