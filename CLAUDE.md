...
| Web bundle | **LIVE = `index-A7uOnzQ8.js` from main tip `e657f282`** (PR #351 HR-P5 commission runs; carres-portal `ffea9a3b` + carres-pos `2f21ee5c`; all 4 canonicals converged; downloaded 4,167,316 bytes, `SERVICE_ROLE` grep 0, `hr/runs` present). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after. **The two projects converge INDEPENDENTLY — poll each until it flips; do not re-deploy.** A canonical serving a DIFFERENT hash right after your deploy is usually the OLDER bundle still cached — check `wrangler pages deployment list` before reacting. A piped `curl` into `grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker `2cf891df` from main tip `e657f282`** (PR #351 — `/api/hr/runs` close / approve / reopen / discard / paid / adjustments / CSV; all four unauth probes 401 live-verified on the custom domain). Same union-tip rule as web. |
