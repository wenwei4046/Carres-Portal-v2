...
| Web bundle | **LIVE = `index-Cmwz6ELk.js` from main tip `17fd71cd`** (Order Detail L1 freeze + Law 7's Decision A — the delivery photo stops being said twice; carres-portal `19b2744d` + carres-pos `248a7897`, both `--branch=main`; **all 4 canonicals converged on the first poll**; downloaded to a file then grepped — 4,451,600 bytes, `SERVICE_ROLE` 0. **This is a deploy a string grep structurally CANNOT prove, and that is stated rather than papered over**: the card removes no string and changes no code-path shape — it narrows what DATA reaches an existing health line, so `Delivery photo` correctly greps **5 in both** bundles and there is no new literal to find (the only new literals in the diff are test fixtures and a comment, neither of which survives minification). What proves it instead: `wrangler pages deployment list` names Production/main source **`17fd71c`** as the newest writer, all four canonicals serve the new hash, and the bundle differs from its predecessor by 93 bytes. **The predecessor had to be fetched from its OWN deployment URL** (`5d205889.carres-portal.pages.dev`) — a superseded asset 404s at the apex, and greps against that 1,757-byte error page would have read as a clean "0" for everything. **No api deploy, and this time it is not a judgement call**: `git diff b464257c..HEAD -- apps/api` is empty, so the live Worker already matches this tip exactly. Migration tail = 0300 = the applied tail, so the union tip carries nothing unapplied.) Prior receipt for reference — `index-CkzR0HuS.js` from main tip `fbfadbf5` (PR #487 — the drawer stops calling two different blocks `Actions`; the left rail's counterparty panel is `Calls`, the word Jess picked from that panel's own locked empty state. carres-portal `5d205889` + carres-pos `dc455145`, both `--branch=main`; 4 canonicals converged after two extra polls — ordinary edge lag on the older bundle, `carres-pos` flipped first. Downloaded then grepped — 4,451,507 bytes, `SERVICE_ROLE` 0. **Proved BOTH directions**: `Calls — 0 calls to make` greps **0** in the previous live bundle and **1** here, while the retired `Actions — 0 calls to make` and `No actions` grep **1** there and **0** here. No api deploy — `git diff` proves the tip changes no `apps/api` file and no migration.) Prior receipt for reference — `index-7GAkvYaD.js` from main tip `7a5579e0` (PR #486 Portal-Core C6 — every action opens the steps that close it; carres-portal `5006ee13` + carres-pos `52f2da7a`, both `--branch=main`; **all 4 canonicals converged on the first poll**; downloaded to a file then grepped — 4,451,521 bytes, `SERVICE_ROLE` 0. **Proved BOTH directions on downloaded files**: `order-action-step` · `order-action-toggle` · `order-action-steps` · `Record ready date` · `Confirm booking` grep **0** in the previous live bundle (`index-C18Onyrj.js`, 4,444,881 bytes) and **1** here. **No api deploy, and the reasoning is one step stricter than C3's**: this tip changes NO file under `apps/api`, and although `packages/shared` DID change, the diff is purely additive (a new module + one new field on the word dictionary) and `grep -rn "orderAction\|order-action" apps/api/src` returns **nothing** — the api's behaviour is identical on every code path. Redeploying it would drag in whatever parallel lines have merged, which is the T10 hazard (an api whose migration is not applied), so web-only is the safer choice, not the lazier one.) Prior receipt for reference — `index-C18Onyrj.js` from main tip `e40cf914` (PR #479 Portal-Core C3 — the Actions column's `+N` and the retirement of `Confirm delivery`; carres-portal `42768ede` + carres-pos `0ba72865`, both `--branch=main`; **all 4 canonicals converged on the first poll**; downloaded to a file then grepped — 4,444,881 bytes, `SERVICE_ROLE` 0. **Proved BOTH directions**: `Delivering` and `Nothing to do until` are present and the retired `Confirm delivery with` greps **0**. **No api deploy**: `git diff` proves this tip changes NO file under `apps/api`, and nothing in the api imports the action engine — the live Worker from C9's deploy already matches this tip's api.) Prior receipt for reference — `index-CsnNUn76.js` from `b464257c` (PR #474 Service-Case S5 — the Numbers tab; carres-portal `e1580e5e` + carres-pos `0f63abc4`, both `--branch=main`; downloaded to a file then grepped — 4,443,920 bytes, `SERVICE_ROLE` 0. **Proved BOTH directions against the previous live bundle**: `Filed before the questions` · `Average working days to finish` · `service-cases/numbers` · `case-numbers` all grep **0** in the prior bundle and **1** here. **`No factory on the case` correctly greps 0** — `CASE_SUPPLIER_UNKNOWN_LABEL` is read by the ENGINE, which runs in the Worker, so the browser never needs the string and it is rightly tree-shaken; the "pick markers from MOUNTED components" trap, met and understood rather than chased. **3 of 4 canonicals converged on the first poll**; `carres-pos.pages.dev` served `index-hp2T-FN4.js` for ~1 minute — `wrangler pages deployment list` named MY `0f63abc4` (source `b464257`) as the newest writer and `git merge-base --is-ancestor` proved `b464257c` contains a12e8ff · 11e9640 · 98dfd9c, so it was edge lag on the older bundle; it flipped on the next poll.) Prior receipt for reference — `index-CToiHJof.js` from `98dfd9c3` (PR #471 C10; carres-portal `e67004d8` + carres-pos `83d2c2b8`; 4,428,600 bytes, `SERVICE_ROLE` 0. **Proved BOTH directions against the PREVIOUS live bundle**: the dot tooltips (`Money — settled` · `Stock — all in` · `Delivery — customer confirmed`) and the `row-dot-` testid grep **0 in `index-BYggEOBr.js`** and **1** here — which is what makes "dead code is now on screen" a measurement rather than a claim. **A composed testid greps 0 as a whole**: `row-dot-${kind}` means the marker is the fragment `row-dot-`, never `row-dot-goods`. **C9 (#472) merged minutes after this deploy and had not yet shipped its own bundle** — when it does, that hash supersedes this one and still carries C10: `git merge-base --is-ancestor 98dfd9c3 a12e8ff6` passes, which is the containment proof the deploy rules ask for.) Prior receipt for reference — `index-BYggEOBr.js` from `bc92e4cc` (PR #466 C2; carres-portal `19d0fbbf` + carres-pos `a8c77e64`; downloaded to a file then grepped — 4,426,596 bytes, `SERVICE_ROLE` 0. **Proved BOTH directions**: the new strings are present (`Nothing to do on this order` · `A new action appears here by itself` · `order-action-list` · both rewritten tab tooltips) and the retired ones grep **0** (`Stock in AND the customer confirmed`, the old `To book` tooltip, and C1's `Chase logistic` / `Unscheduled` / `need booking` still at 0).) Carries every line merged up to #479. **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty, deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals and POLL until they converge. **A canonical serving a DIFFERENT hash is usually the OLDER bundle still cached** — `wrangler pages deployment list` names the true last writer, then `git merge-base --is-ancestor <theirs> <yours>` proves containment. A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file first. **Pick proof markers from MOUNTED components**: a string a page computes SERVER-side, or one from a retired page, is correctly tree-shaken and greps 0. **On a RENAME card, grep both directions** — the banned word at 0 proves only that nothing says it, not that anything says the new one. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker version `ace384c3` from main tip `b464257c`** (deployed with PR #474 S5, which adds `GET /api/ops/service-cases/numbers`. **Safe because every migration file on the tip was confirmed applied first — the repo's migration tail is 0300 and so is the tracker's**, so the union tip carries nothing unapplied. Wrangler's receipt read back: `PUBLIC_WEB_URL: https://pos.carresofficial.com` + the `api.carresofficial.com` custom domain + the 09:00-MYT cron; `GET /health` through the custom domain returns 200, and **the new route answers 401 unauthenticated, not 404** — which is what proves it is really on the live Worker rather than only in the bundle.) Prior: `739d4be8` from `bc92e4cc` (PR #466). **The api deploy is `wrangler deploy --env production`, NEVER bare `wrangler deploy`** — both stanzas share `name = "carres-portal-v2-api"`, so a bare deploy overwrites production with the default env: `PUBLIC_WEB_URL=http://localhost:5173` and NO `api.carresofficial.com` route. Done by accident once and corrected within a minute; **wrangler echoes the bindings it deployed — read them, they are the receipt.** Same union-tip rule as web. |
