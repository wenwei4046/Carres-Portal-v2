...
| Web bundle | **built + deployed 2026-07-19 (v4, latest) from the post-#219 union tip (bundle `index-0fheSKu0.js` — adds PR #219 [HQ create-account staff-profile parity + MANUAL passwords + reset-modal wrap fix] + #218 bundle-preview fix + #217 PromoTab tweak on top of the v3 superset [#208–#216]; deployed to BOTH Pages projects — carres-portal `1cf2f58c` + carres-pos `8c69bff6`, `--branch=main`; `SERVICE_ROLE` scan on dist = 0; pos./erp.carresofficial.com live-verified cache-busted). ⚠️ Same-day parallel-deploy race hit 4×: before ANY Pages deploy `git fetch` + `git log HEAD..origin/main`, rebuild from the union tip, RE-curl the canonicals afterwards. ⚠️ design-standard lint currently fails on main: PR #215's `BundleConfigurePage.tsx` carries 5 un-baselined hex literals (pre-existing, owned by the bundles workstream).** bundle-size regression CF still open, see §17.5. |
