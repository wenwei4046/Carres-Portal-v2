...
| Latest migration | **Applied tail = 0276_hr_kpi_targets** (2026-07-26). **HR-P6 shipped 0276**: `kpi_targets` + `kpi_manual_actuals` (both scoped by TWO nullable FKs with a CHECK, so `scope_kind` is derived and cannot drift from the id) + `dealers.manager_user_id`. Both tables get a **SELECT-only** RLS policy so the audited DEFINER RPC is structurally the only write door — the inverse of `staff_commission_rates`, whose `for all` policy is exactly why 0272 had to put its guard in a TRIGGER. **Numbered 0276, not 0275: the re-check immediately before applying found the parallel line had taken `0275_rental_makes_a_sales_order` during the verification pass** — guardrail #8 paying for itself. **Two defects the prod dry-run caught before apply**: (a) `revoke execute … from anon` alone left anon holding EXECUTE on all 5 functions, because a new function is created with EXECUTE granted to PUBLIC and anon inherits it — the pair `from public, anon` plus an explicit `grant … to authenticated` is what works (verified anon 0/5, authenticated 5/5, and a live call as `authenticated` still succeeding); (b) audit assertions read by `order by occurred_at desc` were returning an arbitrary row, because `occurred_at` defaults to `now()` = **transaction start**, so every audit row written in one txn shares a timestamp — read audit rows back by `ref`, never by time. Prior: 0274 guarantee+service · 0273/0272 commission runs: the month close — `commission_runs` / `commission_run_lines` (text snapshots so a rename or an offboarding cannot change last July's statement) / `commission_adjustments` **keyed to the TARGET open month, not to a run** (the spec's shape would have put a September refund onto July's frozen statement). Draft stays fluid; the month locks at **approved**. Two guards keep a locked month honest: attribution (a `perform` inserted into `hr_assign_salesperson` PROGRAMMATICALLY — 0266's technique, anchor asserted at exactly 1 hit) and back-dated staff rates (a TRIGGER, because `/api/hr/config/staff-rate` upserts the table directly through RLS and a route check would be bypassed by the next writer). Prior: 0271 guarantee sofa height · 0270 guarantee scope · **0269_hr_employee_master** · 0268 rental approver · **0267 x3** (parallel lines, tracker keys on TIMESTAMP so it is cosmetic) · 0266. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) If you flatten any character in the apply payload, reconcile the FILE back to live — for a whole function body, pull it out of `pg_get_functiondef` rather than retyping (0273). (4) Full per-migration history → archive doc + worklog. |
...
| Web bundle | **LIVE = `index-BuTmS3FM.js` from main tip `dd867214`** (PR #352 Guarantee & Service Package; carres-portal `f550ec7a` + carres-pos `ce25d0d8`; downloaded 4,169,655 bytes, `SERVICE_ROLE` grep 0, renamed-label + Visits-per-year + rental-rail markers ✓). That tip CONTAINS PR #351's `e657f282`, proven with `git merge-base --is-ancestor` — so this supersedes their `index-A7uOnzQ8.js` rather than clobbering it. **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after. **A canonical serving a DIFFERENT hash right after your deploy is usually the OLDER bundle still cached — `wrangler pages deployment list` names the true last writer, then `git merge-base --is-ancestor <theirs> <yours>` proves whether your tip contains their code. Contained = you superseded them, just poll until the edge flips (twice now it took ~20s on two of four); NOT contained = you clobbered them and must redeploy from the union.** A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker `ce089f7b` from main tip `dd867214`** (PR #352 — the guarantee product route carries `kind` / `visitsPerYear`; deployed AFTER 0274 was applied. Unauth 401 verified on `/api/guarantees`, `/api/rental/approvals`, `/api/catalog` via the custom domain). Same union-tip rule as web. |
...
| **HR-P6 targets + scoreboard** (2 tables not the spec's 4; scope ladder cut from 5 rungs to person\|store; manager view ships OFF with the gap named) | ✅ built 2026-07-26, awaiting deploy | 2026-07-26 | PR pending · 0276 · mock artifact `e9d96403` |
