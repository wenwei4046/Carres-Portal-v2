...
| Web bundle | **LIVE = `index-D638ZWqg.js` from main tip `5a9c5380`** (PR #319 HR-O1 Overview; union carries #318 SKU-Master + #314 guarantee + #315 Rental offers + #312 HR-P2; carres-portal `a62acde0` + carres-pos `dbda74fa`; all 4 canonicals verified — carres-pos.pages.dev lagged ~1 min behind the other three, re-curl rather than re-deploy; live bundle downloaded 4,082,410 bytes, `SERVICE_ROLE` grep 0, O1 marker ✓. A parallel session had deployed `31e3f97a` 9 minutes earlier; `git merge-base --is-ancestor` confirmed it IS an ancestor of this tip, so that deploy was superseded, not clobbered — run that check whenever the deployment list shows a deploy you did not make). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after (~15s edge lag); a piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). |
