...
| | |
|---|---|
| Active phase | **Dealer POS catalog-first order flow shipped 2026-06-16** (PR #22): `/dealer` landing is now a full-screen POS — `01 CATALOG → 02 CUSTOMER → 03 CONFIRM` (left category rail + searchable product-card grid + per-model configure drawer + cart drawer + floating flame cart CTA). **Web-only**; reuses `draft.ts` / the per-category configurators / payment-signature step / submit pipeline verbatim (`DraftLine` shape unchanged → API / zod schema / `create_order` RPC untouched). Back-office pages (Orders/Products/Settings) keep the sidebar via a `DealerChrome` layout route; dashboard KPIs moved to the Orders page top; old 4-step wizard + `DealerDashboard` deleted; online payment now forwards the bank reference/approval code (was dropped client-side). Prior: SO Maintenance 2026-06-16 (PR #21); Catalog "Product & Maintenance" 2026-06-15 (PR #19); Phase 11 2026-06-14 (PR #15). Phase 10 post-launch fixes still ongoing. |
...
| Test count | api **741/744** (3 pre-existing fails) · web **561/566** (5 pre-existing fails) · shared **206/206** — measured 2026-06-16; all 8 fails pre-existing per §17.7, zero new regressions. Dealer POS added net **+17 web tests** (+24 new: cart/catalog-index/configurators+ConfigureDrawer/CatalogStep; −7 old `ProductPicker.test`). api + shared unchanged by PR #22 (web-only; shared edit was comment-only). |
| Web bundle | 2910.41 KiB raw / 850.61 KiB gzipped (bundle `index-9Mv9HmFC.js`; CSS `index-D1GgluA-.css` 65.98 KiB; built+deployed 2026-06-16 = Dealer POS, Pages deploy `0aa6883b`; +5 KiB raw vs SO Maintenance build; bundle-size regression CF still open, see §17.5) |
