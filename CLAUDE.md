...
| Latest migration | **Applied tail = 0279_rental_signed_at_birth** (2026-07-26): the rental signature gets a WRITER. `rental_current_agreement_template()` = ONE definition of "the wording in force" (the POS read route and the sell RPC both call it, so the paper shown and the version stamped cannot drift); `create_rental_agreement` **DROP + CREATE** to take `p_signature_path`/`p_signed_name`/`p_signed_nric` and stamp `signed_at`+`template_id`+`template_version`; 0268's commented-out `signed_at IS NULL` guard turned ON in `rental_approve_agreement`; a CHECK that the signature facts are whole (backstops the blanket `is_internal()` PostgREST insert door). **New params carry DEFAULTS on purpose** — a stale Worker resolves and fails on `signature_required` (readable) instead of a schema-cache miss (the 0275 window). **Guardrail #8 fired again**: drafted as 0278 against a tail of 0277, and the pre-apply re-check found a parallel line had already applied `0278_hr_staff_comp` → renumbered to 0279 (PR #380). Prior: 0278 HR-P7 staff_comp · 0277 delivery two-stage booking · 0276 HR KPI · 0275 rental Sales Order. **Rules that survive**: (1) `list_migrations` the remote tracker tail immediately before numbering AND again before applying — parallel lines take numbers within minutes. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber APPLIED files, but an unapplied draft SHOULD be renumbered. (3) Reconcile the FILE back to live after apply — compare `md5(prosrc)` + length per function, do not eyeball it. (4) **An assertion that RAISEs inside its own `EXCEPTION WHEN sqlstate 'P0001'` handler catches itself and proves nothing** — set a flag and check it outside the handler (0279's first dry run silently un-tested four assertions this way). (5) `revoke execute … from anon` alone does NOTHING, and `from public` alone does nothing either — the pair PLUS an explicit `grant … to authenticated, service_role`, asserted both directions in the migration's own sanity block. (6) Inside a transaction, read `audit_log` rows back by `ref`, never by time. |
