...
| Web bundle | **LIVE = `index-Boz1uDzu.js` from main tip `e26fd1ae`** (PR #356 one door for care plans; carres-portal `b13303d0` + carres-pos `2fbecd1a`; downloaded 4,170,195 bytes, `SERVICE_ROLE` grep 0, new notice present AND the retired "New service package" string grep **0** — the removal is verified in the shipped bundle, not just in the diff). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after and POLL until they converge (three deploys running: one or two of four lag ~20s, `pos.carresofficial.com` most often). **A canonical serving a DIFFERENT hash is usually the OLDER bundle still cached — `wrangler pages deployment list` names the true last writer, then `git merge-base --is-ancestor <theirs> <yours>` proves whether your tip contains their code. Contained = you superseded them, keep polling; NOT contained = you clobbered them and must redeploy from the union.** A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. bundle-size CF open (§17.5). |
