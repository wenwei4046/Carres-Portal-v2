...
| Web bundle | **built + deployed 2026-07-19 from main `51f01dba` (PR #213 merge — bundle `index-DZeSAUEH.js`: POS customer-step fixes [birthday wheel opens UP `.dki-pop--up` · structured billing MY-cascade `billing*` fields, composed at submit, gates mirror delivery, autofill re-parses · Building type select → `entry_data.fields.building_type`, shown on CONFIRM + PosOrderDetail + ops drawer CustomerIdentityCard · ThankYou View-orders button REMOVED, onClose prop retired] — strict superset of the day's `index-ItuJHcJ4.js` [#208/#209/#210/#211/#212/#214/#215/#216]; deployed to BOTH Pages projects — carres-portal `89b0851a` + carres-pos `eff1d9af`, `--branch=main`; `SERVICE_ROLE` scan on dist = 0; carres-portal.pages.dev + pos.carresofficial.com live-verified cache-busted). ⚠️ Same-day parallel-deploy race is real — before ANY Pages deploy: `git fetch` + `git log HEAD..origin/main`, rebuild from the union tip. ⚠️ design-standard lint currently fails on main: PR #215's `BundleConfigurePage.tsx` carries 5 un-baselined hex literals (not from #213).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 from main `51f01dba` (Worker version `ff0994f2-d78d-4954-a824-d32a02605d3b` — GET /api/operation/orders/:id now also selects `entry_data` so the ops drawer shows the POS building type; strict superset of same-day `226ae939` [0241 staff profile fields]; GET /api/catalog → 401 live-verified).** |
