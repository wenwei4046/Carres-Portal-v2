...
| Web bundle | **built + deployed 2026-07-19 (final) from merged main `a8d3a92e`+docs (bundle `index-YoJrEACq.js` — the day's FULL superset: PR #209 staff profile/one-step PIN/English-only/centred page + #210 POS-only + #211 PJ postcodes + #212 corner-control logout + #208 bundles; deployed to BOTH Pages projects — carres-portal `7bb1c28a` + carres-pos `96dbffef`, `--branch=main`; supersedes same-hour `index-DDzoHDae.js` which lacked #211/#212); `SERVICE_ROLE` scan on dist = 0; both pages.dev canonicals + pos./erp.carresofficial.com all live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
