...
| Latest migration | **Applied tail = 0267 — THREE of them** (2026-07-26, three parallel lines minutes apart): **0267_rental_agreement_templates** (the agreement wording, ordered blocks + immutable versions + the private no-delete `rental-agreements` bucket) · **0267_guarantee_id** · **0267_auth_hook_wont_mint_a_role_for_disabled**. Prior: 0266_disabled_accounts_actually_lose_access · 0265_hr_source_exclude_imported_archive · 0264_rental_offers. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8; collisions renumber at apply. (2) Migrations apply manually via MCP; the tracker keys on TIMESTAMP, so dual/triple-numbered FILES are cosmetic — never renumber applied files. (3) Full per-migration history → archive doc + worklog. |
...
| Web bundle | **LIVE = `index-K-sQWa48.js` from main tip `5cf6c092`** (PR #329 agreement wording; that tip is the UNION — it contains the parallel lines' #326/#328, whose own deploy `index-Dygf2tsy.js` it supersedes; carres-portal `614b51bc` + carres-pos `8e81b245`; all 4 canonicals re-verified after both sessions deployed; live bundle 4,107,142 bytes, `SERVICE_ROLE` grep 0). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after (~15s edge lag; a custom domain can lag the pages.dev canonical by ~40s). **When a parallel session deploys around the same time, a canonical can briefly serve THEIR bundle — check `wrangler pages deployment list` for the true last writer and confirm your commit CONTAINS their merge before assuming a clobber.** Piping curl into grep on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). |
