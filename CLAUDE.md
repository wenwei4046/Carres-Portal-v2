...
| Test count | api **795/798** (3 pre-existing fails) · web **690/695** (5 pre-existing fails) · shared **287/287** — measured 2026-06-21 after Sofa engine Phase 2 (PR #31); all 8 fails pre-existing per §17.7 (OhanaSofaTab ×4 + NiceFutureMattressTab ×1 web; supplier/pos ×2 + pickups ×1 api), zero new regressions. Phase 2 added: shared sofa-combo schema/adapter + the pure pricing-engine battery (**53 sofa-pricing tests** — Kuhn match, residue-on-last, mirror-C1, combo-even-if-pricier, per-height selection); api sofa-combo GET + principal-gated CRUD (+11); web SofaCombosPanel (+14). |
| Web bundle | built + deployed from `main` 2026-06-21 (Sofa engine Phase 2, Pages deploy `5bfbaa9b`); `SERVICE_ROLE` scan on dist = 0. bundle-size regression CF still open, see §17.5. |
...
- `sofa-engine-roadmap-remaining-phases` (watch) — Phase 1 (compartments) + Phase 2 (pricing engine + `sofa_combo_pricing` + the pure `computeSofaPrice`/`matchSofaCombo`/`explodeSofaBuild`) are SHIPPED; the ~5-phase **2990s sofa custom-cell engine** roadmap (`docs/superpowers/plans/2026-06-21-sofa-engine-roadmap.md`) still has **P3 (visual drag plan-view builder, web-only, NO migration — port 2990s `sofa-build.ts` geometry: edge-snap/connected-detect/arm-cap/auto-canonical + live `computeSofaPrice` preview + per-compartment silhouette assets)** · P4 (Hono server-recompute + 0.5% drift-reject + explode into per-compartment `order_lines` via the existing `create_order`) · P5 (downstream regroup + cutover). Each phase = its own plan + SDD; migrations go through the §7 gate (P4 touches NO schema if config stays in `order_lines.attrs` free jsonb). Promo (PWP/GWP) + bedframe remain deferred. **Two P2 review minors to confirm byte-aligned at P4's Hono recompute**: 0-priced-combo gate (already fixed to match 2990s — kept through filter, gated `>0` post-rank) + `pickSofaCombo` `tier=null` wildcard (Carres-only superset, the real `computeSofaPrice` path always passes a concrete tier so it's unreachable there).
