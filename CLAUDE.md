...
| Latest migration | **Applied tail = 0260** (verified at apply 2026-07-26): **0260_hr_duty_keys** (HR-P2 — `org_duties` + `org_position_duties` + self-only `my_org_duties()` + list-read `org_duty_holders()` + audited `hr_set_position_duty()`; folds the missing audit into `hr_set_reports_to`). Prior: 0259_org_departments · 0258 (edit_order_addon) · 0257 (replace_lines + p_addons_append) · 0254-0256. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8; collisions renumber at apply. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) Full per-migration history → archive doc + worklog. |
...
| **HR-P2 权限跟着职位走** (duty keys retire the 3 email hardcodes; Permissions matrix in Team tab; legacy fallback for ONE release) | ✅ code done, undeployed | 2026-07-26 | 0260 · branch `feat/hr-p2-duty-keys` |
