...
| Web bundle | **built + deployed 2026-07-19 from main `942e80da` (dealer POS-only, PR #210 — the Orders/Products/Settings back-office tabs DELETED, corner control now LOCKS the register, Outlets moved into the POS Staff overlay; strict superset of same-day PR #208 bundle pricing; bundle `index-BBnNMd71.js`, deployed to BOTH Pages projects — carres-portal `9aa28f4b` + carres-pos `8153d394`, `--branch=main`); `SERVICE_ROLE` scan on dist = 0; both pages.dev canonicals + pos.carresofficial.com live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 from main `fcfaea6` (Worker version `88fa0ac1-8078-4ec8-a022-473732d31efc` — bundles in GET /api/catalog [POS active-only / admin all] + principal-gated /api/catalog/bundles CRUD; strict superset of the same-day `ca4faad8` deploy [PR #204 /api/account email-change routes], carries everything through 0240; GET /api/catalog → 401 live-verified).** |
