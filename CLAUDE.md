...
| Web bundle | **LIVE = `index-CC9gQUto.js` from main tip `ec3cafd1`** (PR #291 service add-on edit; carres-portal `06ce0d6e` + carres-pos `52865f29`; all 4 canonicals verified first curl; full bundle 4,018,678 bytes — addon-modal marker present, `SERVICE_ROLE` grep 0). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after (~15s edge lag); a piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). |
| API bundle | **LIVE = Worker `4404af26` from main tip `ec3cafd1`** (PR #291 `POST /:id/addons/:addonId/edit` + edit_addon change-request branches; unauth 401 live-verified; deployed AFTER 0258 was applied). Same union-tip rule as web. |
