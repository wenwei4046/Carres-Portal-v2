...
| Latest migration | **Applied tail = 0270** (2026-07-26): **0270_guarantee_scope** — four scope columns on `guarantee_terms` (`covers_model_id` / `covers_variants` / `covers_combo_id` / `covers_compartment_id`; NULL at any level = ANY at that level, so the pre-0270 term is untouched) + 2 CHECKs (combo/compartment are sofa-only and mutually exclusive; variants only where a size axis exists). Prior: 0269 HR employee master · 0268 rental approver · **0267 x3 — THREE files share the number** (auth-hook / **guarantee_id** / rental templates, applied minutes apart by parallel sessions; tracker keys on TIMESTAMP so it is cosmetic and none were renumbered) · 0266 · **0261-0263 guarantee packages**. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) If you flatten any character in the apply payload, reconcile the FILE back to live — file-equals-live beats a tidier file (0269). (4) Full per-migration history → archive doc + worklog. |
...
| Web bundle | **LIVE = `index-DIco4n1v.js` from main tip `09b6933a`** (PR #342 guarantee scope authoring; union carries the parallel line's #341 HR-P4; carres-portal `73806dbe` + carres-pos `8aa0d1d7`; all 4 canonicals verified after convergence; downloaded 4,155,071 bytes, `SERVICE_ROLE` grep 0). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after. **A canonical serving a DIFFERENT hash right after your deploy is usually the OLDER bundle still cached — check `wrangler pages deployment list` before reacting; the two projects lag independently, so poll each until it flips.** A piped `curl` into `grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker `3c543c05` from main tip `09b6933a`** (PR #342 `POST /api/guarantees/products`; unauth 401 live-verified). Same union-tip rule as web. |
| Catalog state | 11 suppliers · 171 product_models · ~1150 product_skus (1013 AutoCount + SVC + minted compartment skus; 123 compartment skus flipped ON 2026-07-24, mostly RM0 pending pricing). Bundles: 1 active (King Bedroom Set RM2500). PWP/free-gift/delivery-fee/rental config tables exist, dormant until authored. |
| Orders state | **55 orders** (counted live 2026-07-26 — the old "~190 / 153 AutoCount" figure predated the 2026-06-24 catalog/data reset and was stale): **18 native POS orders, ALL 18 attributed**, + **37 `source_system='autocount'` archive rows** imported 2026-07-23, all unattributed and all carrying RM 0 of line value. That split is why HR-P3 was cut and why 0265 excludes archive from the attribution worklist. `dealers.channel` is THE showroom-vs-dealer authority (PR #226). |
| Test count | Baselines + pre-existing fails → §17.7. Run full suites before merge; ZERO new failures is the bar. |
