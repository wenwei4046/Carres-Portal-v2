...
| Web bundle | **built + deployed 2026-07-18 from main `bea13ec` (New Order payment→ledger + Order-Entry follow-ups, PR #181; Pages deploy `6ad3acf3`, bundle `index-DCgH0Qz9.js`); `SERVICE_ROLE` scan on dist = 0; canonical https://carres-portal.pages.dev live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-18 from main `bea13ec` (Worker version `a66f5506-a64d-49fc-bb9c-1ec2cacf08c8` — raw door now posts paid>0 into the 0193 `order_payments` ledger [kind deposit, best-effort]; carries add-product P1+P2 [PRs #174/#179, 0231/0232 on prod]; GET /api/catalog → 401 live-verified).** |
