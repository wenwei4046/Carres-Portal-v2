...
| Web bundle | **built + deployed 2026-07-19 (evening) from main `82be9bc` (POS owner self-service, PR #204; bundle `index-CBEOyPTg.js`, deployed to BOTH Pages projects — carres-portal `c52a241e` + carres-pos `ea66ff66`, `--branch=main`); `SERVICE_ROLE` scan on dist = 0; both pages.dev canonicals AND pos./erp.carresofficial.com live-verified (cache-busted).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 (evening) from main `82be9bc` (Worker version `ca4faad8-015b-4a42-aede-61055d23040e` — NEW `/api/account` email-change routes [dealer-principal submit/cancel] + `/api/principal/accounts/email-change-requests` [list/approve/reject, approve = real auth email swap]; carries everything through 0240; GET /api/catalog → 401 + /api/account/email-change → 401 live-verified).** |
