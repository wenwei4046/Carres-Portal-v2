...
| | |
|---|---|
| Active phase | **Sales Order Maintenance shipped 2026-06-16** (PR #21): Operation "SO Maintenance" page — AutoCount-style configurable SO grid (orders flattened one-row-per-line, read-only; resizable columns + per-column filters + `Columns N/M` picker + shared column/option config); migration 0174. Prior: Catalog "Product & Maintenance" shipped 2026-06-15 (PR #19); Phase 11 shipped 2026-06-14 (PR #15). Phase 10 post-launch fixes still ongoing. |
...
| Test count | api **741/744** (3 pre-existing fails) · web **544/549** (5 pre-existing fails) · shared **206/206** — measured 2026-06-16; all 8 fails pre-existing per §17.7, zero new regressions. SO Maintenance added 31 tests (shared 13 + api 9 + web 9). |
| Web bundle | 2905.14 KiB raw / 849.73 KiB gzipped (bundle `index-DRrSyifV.js`; CSS `index-DYlywJo6.css` 64.58 KiB; built+deployed 2026-06-16 = SO Maintenance, Pages deploy `f1370c4b`; +25 KiB raw vs Catalog build; bundle-size regression CF still open, see §17.5) |
