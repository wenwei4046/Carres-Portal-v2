...
| Web bundle | **built + deployed 2026-07-19 (v3, latest) from union main `d8952ebf` (bundle `index-DZeSAUEH.js` — the day's FULL superset: PR #216 staff Edit form + the building-types work + #215 bundle V2 + #214/#212 logout + #211 postcodes + #210 POS-only + #209 staff profile/English-only + #208 bundles; deployed to BOTH Pages projects — carres-portal `8c34e952` + carres-pos `5aefa83e`, `--branch=main`; supersedes `index--8-5zdIF.js` / `index-ItuJHcJ4.js`); `SERVICE_ROLE` scan on dist = 0; pos./erp.carresofficial.com live-verified (cache-busted). ⚠️ Same-day parallel-deploy race hit repeatedly — before ANY Pages deploy: `git fetch` + `git log HEAD..origin/main`, rebuild from the union tip.** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 (v3, latest) from union main `d8952ebf` (Worker version `48640203-8eed-49b9-9b34-f8cf7c97f7f9` — staff PATCH `phone` [PR #216] + the union of the day's api work [bundles V2, /api/account, 0241 profile fields]; carries everything through BOTH 0241s; GET /api/catalog → 401 live-verified).** |
