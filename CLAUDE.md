...
| Latest migration | **Applied tail = 0271** (2026-07-26): **0271_guarantee_sofa_height_scope** — widens the variants CHECK so a SOFA guarantee can be narrowed by SEAT HEIGHT (its own Maintenance pool, `sofa_size`), orthogonal to the combo/compartment scope. Prior: **0270_guarantee_scope** (4 scope columns; NULL at any level = ANY) · 0269 HR employee master · 0268 rental approver · **0267 x3 — three files share the number** (auth-hook / **guarantee_id** / rental templates; tracker keys on TIMESTAMP so it is cosmetic and none were renumbered) · **0261-0263 guarantee packages**. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) Full per-migration history → archive doc + worklog. |
...
| Web bundle | **LIVE = `index-CYqtv_4v.js` from main tip `9dee5f29`** (PR #347 Rent-to-Own becomes a POS category + the app's first ErrorBoundary. That tip CONTAINS PR #348's `24f50a72`, verified with `git merge-base --is-ancestor` before trusting the deploy — so this supersedes #348's own `index--HiWvb7v.js` rather than clobbering it. carres-portal `5d632cf9` + carres-pos `5bfc034d`; all 4 canonicals matched on the FIRST poll; downloaded 4,153,052 bytes, `SERVICE_ROLE` grep 0, rental-rail + ErrorBoundary + Rental-term markers ✓). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after. **A canonical serving a DIFFERENT hash right after your deploy is usually the OLDER bundle still cached — check `wrangler pages deployment list` before reacting; the two projects lag independently, so poll each until it flips.** **And when a parallel line's doc row names a different bundle than the one actually serving, do NOT edit their row on vibes: `wrangler pages deployment list` names the true last writer, then `git merge-base --is-ancestor <theirs> <yours>` proves whether your tip contains their code. Contained = you superseded them; NOT contained = you clobbered them and must redeploy from the union.** A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker `f56e7391` from main tip `9dee5f29`** (union deploy — PR #347 is web-only, but the tip carried parallel lines' UNDEPLOYED guarantees / hr-people / accounts / catalog changes, incl. PR #342's `POST /api/guarantees/products`, so the Worker went out from the same tip and supersedes `3c543c05`. Unauth 401 verified on `/api/rental/pos-plans`, `/api/rental/approvals`, `/api/guarantees`, `/api/catalog` + the custom domain). Same union-tip rule as web. |
| Catalog state | 11 suppliers · 171 product_models · ~1150 product_skus (1013 AutoCount + SVC + minted compartment skus; 123 compartment skus flipped ON 2026-07-24, mostly RM0 pending pricing). Bundles: 1 active (King Bedroom Set RM2500). PWP/free-gift/delivery-fee/rental config tables exist, dormant until authored. |
| Orders state | **55 orders** (counted live 2026-07-26 — the old "~190 / 153 AutoCount" figure predated the 2026-06-24 catalog/data reset and was stale): **18 native POS orders, ALL 18 attributed**, + **37 `source_system='autocount'` archive rows** imported 2026-07-23, all unattributed and all carrying RM 0 of line value. That split is why HR-P3 was cut and why 0265 excludes archive from the attribution worklist. `dealers.channel` is THE showroom-vs-dealer authority (PR #226). |
| Test count | Baselines + pre-existing fails → §17.7. Run full suites before merge; ZERO new failures is the bar. |
