...
E2E NEW carry-forwards (added 2026-05-09 morning recon):
  ...
  • phase-7-trigger-stale-column-refs (medium, NEW BUG) — migration 0046's `enforce_partner_po_column_whitelist` trigger function still references columns DROPPED in 0052: `NEW.delivery_partner_id` (renamed to `procurement_partner_id`) and `NEW.request_for_delivery_at` (now lives on `order_supplier_threads`). For partner-role UPDATEs the function errors at runtime trying to access non-existent columns. Currently silently broken because partner-role UPDATEs are rare (most go through SECURITY DEFINER RPCs that may or may not bypass — needs verification). Discovered while attempting to un-fixme `lp-update-column-whitelist` E2E. Fix: migration 0068 to update the trigger function's blocklist to match the current column set. Production impact: any partner-role UPDATE on purchase_orders fails today.
