...
| Latest migration | **Applied tail = 0267** (verified at apply 2026-07-26): **0267_auth_hook_wont_mint_a_role_for_disabled** (the auth hook selects `and status='active'`, so a disabled account gets NO role claim and the middleware's existing check 401s it — one predicate, no new claim, no code change) · **0266_disabled_accounts_actually_lose_access** (`app_role`/`is_internal`/`is_operation`/`is_principal` now require `status='active'` + all 8 `hr_*` gates rewritten fail-closed on a NULL role — the two halves MUST ship together, see the file header). Prior: **0265_hr_source_exclude_imported_archive** (HR-O1) · 0264_rental_offers · 0261-0263 guarantee · 0260_hr_duty_keys (HR-P2) · 0259_org_departments. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8; collisions renumber at apply. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) Full per-migration history → archive doc + worklog. |
...
| Web bundle | **LIVE = `index-B4SBrkkQ.js` from main tip `924429ea`** (re-deployed at the #326 union tip; the hash is UNCHANGED because that PR touched no web code — an identical hash after a deploy means the build was already current, not that the deploy failed; carres-portal `2aec81d3` + carres-pos `84f219b2`, all 4 canonicals verified) (PR #324 Rental own tab, union with #319/#321; carres-portal `92157205` + carres-pos `8b19c4d1`; 4 canonicals verified; full bundle 4,083,949 bytes, `SERVICE_ROLE` grep 0; NOTE: right after a deploy the asset URL can briefly serve the SPA fallback index.html — a <3MB "bundle" is the fallback, retry; a custom domain can lag the pages.dev canonical by ~40s). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after (~15s edge lag); piping curl into grep on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers (this sentence deliberately avoids an escaped pipe: a `\|` inside a table cell has now been mangled by TWO separate merges). bundle-size CF still open (§17.5). |
| API bundle | **LIVE = Worker `6cb6d516` from main tip `924429ea`** (PR #326 security — 0266+0267 + the `disable_signout_failed` log; unauth /api/hr/team and /api/principal/accounts both 401 live-verified; deployed AFTER both migrations were applied). Same union-tip rule as web. |
