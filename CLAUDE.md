...
| Web bundle | **built + deployed 2026-07-19 (final) from main tip after PR #214 (bundle `index-ItuJHcJ4.js` — the day's FULL superset: PR #209 staff profile/one-step PIN/English-only/centred page + #210 POS-only + #211 PJ postcodes + #212 corner-control logout + #214 logout copy English-only + #208 bundles; deployed to BOTH Pages projects — carres-portal `9c621126` + carres-pos `cc76f1f0`, `--branch=main`; supersedes same-hour `index-YoJrEACq.js` [lacked #214] and `index-DDzoHDae.js` [lacked #211/#212]); `SERVICE_ROLE` scan on dist = 0; all 3 canonicals (both pages.dev + pos.carresofficial.com) live-verified incl. the #212 logout marker string (cache-busted). ⚠️ Same-day parallel-deploy race hit twice — before ANY Pages deploy: `git fetch` + `git log HEAD..origin/main`, rebuild from the union tip.** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 (late) from main `6ceeeb48` (Worker version `226ae939-433b-4081-871c-708ce1ec36f2` — staff POST/PATCH carry the 0241 email/birthday/gender profile fields; strict superset of same-day `88fa0ac1` [bundles] + `ca4faad8` [/api/account email-change]; carries everything through 0241; GET /api/catalog → 401 live-verified).** |
