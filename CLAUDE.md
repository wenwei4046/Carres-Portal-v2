...
| Latest migration | **Applied tail = 0278_hr_staff_comp** (2026-07-26, HR-P7): `staff_comp` — effective-dated salary, **SELECT-only RLS** so the audited DEFINER RPC is structurally the only write door (a direct `insert` as `authenticated` was measured refused 42501). **Loo's ruling made structural**: "separate, don't merge" — the shared engine exposes `fixedCost` and `commissionCost` and has **no field that sums them**, with a test that fails if one is added. `employer_burden_pct` stays a typed-in estimate; EPF/SOCSO/EIS/PCB are never computed (§6, permanent). 18/18 prod assertions passed in ONE rolled-back txn — no second pass needed, because 0276's two lessons (the `from public, anon` grant pair, and reading audit rows by `ref` not `occurred_at`) were applied up front. NOTE: the leading claim on this row had drifted to `0275` while its prose described 0277_delivery_booking_two_stage — a merge artifact, corrected here. Prior: 0277 Delivery D1: `ops_order_control` + `booking_stage`/`confirmed_date`/`confirmed_time_slot`/`customer_confirmed_at`/`customer_confirmed_by`; invariant #1 (date+slot both or not Confirmed) as a DB CHECK; provisional derives from `logistic_eta` by BEFORE trigger (every writer covered); `customer_confirmed` (0220) comment-marked DEPRECATED; 0211 activity trigger extended from live `pg_get_functiondef`. Checked the tracker tail first — parallel lines had already taken 0275_rental_makes_a_sales_order + 0276_hr_kpi_targets the same day, which is exactly why guardrail #8 exists. Prior: 0274 guarantee+service package · 0272-0273 commission runs · 0271-0269 (guarantee scope / employee master) · 0268 rental approver. **Rules that survive**: (1) before numbering ANY migration, `list_migrations` the remote tracker tail first — guardrail #8. (2) Migrations apply manually via MCP; dual-numbered FILES are cosmetic — never renumber applied files. (3) If you flatten any character in the apply payload, reconcile the FILE back to live — for a whole function body, pull it out of `pg_get_functiondef` rather than retyping (0273). (4) Full per-migration history → archive doc + worklog. (5) **`revoke execute … from anon` alone does NOTHING** — a new function is created with EXECUTE granted to PUBLIC and anon inherits it, so the pair is `from public, anon` PLUS an explicit `grant … to authenticated, service_role` or every Hono user-client call gets permission denied. Assert `has_function_privilege` for both roles in the migration's own sanity block (0276 — the first draft looked right and left anon with EXECUTE on all 5 functions). (6) **Inside a transaction, read `audit_log` rows back by `ref`, never by time** — `occurred_at` defaults to `now()` = TRANSACTION START, so every audit row written in one txn shares a timestamp and `order by occurred_at desc` returns an arbitrary one (0276 — this silently un-verified a `SELF -` marker assertion). |
...
| Web bundle | **LIVE = `index-BDR0dVav.js` from main tip `9ad44686`** (PR #376 Stock K0 one-door merge; deployed carres-portal `cc6a901e` + carres-pos `556e1400`; all 4 canonicals converged after the usual pos poll; bundle downloaded 4,210,141 bytes, `SERVICE_ROLE` grep 0, `stock-tabs` + "In & out" present AND the retired "Stock · Movements" label greps **0**). Prior: `index-B3tXaG7G.js` (PR #374 HR-P7 people cost; carres-portal `03908e0a` + carres-pos `3b7e7932`; all 4 canonicals converged after polling — three of them served the parallel line's older `index-DGezupH6.js` for a minute, and `wrangler pages deployment list` confirmed `3b7e7932` from source `c1f8b0d` was already newest, so it was edge cache and not a clobber; live bundle downloaded 4,208,930 bytes, `SERVICE_ROLE` grep 0, `hr/comp` + "People cost" + `hr/kpi` all present) (PR #358 rental Sales Order + the rental confirm-gate fix; carres-portal `31c22186` + carres-pos `f98a22fa`; 4,193,674 bytes downloaded-then-grepped, `SERVICE_ROLE` 0, live-screen markers ✓). That tip CONTAINS PR #363's `474e67bd`, proven with `git merge-base --is-ancestor`. **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; `git fetch` + `git log HEAD..origin/main` empty before ANY Pages deploy; deploy to BOTH Pages projects `--branch=main`; RE-curl all 4 canonicals and POLL until they converge (four deploys running now: one or two of four lag 20-40s). **A canonical serving a DIFFERENT hash is usually the OLDER bundle still cached — `wrangler pages deployment list` names the true last writer, then `git merge-base --is-ancestor <theirs> <yours>` proves whether your tip contains their code. Contained = superseded, keep polling; NOT contained = clobber, redeploy from the union.** **When grepping the shipped bundle for proof, pick a string from a MOUNTED component: a marker from a retired/unmounted page is correctly tree-shaken and greps 0, which reads as a failed deploy when nothing is wrong.** A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file first. bundle-size CF open (§17.5). |
| API bundle | **LIVE = Worker `dc730db8` from main tip `c1f8b0d9`** (PR #374 — `/api/hr/comp` GET + PUT + DELETE; deployed AFTER 0278 was applied. Unauth 401 live-verified on `/api/hr/comp`, `/api/hr/kpi`, `/api/hr/runs` via the custom domain). Same union-tip rule as web. |
...
| HR full-system roadmap RATIFIED: **P2 → O1 → P4 → P5 → P6 → P7 → P8 → O4** (P3 cut; D1-D5 all decided; statutory payroll excluded forever) | 📋 spec locked | 2026-07-26 | `docs/hr-system-full-spec.md` · PR #309-#311 |
