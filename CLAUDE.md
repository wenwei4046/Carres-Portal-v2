...
| Web bundle | **built + deployed 2026-07-19 (v5, latest) from union tip `22b1ca2` (bundle `index-BH4N0gbp.js` — adds PR #221 [staff birthday = the shared SO BirthdayWheelField drum picker in all 3 staff forms] + #220 bundle-UI round on top of the v4 superset [#208–#219]; deployed to BOTH Pages projects — carres-portal `cdf0f6d6` + carres-pos `e4bbc720`, `--branch=main`; `SERVICE_ROLE` scan on dist = 0; ALL 4 canonicals re-curled AFTER a 14-second parallel-deploy near-miss — `index-BH4N0gbp.js` serves everywhere). ⚠️ Same-day parallel-deploy race hit 5×: before ANY Pages deploy `git fetch` + `git log HEAD..origin/main`, rebuild from the union tip, RE-curl the canonicals afterwards (allow ~15s edge propagation). ⚠️ design-standard lint currently fails on main: PR #215's `BundleConfigurePage.tsx` carries 5 un-baselined hex literals (pre-existing, owned by the bundles workstream).** bundle-size regression CF still open, see §17.5. |
| API bundle | **Deployed 2026-07-19 (v4, latest) — Worker version `fbddf3c8-3867-48fd-ba96-f3551b9e0fca`, built from the post-#219 union tree (principal accounts initialStaff insert now writes email/birthday/gender/phone/color [PR #219]; strict superset of `acac8ec6` / `48640203`; GET /api/catalog → 401 live-verified).** |
