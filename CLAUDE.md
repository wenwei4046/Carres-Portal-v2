...
| Web bundle | **LIVE = `index-CC1XlVpH.js` from main tip `31e3f97a`** (PR #318 SKU-Master SIZE column dropped for sizeless categories; union carries #314 guarantee packages + #312 HR-P2 + #315 Rental offers; carres-portal `816fadd0` + carres-pos `29de7199`; all 4 canonicals verified; live bundle downloaded 4,079,060 bytes, `SERVICE_ROLE` grep 0). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after (~15s edge lag); a piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). |
| API bundle | **LIVE = Worker `3da6c7bb` from main tip `bd27e1ad`** (PR #314 `/api/guarantees/*`; unauth `/api/guarantees/terms` + `/api/guarantees` both 401 live-verified on api.carresofficial.com AND the workers.dev host; union carries #312/#315). Same union-tip rule as web. |
...
| **HR-O1 Overview** (HR lands on "what needs me today"; imported archive stops counting as a worklist item) | ✅ code done, undeployed | 2026-07-26 | 0265 · branch `feat/hr-o1-overview` |
