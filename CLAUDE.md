...
| Test count | api **775/778** (3 pre-existing fails) · web **667/672** (5 pre-existing fails) · shared **226/226** — measured 2026-06-20 after Phase 4 combo pricing (PR #27); all 8 fails pre-existing per §17.7 (OhanaSofaTab ×4 + NiceFutureMattressTab ×1 web; supplier/pos ×2 + pickups ×1 api), zero new regressions. Phase 4 added combo tests (shared `explodeCombo` price-split + residue-on-last + non-finite-price guard; catalog API combo GET + principal-gated CRUD branches; P&M combo editor non-principal/reactivate/markup branches). |
| Web bundle | ~2956 KiB raw / ~861 KiB gzipped; built + deployed from `main` 2026-06-20 (Phase 4 combo pricing, Pages deploy `5cf619ef`). bundle-size regression CF still open, see §17.5. |
| API bundle | 1352.69 KiB raw / 254.94 KiB gzipped. **Redeployed 2026-06-20 from `main` (Worker `9412d21b`, PR #27 — catalog GET returns combos + principal-gated combo CRUD; backward-compatible).** |
