...
Phase 5 NEW carry-forwards (added 2026-05-09 Chunk B):
  ...
  • phase-5-ardrawer-download-button (low) — FinanceInvoices is the only PDF entry point today; ARDrawer's proto shows a Download button that V1 doesn't render. Add post-issue: Issue button creates invoice → drawer surfaces Download button using the new invoice id.

Phase 6 NEW carry-forwards (added 2026-05-09 supplier autonomous run):
  • phase-6-supplier-me-endpoint (low) — SupplierDashboard + SupplierPOs need `suppliers.kind` to gate own_logistics-vs-factory_pickup action buttons. V1 workaround: shows both Acknowledge + Mark-in-production on pending state and lets the RPC's wrong_supplier_kind 422 error surface in toast. Add `/api/supplier/me` returning the supplier row (kind / cat_covered / lead_time / contact_email) — small (~10 LOC) — closes the Coverage callout proto:184-195 too.
  ...
  • phase-6-supplier-recent-activity (low) — proto Dashboard shows last 6 po_history entries. V1 omits — needs `/api/supplier/activity` endpoint reading po_history filtered by supplier_id.
