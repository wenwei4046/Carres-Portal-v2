...
E2E NEW carry-forwards (added 2026-05-09 morning recon):
  ...
  • phase-7-e2e-missing-ui-topup-approve (HIGH — production bug, not test-only) — `phase-5-dealer-topup-approve` spec rewritten to API-only smoke (commit `d40a733`, GREEN). The UI gap is a real prod bug: `ApprovalDrawer` (used by Finance + Principal for all approvals) calls `useDecideApproval` → `/api/principal/approvals/:id/decide` → `approval_decide` RPC. `approval_decide` handles `refund` + `new_dealer` + `price_change` but does NOT handle `top_up` — Finance clicking Approve sets `approvals.status='approved'` without inserting payments row or bumping `dealers.deposit_balance`. Phase 5 acceptance A1 silently fails in production. Fix: ApprovalDrawer detects `kind='top_up'` and routes to `useTopupApprove` (which posts to `/api/finance/payments/topup-approve` → `finance_topup_approve` wrap RPC). Need method (cash/bank_transfer/cheque/card) + reference inputs in the action footer. ~1-2h.
