...
| Web bundle | **LIVE = `index-CPq_A7sa.js` from main tip `c4c50479`** (PR #345 guarantee size chips canonical; carres-portal `e06f214f` + carres-pos `c7d1bae1`; all 4 canonicals verified after convergence; downloaded 4,155,658 bytes, `SERVICE_ROLE` grep 0). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after. **A canonical serving a DIFFERENT hash right after your deploy is usually the OLDER bundle still cached — check `wrangler pages deployment list` before reacting; the two projects lag independently, so poll each until it flips.** A piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping. bundle-size CF open (§17.5). |
