...
E2E NEW carry-forwards (added 2026-05-09 morning recon):
  ...
  • phase-7-e2e-stale-column-references (medium) — 3 fixme'd specs reference `purchase_orders.delivery_partner_id` which was DROPPED in migration 0052 + replaced with `procurement_partner_id` (procurement leg) + `order_supplier_threads.delivery_partner_id` (customer leg). Affects: `lp-creation-and-login` cross-tenant test (RLS premise stale), `lp-update-column-whitelist` (UPDATE assertion uses dropped column), `per-leg-lp-split` (test premise OK but setup may fail). Need spec rewrites against new schema.
  • phase-7-e2e-half-stubbed-specs (medium) — `mattress-full-happy`, `bed-frame-full-happy`, `sofa-accept-happy`, `sofa-reject-relocate` all have several test steps written as comment-only stubs (e.g. "// 1-3. Dealer order → Logistics confirm → ..."). Need the actual test bodies written, not just locator fixes. ~1-2h per spec, multi-context (3+ browser contexts each).
  • phase-7-e2e-missing-ui-topup-approve (medium) — `phase-5-dealer-topup-approve` blocked because dealer-side top-up flow only exists at per-order level (DealerOrderDetail.tsx "+ Top up to 50%") AND finance-side topup-approve has the API endpoint + queries hook but no UI button wired up. Spec's premise (top-up via /me + finance-page approve panel) doesn't exist. Either build the UI or rewrite spec to use the per-order top-up flow.
