...
E2E NEW carry-forwards (added 2026-05-09 morning recon):
  ...
  • phase-7-e2e-spec-rewrite (medium, NEW) — phase-5-invoice-issue-after-delivered + phase-5-ar-aging-buckets both still fixme'd despite fixtures landing. Both have UI-introspection-heavy assertions (speculative aria-labels, wrong row matchers, inexact button names) that don't match current shipped UI. Need to read the AgingCard + FinanceAR + FinanceInvoices DOM and rewrite assertions against real testids. ~1-2h per spec.
