...
| Web bundle | **LIVE = `index-B4SBrkkQ.js` from main tip `93a64a7a`** (PR #324 Rental own tab, union with #319/#321; carres-portal `92157205` + carres-pos `8b19c4d1`; 4 canonicals verified; full bundle 4,083,949 bytes, `SERVICE_ROLE` grep 0; NOTE: right after a deploy the asset URL can briefly serve the SPA fallback index.html — a <3MB "bundle" is the fallback, retry; a custom domain can lag the pages.dev canonical by ~40s). **Deploy rules (Loo, permanent)**: NEVER deploy prod from a feature branch — merge to origin/main, deploy from the main tip; before ANY Pages deploy `git fetch` + `git log HEAD..origin/main` must be empty (union tip), deploy to BOTH Pages projects `--branch=main`, RE-curl all 4 canonicals after (~15s edge lag); a piped `curl \| grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). | grep` on the ~4 MB bundle truncates and reports a FALSE 0 — download to a file before grepping markers. bundle-size CF still open (§17.5). |
...
- `disabled-user-service-role-routes` — 0266 closed `disabled` at the DB layer, but `adminClient` (service_role) routes bypass RLS and their Hono gates read the JWT, not `app_users`. A disabled user with an unexpired token can still reach them (~1h window). Fix = status claim in the auth hook + a middleware check; NOT a per-request DB read.
