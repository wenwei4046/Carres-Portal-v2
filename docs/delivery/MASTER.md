...
- **The P0 is FIXED (0575).** Measured 2026-09-23 on `855c305c4`: the tail was `FNV(order id) mod
  ...
  Evidence, EXECUTED 2026-09-24 and not merely written:
  `apps/api/src/test/delivery-order-number-0575.integration.test.ts` (9 cases) is
  `describe.skipIf(!URL || !LOCAL)`, so until that day it had never actually run — a suite gated on
  an environment variable reports as PASSING in the one place anybody looks. It was run twice
  against kept full-chain replays: stopped at 0574 the headline case returned
  `{ ok: true, value: 'DO-230926-4242' }` (the second order WAS allowed to wear the first order's
  number), and with 0575 applied all nine pass. Also `delivery-order-issue.test.ts` fails against
  the unfixed module and passes after; `delivery-order-issue.test.ts` fails against the unfixed module
...
Carres staff do. The Work Customer card (`../workspace/MASTER.md` §5.9) reads it; the card and its
