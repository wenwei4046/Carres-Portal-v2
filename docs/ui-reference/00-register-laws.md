...
7. Detail opens in a FIXED 480px side pane (owner ruling A, RG-2 2026-08-09; below a 1200px viewport it clamps to max(400px, 40% of the register area)); the grid reflows — every column kept, shrinking, ellipsizing, horizontal scroll takes the rest. ↑↓ navigates, Esc closes, `?order=<id>` is the URL.
...
- Fixed 480px pane, not resizable (owner ruling A, RG-2 2026-08-09; revisit only if operators complain).

13. ONE Register Engine for every Carres register. It lives at
    `apps/web/src/components/register/DataGrid.tsx` — 2990's shipped
    DataGrid, COPIED 2026-08-09 (RG-2), adapted only in import paths,
    tokens and storage-key namespace. A module may configure:
    columns · filters · exports · detail panel.
    ...
    (extensions so far: the selection bar "N selected · Clear ·
    Export Excel (N)" with selection-scoped export; search moved to
    the toolbar's left per Law 2).
