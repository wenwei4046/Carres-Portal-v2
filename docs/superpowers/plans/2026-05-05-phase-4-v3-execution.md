...
| 0031 | `0031_rls_policies_v3.sql` | RLS policies for `partner` role: partner_sees_own_po (per spec §3.4 RLS template), partner_sees_own_po_lines. **partner_sees_own_warehouses DEFERRED to v3-S7 / Phase 4.5** — warehouses.owning_partner_id is null on all existing rows; the policy would be vacuous until partner-WH inventory exists, which lands with partner role full enablement. Wrapped per CLAUDE.md §8.2 Fix 2; uses STABLE helpers per Fix 3. Note: helpers are in `public.*` schema (NOT `auth.*` as spec template said) — the spec's `auth.app_role()` reference is incorrect; production migrations use `public.app_role()` per `0002_rls.sql:18-40`. |
