...
| The wizard refuses a missing production start | **`Proceed date — pick the day production should start`** | …`or tick 'Confirm later'` · `Proceed date is required. Choose the day production should start.` — a second spelling that already exists at `packages/shared/src/schemas/orders.ts:326` and is **not** governed; the office door (0391) uses the ruled words above, and the POS schema is corrected whenever it is next touched |
