...
| The rail row for every arrangement still owing physical work (the menu default) | **`Not finished`** — Inbound: correct goods are still owed (`Pending Delivery Qty` above zero) or records incomplete; Outbound: not fully loaded. **Damaged and wrong goods keep an arrangement unfinished** — the supplier still owes a replacement, so FULLY ARRIVED IS NOT FULLY FULFILLED | Open · Pending · To do · Unfinished |
...
| Inbound's three arrival filters (receiving-workspace card 2026-09-15) | **`Not finished`** (the menu default) · **`Received`** · **`All arrivals`**. `Expected`, `Part received` and `With issue` are RETIRED as filters — they overlapped each other and every other word, and they are facts on the row | Awaiting receipt · Fully received · Open · any fourth or fifth slice |
