...
| Inventory's place column (owner ruling 2026-09-25) | the existing heading **`Stock Location`** with the existing place words while the Unit stands in a Site: **`Carres Klang`** · **`PJ Showroom`** · `AL Sungai Buloh` · `HOUZS Balakong`; once the Warehouse posted OUT and no Site posted IN: **`{Logistics} · {Delivery Location}`** (`NETS Logistics · PJ Showroom`). No holder column exists: Carres owns the goods | `Who has it` · `Site` (as a column head) · `Holder` · `Where` · `With NETS Delivery` (Delivery's Items-panel word, not Inventory's) · `In transit` · `On the way to {destination}` · `NETS` as a warehouse |
| Inventory `Stock use` words (owner ruling 2026-09-25) | **`Available` · `Reserved` · `Incoming`**, and for a Unit that cannot be promised the ACTUAL reason: **`Damaged` · `Wrong item` · `Waiting inspection` · `In repair` · `Customer return · check required`** | `Reserved / sold` (a sold Unit is delivered, in History) · `Not available` (says something is wrong without saying what) · Hold · Quarantine · Blocked |
