...
| Inventory's place column (owner ruling 2026-09-25) | the existing heading **`Stock Location`** with the existing place words: **`Carres Klang`** · **`With NETS Delivery`** · **`PJ Showroom`** · `AL Sungai Buloh` · `HOUZS Balakong`. No holder column exists: Carres owns the goods, and the place already says who is responsible | `Who has it` · `Site` (as a column head) · `Holder` · `Where` · `On the way to {destination}` (that is Delivery's Journey word) · `NETS` as a warehouse |
