...
| The record's heads | `PO No` · `Unit ID` · `SKU` · `Item` · `Qty` · `Supplier Deliver To` · `Supplier` · `PO Status` · `PO Default Delivery Date`. **`PO No` first and `Unit ID` beside it** — the two identifiers a person copies. Both print in FULL: `PO-20260904-4665`, **never** `PO-260904-4665`. **Absent on purpose:** `Ready Stock` · `To buy` · `Category` · any tick — a column of dashes states nothing. |
...
**MANUAL PURCHASE goods and shelf — ALIGNMENT PROPOSAL / NOT LAW (2026-09-18).** The parent column
order is approved (Purchasing §9.2); these two expansion tables are this round's alignment proposal
and are not yet approved.

| Where | The words |
|---|---|
| Manual Purchase goods heads | `Category` · `Qty` · `Item` · `Ready Stock` · `Supplier` · `Supplier Deliver To` · `PO No` · `PO Default Delivery Date`. **No selection checkbox** (the request is ticked on its parent row) and **no generic `Status` column** (`Approval Status`, the three groups and `PO No` already answer it). `SKU` leaves the table and prints as the 11px second line inside `Item` beside the configuration; it stays searchable. The retired 2026-08-15 `Item`-last ordering is deleted, not kept beside this one. |
| A Manual Purchase goods row with no document | `Not ordered yet` under `PO No` — the remainder row, from the one governed remainder arithmetic. One row is one allocation: a line split across two POs prints each document's own quantity, never the whole request quantity twice. |
| Manual Purchase `Ready Stock` cell | `{n} available` with its own borderless disclosure arrow — **no `{n} reserved` line**, because a Manual Purchase reserves nothing. `0` with no arrow only for a successful empty read. Loading, failed or unknown never renders as `0`. Free stock never reduces what the request asks for. |
| Manual Purchase stock table heads | `Goods Received Date` · `Stock Location` · `Supplier` · `PO No / Ref No` with `Unit ID` on line two · `Condition` · `Owner`. Read-only: no checkbox, no `Choose Ready Unit`, no `Save changes`, no `Cancel`. |
| `Owner` on any shelf table | `Carres` · `Supplier`. It is a DIFFERENT fact from the `Supplier` column, which states provenance — who Carres bought from. A consignment Unit is owned by a supplier and still has a Carres purchase behind it, so one column cannot carry both. 🔴 The approved SO Batch picker requires supplier-owned stock to stay distinguishable but names no column for it; this proposal is that both tables carry `Owner`. |
| A quantity-managed row on any shelf table | Its key with `Counted stock` on line two in place of a Unit ID — never drawn as a choosable Unit, never hidden (hiding bulk makes a full shelf read as an empty one). |
...
| `PO Default Delivery Date` | The ORIGINAL supplier-facing delivery date stamped on the PO when it is created, and never changed afterwards. A date, not a supplier's number of lead days. Preserved when the supplier replies or Settings later change. Replaces PO Delivery Date. **Its SOURCE differs by the door that issued the PO, and the label must not claim one of them** (measured 2026-09-18): from SO Batch Purchase the date is derived from governed production and transit settings (`to-order.ts`); from Manual Purchase it is the request's own approved Delivery Date, chosen by staff and approved with the request (`manual-purchase.ts:2074` writes `eta_date: group.deliveryDate`, and migration 0432's trigger copies it to `official_delivery_date`). One column name across the pages is correct — it is one fact, the original date on the paper the supplier holds. **Never rename it `Supplier Default Delivery Date`**: on a Manual Purchase PO that would describe a staff-chosen date as the supplier's default. |
