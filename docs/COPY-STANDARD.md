...
| 🔴 Retired words live on the Receiving Register — measured 2026-09-18 | `OperationReceiving.tsx` still prints `Supplier Delivery Date` (234), `Goods received on` (263), `Deliver To` (335), `Product` (308) and a `Status` column (386). The governed replacements are `Supplier Confirmed Delivery Date`, `Goods Received Date`, `Supplier Deliver To`, `Items`, and no status label on a normal GRN with `Cancelled` under its `GRN No`. All five are dictionary repairs, not new copy. |
| Receiving keeps `Clear filters` | The removal of the rail's `Clear filters` button is a **Purchase Orders** correction and applies there only. On Receiving, `Clear filters` restores the complete listing and stays. |
