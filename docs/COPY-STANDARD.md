...
| Supplier Claims `Claim status` — owner confirmation 2026-09-18 | `In progress` · `Closed` · `Cancelled`. **`Open` is retired on this surface** and may not return; the stored database value is not renamed to follow the display word (§0's label-versus-identifier rule). Measured 2026-09-18: `OperationSupplierClaims.tsx:25` still maps `open` to the retired word `Open` — a source read, not a production walk. |
| Supplier Claims row expansion — owner confirmation 2026-09-18 | **One row per held Unit**, with `PO No + Unit ID` in ONE cell: the document number on the first line, the Unit ID beneath it — the same identity pairing the sibling Purchasing pages use. Beside it, the compact photo/video viewer. Read-only: `Open Claim` is the one door out, no editor, and selection is for export only. The earlier per-claim problem/evidence inspector wording is retired. |
| The compact evidence viewer | Its words already exist and are not re-invented: `Photo {n} of {total}` · `Previous photo` · `Next photo` · `Close` · `← → change photo · Esc closes` · `Photo {n} of {total} could not be loaded` · `Try again`. It states its count, tells a video from a photo, and names a file it could not read rather than showing one fewer. Read-only — uploading stays a separate component. |
| 🔴 THREE CLAIM VOCABULARIES, AND `Replace` IS IN TWO OF THEM — measured 2026-09-18 | A supplier claim carries three separate answers, each with its own closed list in `packages/shared/src/supplier-claim.ts`. **`Requested Result`** (what Carres asked): `Replace` · `Deliver missing parts` · `Deliver correct item` · `Repair` · `Return for inspection` · `Deliver remaining`. **`Supplier Response`** (what the supplier answered): `Replacement` · `Deliver remaining` · `Repair` · `Return & replace` · `Reject` · `Other agreement`, absent reading `Not recorded`. **`Customer Resolution`** (what the customer got): `Replace` · `Repair` · `Accept As-Is` · `No Replacement Required`. **`Replace` is NEVER a Supplier Response** — that list's word is `Replacement`. Because two of the three lists do contain `Replace`, the wrong one reads as a perfectly ordinary supplier answer, which is exactly why the three lists exist. A 2026-09-18 planning mockup printed `Replace` under Supplier Response; recorded here so the next surface does not repeat it. |
