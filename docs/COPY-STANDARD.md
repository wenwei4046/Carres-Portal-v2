...
| A Unit cell with no Unit | **FIVE answers, never one.** `Loading…` in flight · `Could not be loaded` on failure · **`Not read`** when the read answered for the ORDER but carried no entry for this item line · **`Not unit-tracked`** when the goods are counted rather than individually tracked (0453 — the technical `QTY-` key never reaches a `Unit ID` heading) · `Not allocated` ONLY when the read answered for this line and nothing is tied to it. **Printing any of the first four as the last tells an operator goods do not exist because a request was slow.** |
...
| `To buy` when it is not a remainder | **`Already on a PO`** under the figure, with the governed `Demand is already covered by an open Purchase Order.` as its title. The engine prints the covering document's quantity under `To buy` on a build every unit of which is already on an OPEN purchase order, so one figure means two opposite things; this says which. **Never `Open PO …`** — a retired column head. |
