...
| Purchase Orders expansion heads — ALIGNMENT PROPOSAL / NOT LAW (2026-09-18) | `Qty` · `Item` · `Supplier Deliver To`, read-only. `SKU` leaves the table and prints as the 11px second line inside `Item` beside the configuration; it stays searchable. **Absent on purpose:** `Category` (one purchase order is one category by its partition key, so the column repeats one value), `Status`, any checkbox, `Ready Stock` and `Unit ID` (a line of `Qty 3` carries three Units; they stay on the PO object and the official PDF). |
...
| The Purchase Orders footer | `{n} Purchase Orders` · `{n} of {m} Purchase Orders` · `1 Purchase Order` — Title Case, like `27 Sales Orders` and `{n} Manual Purchases`. The built lower-case `{n} purchase orders` is corrected (2026-09-18); no quantity totals. |
...
| Purchase Orders group band — PROPOSAL / NOT LAW (2026-09-18) | `Sending not confirmed` in place of the imperative `Confirm PO sent to supplier`, so all four bands name a state. **Action ≠ Status** is this file's own law, and no Register cell may carry an action sentence — a band is that statement one level up. The imperative stays where actions live: My Work, Team Work and the PO detail work card. |
