...
| Selected Issue action (corrected 2026-09-21 — every count carries its noun) | `1 Sales Order · 1 item line · Qty 1 · Issue 1 PO  [Clear]  [YJ]  [Issue PO]          [Export Excel (1)]` — Sales Orders are the ticked records, item lines the order lines their demand covers (a set counts every line), `Qty` the quantity. Never `units`: a Unit is an exact physical Unit ID, which an unissued purchase line never has. |
...
| The selection bar — replaces the top toolbar, no action bar below the table | `{n} Manual Purchase(s) · {l} item line(s) · Qty {q} · Issue {p} PO(s)` (corrected 2026-09-21 — the same sentence SO Batch prints; item lines are the selected lines with quantity left to buy) beside the resolved PO Duty person and `Issue PO` — PO Duty renders NOWHERE without a selection; `Select at most 20 requests for one issue.` |
...
| `Page not found.` · `This address does not open a page.` · `Go to Dashboard` | an Operation address whose `?tab=` names no page (e.g. `/operation?tab=purchase-orders`); it used to render an empty pane | **REGISTERED 2026-09-21** (Purchasing defect fix — the owner may re-word) |
