...
| The middle list row (Jess, 2026-09-26) | line 1 `{document}` + `{fmtDate}` / **`Was due {fmtDate}`** / `No date`; line 2 the action sentence; a covered row **`For {normal owner}`**; the line over the list = the chosen Date word alone | `Late —` · a dash anywhere · `Covered for` / `Covered by` on a row · a count in the heading · the problem sentence on the row |
