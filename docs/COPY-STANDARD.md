...
| `Confirmed` · `Cancelled` | GRN document status — `GRN Status` column, object pill, GRN paper | **Owner instruction 2026-09-13 (supersedes the 2026-09-06 `Valid`):** the document labels, mapped onto the EXISTING internal states `posted` / `voided` — no new workflow state. `Posted`, `Voided` and `Valid` never reach a normal user's Receiving screen. A cancelled GRN keeps its number and prints `CANCELLED` on the document. |
...
**OWNER INSTRUCTION 2026-09-13 — the GRN Register redesign words.** Registered with the build on
`build/receiving-grn-redesign` (owner review pending; rows marked *pending acceptance* are the
13 September review's refinements, shown in the build and NOT owner-approved):

| Word | Where | Why this word |
|---|---|---|
| `GRN No` · `GRN Date` · `Supplier DO No` · `Supplier` · `PO No` · `Items` · `Received Qty` · `Exceptions` · `GRN Status` | the Register's DEFAULT column heads, exactly and in this order (after `▸`) | The owner's table. `GRN Date` is the posting's date in MYT — the document's birth, never parsed from the number; `Goods received on` stays the optional physical-arrival column. `Received Qty` is the good count only. |
| `CO` | a marker beside the number in `PO No`, and `(CO)` in export | A consignment source is IDENTIFIED, never silently relabelled a PO. |
| `Item` · `Received` · `Damaged` · `Wrong Item` · `Extra` | the expansion's five heads (shared child table, receiving layout) | This GRN's own lines only. `Item` is the goods' FULL name with its configuration; no SKU column, no SKU under a resolved name, no `Receiving Result` heading, never the variant alone. |
| `name from the current catalog` · `no catalog name` | the note under an `Item` whose name is not the receipt's own snapshot | The fallback is SAID: a name resolved from today's catalog is not the name the paper carried, and a bare SKU is not a name. |
| `No exceptions` | the `Exceptions` cell of a clean GRN | The governed absence — muted, never a `0`, and no evidence door. |
| `{n} damaged · {n} wrong item · {n} extra` | the `Exceptions` summary | One arithmetic (`grnExceptionSummary`) over the receipt's positive exceptions. |
| `Photos {n}` · `Videos {n}` · `Photos · Not verified` | the evidence doors, per exception, in the expanded cell and beside the summary | `{n}` is the evidence store's VERIFIED count of recorded files; when the store did not answer the door says `Not verified` rather than a reassuring `0`. A zero exception draws no door. |
| `Damaged Photos · {GRN}` · `Wrong Item Videos · {GRN}` … | the viewer's title | GRN + exception + kind, so the operator can never mistake whose files these are. |
| `For: {item} ({n} damaged)` | the viewer's line list | A summary spanning two lines opens a viewer that NAMES both. |
| `Opening the evidence…` · `No photos on file for this exception.` · `No videos on file for this exception.` · `The evidence records could not be verified — the evidence store did not answer.` · `You do not have permission to open this evidence.` · `The evidence could not be opened` + `Try again` | the viewer's five states | Loading · verified none · records not verified · permission denied · load failure — each in words, never one blank for all five. |
| `Recorded file is not in storage` · `File could not be opened — not verified` | under a file the viewer cannot show | A recorded fact is NAMED, never hidden; the two absences are different facts. |
| `Open larger` · `Previous` · `Next` · `Back to all files` | the photo viewer's controls | Enlarge and step through; the arrow keys do the same. |
| `EXPECTED ARRIVALS` | the rail block heading over the two-month display | It is a display of supplier-confirmed dates, not a filter and not work. |
| `{iso} — {n} expected supplier arrival(s)` | a marked day's aria sentence | The count in words — colour is never the only signal. |
| `… , overdue` · `No supplier arrivals expected in {Month YYYY}` | the overdue word in a day's aria sentence (red ink on the count) · the sentence under an empty month | **Pending acceptance** (13 Sep review refinements): both are shown in the build for the owner to accept or strike. |
| `{n} GRNs` · `showing {from}–{to}` · `{n} of {all} match the filters` | the information-only footer | The complete matching count, the loaded window, and narrowed-versus-total; no control ever enters the footer. |
| `{from}–{to} of {total}` · `Previous` · `Next` | the pager in the top control area, only when more than one page exists | |
| `This receipt` · `Current PO balance` · `Related receipts` · `Exception follow-up` · `Inventory Result` · `Evidence and audit history` | the GRN object's six sections, in order | Six questions, six answers, never one taken for another. `What this saving did` is retired. |
| `As of now, across every receipt on this source — not the balance at the time of this receipt.` | under `Current PO balance` | The balance is CURRENT and says so. |
| `Ordered` · `Received (all receipts)` · `Outstanding` | the current-balance figures | |
| `No other receiving on this source.` · `No Claim is linked to this receiving.` · `No exceptions recorded on this receipt.` · `A closed source balance does not close a Claim — the Claim's own state rules.` | `Related receipts` / `Exception follow-up` absences and the rule | Outstanding 0 rules nothing about a Claim; the Claim's own number and state are printed, linked to the exact Claim. |
| `Unit outcomes were not recorded on this receiving — it was posted before Unit tracking.` · `{n} received piece(s) posted as counted stock at {site}.` · `This receiving is cancelled — its stock consequences were reversed.` · `Current register state not available` · `Nothing entered Inventory from this receiving.` | `Inventory Result` | Facts read from the register or stated as unknown — availability is never inferred. |
| `Exception evidence` · `Add exception evidence` · `Exception evidence added` | the record's evidence row · the Amend form's append block · the History line for an evidence append | Append-only; the history says what was added. |
| `Damage video (optional) — {sku}` · `Wrong-item video (optional) — {sku}` · `Extra goods photo or video (optional) — {sku}` | the Session's evidence pickers | Videos never replace the required photo; extra goods may carry both. |
...
| The owner on the action | the avatar chip; accessible name = the acting person; hover **`Normal owner: {name} · Today's cover: {name}`** when covered; **`Nobody holds Delivery Duty.`** + **`Staff & Duties`** when no collection owner could be established (owner ruling 2026-09-13) | a name inside the sentence |
...
| | **`Includes {money} from customers who paid before their invoice.`** | Second line under `2210 Customer deposits held`. A customer whose receivable balance is below zero paid before the invoice; the Balance Sheet shows that money here and leaves it out of Receivables (migration 0506, report only — the Trial Balance and the Journal still show it on 1210). The line label is the account's own name. **PENDING APPROVAL.** | A finance user opens 2210 in the Journal, finds no entries, and cannot tell where the figure came from. |
| | **`Leaves out {money} that customers paid before their invoice.`** | Second line under `1210 Trade receivables` on the Balance Sheet, the other side of the note under 2210: the amount 0506 moved off receivables, so the line still reconciles to the Trial Balance and the Journal. **PENDING APPROVAL.** | A finance user compares Receivables with 1210 on the Trial Balance, sees two different numbers, and cannot tell why. |
...
| `Overdue (>30d)` | Dashboard tile, and the AR register's removable condition, for what customers still owe on orders placed more than 30 days ago (Malaysia days, the same age the old `finance_ar_aging` used — not a due date). Kept in the pre-#1248 spelling by the owner's 2026-09-14 layout ruling; it breaks "No abbreviations" and is flagged for a ruling. | The tile differs from the AR register's footer at `?age=over-30`, or an order placed exactly 30 days ago is counted. |
| `A/R Aging · Outstanding by age` · `{bucket} days` (`0-30 days` · `31-60 days` · `61-90 days` · `90+ days`) · `{RM} · {n} orders` | The Dashboard's aging card: what customers owe, split by how many days ago the order was placed; each row opens AR · Receivables narrowed to that age. Pre-#1248 heading kept by the owner ruling; `A/R` flagged as above. | A bucket differs from the AR register's footer at that `?age=`, or the four buckets do not add up to Outstanding. |
| `Age` · `{n} days` · `1 day` · `Order date not available` | The AR register's Age column: days since the order was placed; an order without a placing date says so. | An order with no placing date shows 0 days or lands in 0-30. |
