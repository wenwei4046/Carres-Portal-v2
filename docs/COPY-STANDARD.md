...
| Inventory's can-it-be-sold column (owner ruling 2026-09-25) | **`Inventory Status`** with exactly **`Available` · `Reserved` · `Cannot sell`**; the reason for `Cannot sell` lives in the **`Stock Condition`** column (`New` · `Damaged` · `Wrong item` · `In repair` · `Waiting inspection`) | `Stock use` · `Not available` · `Blocked` · `Reserved / sold` · a condition word inside the status |
| Inventory's three after-OUT columns (owner ruling 2026-09-25) | **`Ship Date`** (the day the goods left the warehouse with the Logistics company — the pair to `Goods Received Date`; owner chose the international word over SAP's `Goods Issue Date`) · **`Logistics`** (the company whose driver took the goods) · **`Delivery Location`** (where it is going); all three blank while the Unit stands in a Site | Handed over (as a column head) · Goods Out Date · Pickup Date · Dispatch Date · With NETS Delivery · In transit · On the way · a company inside `Stock Location` |
