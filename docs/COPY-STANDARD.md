...
| Purchasing PO window words (owner ruling 2026-09-25; Purchasing MASTER §5.6.1) | fact `{time} PO window · {n} suppliers · {n} Sales Orders` · `{time} supplier cut-off · {Supplier} · {n} Sales Orders` · `{time} PO window · {n} of {m} POs · Sending not confirmed` · `Window passed · {time}` · action `Issue POs to {suppliers}` · `Click WhatsApp, send {PO No} to {Supplier}` · `Click Email, send {PO No} to {Supplier}` · `Send {PO No} to {Supplier}` (no channel recorded) · permission `Only PO Duty can issue POs` · Settings `PO Days` · `First PO window` · `Second PO window` | `Not marked as sent` · `then press PO sent to supplier` on the card · one card per Sales Order · `Supplier date missing` · treating an opened WhatsApp as sent |
