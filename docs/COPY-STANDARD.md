...
| | **`Leaves out {money} that customers paid before their invoice.`** | About `1210 Trade receivables`. Shown in the tooltip of the info mark next to the account on Reports → Balance Sheet, and as its own row in the month-end pack. The other side of the note on 2210: the amount 0506 moved off receivables, so the line still reconciles to the Trial Balance and the Journal. **PENDING APPROVAL.** | A finance user compares Receivables with 1210 on the Trial Balance, sees two different numbers, and cannot tell why. |
