...
| Inventory `Who has it` (owner rulings 2026-09-25) | **the organisation responsible for the goods, its own name only**: `Carres` inside any Carres-owned Site (Klang, PJ Showroom) · `NETS` once the driver has taken the goods · `AL` / `HOUZS` inside a partner Site. NETS operating Klang is a Settings fact (`Operated by`), never a holder value | `NETS Warehouse` · `NETS Delivery` · `NETS (operator)` · `PJ Showroom` as a holder · any role word glued to a party name |
