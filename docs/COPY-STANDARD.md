...
| Manual Purchase Request name and prefixes — latest owner correction 2026-09-23; rename TARGET / NOT BUILT | Document and screen name: `Manual Purchase Request`; `MPR` stands for Manual Purchase Request. Prefixes remain Outright `MPR` and Subscription `SMPR` (Subscription target not built). Replace the future screen-name target `Manual Purchase`. Standalone `Purchase Request` and `PR` / `SPR` or `REQ` / `SREQ` request prefixes remain withdrawn. `MP` remains Mattress Protector; `PR` remains Purchase Return. Full approved formats: `MPRYYMMDD-NNNN` / `SMPRYYMMDD-NNNN`, original document date plus four fixed random digits including leading zeros, independent daily 10,000-reference pools. No application implementation or live cutover is authorized. |
| Existing Manual Purchase identity / historical references | Existing screens use `MPR No` and stored `MPR-YYYYMMDD-RRRR`; retain existing numbers, records and historical documents. Historical `REQ-…` remains searchable. The supplier receives the PO. Do not renumber Purchase Returns or change their `PR-` prefix. |
...
| `MPR No` | Outright Manual Purchase Request number; `MPR` stands for Manual Purchase Request. Latest owner correction 2026-09-23 approves screen name `Manual Purchase Request`, retaining `MPR` and Subscription prefix `SMPR`. The screen rename and Subscription prefix are approved targets, not built. Existing `MPR-YYYYMMDD-RRRR` is implementation evidence, not fresh suffix-format approval. Preserve historical `MPR-…` identities and searchable `REQ-…`; do not rewrite historical documents. `MP` is Mattress Protector; `PR` is Purchase Return. No additional column-header or navigation restructuring is approved by this name correction. |
...
New Outright PO target identity is `POYYMMDD-NNNN`; Subscription uses `SPOYYMMDD-NNNN`,
including leading zeros. Both display versions using the same no-space parentheses. Display the original as
