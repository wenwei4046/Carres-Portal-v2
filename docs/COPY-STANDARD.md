...
| Customer status (one line, collapsed) | `Delivered {date}` · `Customer refused delivery` · `Phone number is wrong` · ⭐ `Customer requested another date · {date}` · `Contact missed {date}` · ⭐ `No answer · Follow up today` · ⭐ `Waiting for customer · reply due {date}` · `No requested delivery date` · ⭐ `Scheduled {date}` (· ⭐ `Contact due today`) · ⭐ `Contact due today` · `Contact due {date}` · `{company} contacts the customer · by {date}` |
| Customer current action | ⭐ `Contact customer today` · `Contact customer by {date}` · `Agree the delivery date, then record the reply` · `{company} contacts the customer.` · `Decide the next step for this delivery` · `Correct the phone number` · `Change it in the Sales Order, then contact the customer` · `Record scheduled delivery` · `The customer asked for {date}` |
