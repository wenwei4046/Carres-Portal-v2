...
| Manual Purchase Request name and prefixes — latest owner correction 2026-09-23; rename TARGET / NOT BUILT | Document and screen name: `Manual Purchase Request`; `MPR` stands for Manual Purchase Request. Prefixes remain Outright `MPR` and Subscription `SMPR` (Subscription target not built). Replace the future screen-name target `Manual Purchase`. Standalone `Purchase Request` and `PR` / `SPR` or `REQ` / `SREQ` request prefixes remain withdrawn. `MP` remains Mattress Protector; `PR` remains Purchase Return. This correction defines no date pattern, separator or digit length and authorises no application implementation or live cutover. Wider numbering-table approval is recorded separately; ellipses do not define a format. |
