...
| Inventory `Stock use` words (owner ruling 2026-09-25) | **`Available` · `Reserved` · `In transit` · `Incoming`**, and for a Unit that cannot be promised the ACTUAL reason: **`Damaged` · `Wrong item` · `Waiting inspection` · `In repair` · `Customer return · check required`** | `Reserved / sold` (a sold Unit is delivered, in History) · `Not available` (says something is wrong without saying what) · Hold · Quarantine · Blocked |
| Inventory `Who has it` (owner rulings 2026-09-25) | **who is responsible for the goods, short and clear**: `Carres Klang` · `PJ Showroom` inside a Carres-owned Site · `NETS` once the driver has taken the goods · `AL` / `HOUZS` inside a partner Site. NETS operating Klang is a Settings fact (`Operated by`), never a holder value | `NETS Warehouse` · `NETS Delivery` · `NETS (operator)` · `NETS` for goods inside a Carres warehouse · any role word glued to a party name |
| Inventory `Site` while a Unit is moving | **`On the way to {destination}`** | In transit (as a place) · a carrier's name as a place |
