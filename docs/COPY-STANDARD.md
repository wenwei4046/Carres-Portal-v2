...
| Work left rail (owner-approved UI, Jess 2026-09-26 — replaces 2026-09-24) | header `Previous month` · **`{Mon YYYY}`** (`Sep 2026`, one line) · `Next month`; the Monday–Saturday month grid (column heads `MON`–`SAT`; Sunday never drawn): the day number over the count (empty when none); a closed day is the grey tile with its name in the accessible name/tooltip only; today is the ringed black number and this week's row is the tinted row; the chosen day is the solid-blue tile (the page's one blue); fixed rows `Missed` · `No date` (`0` printed); `Status` rows **`To do`** · **`Waiting for answer`** · **`Done today`**; `Page` with first row `All pages`; `Owner` select `All owners`. Accessible names: `{fmtDate} · Today · {holiday} · {n} action(s)` (each part only when true) | `Today` on screen · `No work` · `Public holiday ·` · `Hol` · a two-date week label · a dash anywhere in the rail · `No working date` · `Waiting` alone · `Completed` · tabs of any kind · `Covering` · a hidden `0` on a fixed row · an icon beside each page row |
...
| The Work rail (Payment Monitor grammar, Jess 2026-09-26, corrected the same day) | header `‹ {Mon YYYY} ›` · the Monday–Saturday month grid (day number · count) · fixed rows `Missed {n}` · `No date {n}` (`0` printed) · `Status` rows `To do` · `Waiting for answer` · `Done today` · `Page` group `All pages` · `Owner` select `All owners` · `Hide filters` / `Show filters`; an empty Status choice reads **`No work for this Status choice.`** | a hidden `0` · `No work` · `Today` in words · `Public holiday ·` · a two-date header · toolbar selects · list tabs · `Covering` |
...
| The middle Work card (Jess, 2026-09-26, "this format is correct") | line 1 `{document}` + `{fmtDate}` (red when missed, no word) / `No date`; line 2 the action sentence; line 3 the fact; line 4 chips `{page}` · `{party}` and, on a covered row, **`For {normal owner}`**; every line truncates with `…` and shows whole on hover; the line over the list = the chosen Date word alone | `Was due` · `Late —` · a dash anywhere · `Covered for` / `Covered by` on a row · a count in the heading · the problem sentence on the row |
