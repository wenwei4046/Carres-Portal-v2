...
| Create header fields (owner 2026-09-22; BUILT CARD 13; owner correction 2026-09-26) | `Purpose` (replaces `Need for` in this create/edit target) · `Requested By` (automatic, replaces `Raised by` here) · `Proceed Date` (automatic) · `Deliver To` · `Delivery Date`. Preserve the existing purpose-specific field words and explicit stock-intent choice. **RETIRED 2026-09-26 (Jess: "no free text"):** `Purchase requirement` and the per-line `Note` — a line is CONFIGURED through the Sales portal's own `Configure` (colour · fabric · size · options), printed under the item as the PO paper prints it. |
