...
| `To buy` when it is not a remainder | TWO short lines under the figure: **`Already on a PO`** then **`Issue PO buys again`**, with `Demand is already covered by an open Purchase Order. Issue PO creates a NEW purchase order for this quantity — it does not change, replace or reassign the existing one.` as the cell's title. The engine prints the covering document's quantity under `To buy` on a build every unit of which is already on an OPEN purchase order, so one figure means two opposite things; the first line says which, and the second says what the act DOES — traced through `issue-batch` → `purchasing_issue_pos_batch`, whose every write is an insert scoped to the purchase order it creates. **The quantity, the action and the consequence are read together or not at all.** Written as two lines, never left to wrap: the long sentence takes the item row to 91px. **Never `Open PO …`** — a retired column head. |
