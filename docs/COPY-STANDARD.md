...
| Inventory `Who has it` (owner ruling 2026-09-25) | **the organisation's own name only**: `NETS` · `PJ Showroom` · the future 3PL's name. The role (warehouse operator or delivery company) is read from `Site` and `Stock use` beside it | `NETS Warehouse` · `NETS Delivery` · `NETS (operator)` · any role word glued to a party name |
