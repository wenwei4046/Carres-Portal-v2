...
| | **`Leaves out {money} that customers paid before their invoice.`** | Second line under `1210 Trade receivables` on the Balance Sheet, the other side of the note under 2210: the amount 0506 moved off receivables, so the line still reconciles to the Trial Balance and the Journal. **APPROVED (YH, 14 Sep 2026).** | A finance user compares Receivables with 1210 on the Trial Balance, sees two different numbers, and cannot tell why. |
