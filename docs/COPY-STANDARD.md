...
| Sales Orders register `PO No` / `DO No` cell when no PO or DO exists for the order yet — OWNER RULING 2026-09-21 (Jess) · APPROVED / NOT BUILT | **`Not ordered yet`** (the SO Batch Purchase word for the same fact — owner, 2026-09-21: follow Purchasing) · **`No DO yet`**, muted, one line | `Not recorded` (says Carres failed to write a number down; the document simply does not exist yet) · `No delivery order yet` (too long for the cell, and a second grammar beside `PO No`) · a blank cell |
