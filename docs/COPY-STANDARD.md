...
| Manual Purchase name and prefixes — final owner ruling 2026-09-23 | Name: `Manual Purchase`. Outright prefix: `MPR`; Subscription prefix: `SMPR` (approved target, not built). The proposed name `Purchase Request` and future prefixes `PR` / `SPR` or `REQ` / `SREQ` are withdrawn. `MP` remains Mattress Protector; `PR` remains Purchase Return. This ruling approves no date pattern, separator, digit length, navigation change, full numbering table, implementation or live cutover. |
...
| `MPR No` | Existing Outright Manual Purchase number. Final owner ruling 2026-09-23 retains `Manual Purchase` and `MPR`, with `SMPR` as Subscription's approved/not-built prefix. Existing `MPR-YYYYMMDD-RRRR` is implementation evidence, not fresh suffix-format approval. Preserve historical `MPR-…` identities and searchable `REQ-…`. `MP` is Mattress Protector; `PR` is Purchase Return. This ruling does not approve new column headings or navigation. |
