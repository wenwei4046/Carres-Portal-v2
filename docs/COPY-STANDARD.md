...
| Inventory's can-it-be-sold column (owner ruling 2026-09-25) | **`Inventory Status`** with exactly **`Available` · `Reserved` · `Cannot sell`**; the reason for `Cannot sell` lives in the existing `Condition` column (`New` · `Damaged` · `Wrong item` · `In repair` · `Waiting inspection`) | `Stock use` · `Not available` · `Blocked` · `Reserved / sold` · a condition word inside the status |
| Inventory's three after-OUT columns (owner ruling 2026-09-25) | **`Handed over`** (the OUT date) · **`Logistics`** (the company whose driver took the goods) · **`Delivery Location`** (where it is going); all three blank while the Unit stands in a Site | With NETS Delivery · In transit · On the way · a company inside `Stock Location` |
