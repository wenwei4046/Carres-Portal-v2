...
| Register read failure / in flight — PROPOSAL / NOT LAW (planner 2026-09-18; the shared error pattern applied to this Register) | `Sales Orders could not be loaded` + `Try again` (the grid keeps its toolbar; never shown as zero orders) · `Reading the Sales Orders…` |
...
| Where | The words |
|---|---|
| The actionable goods table — owner ruling 2026-09-18 (overwrites the 2026-09-11 heads) | `Status` · `Category` · `Qty` · `Item` · `Ready Stock` · `Supplier` · `Supplier Deliver To`, after the tick column, exactly in this order. **`To buy`, `Ordered Qty` and `SKU` are absent here** — the quantity this purchase buys appears only after ticking, in the selection bar and the `Issue PO` preview, net of chosen Ready Units and existing PO coverage; `SKU` stays on the Sales Order and in `Purchase order details`. `Item` is the name with the recorded configuration as the 11px table second line. Retired heads never return: `On PO` · `Covered by` · `Deliver To` · `Still To Order`. |
...
| `Status` on the actionable goods table — existing line words only (PROPOSAL / NOT LAW for the exact mapping, planner 2026-09-18) | One word per line, the document it refers to as the 11px second line: **`Not ordered yet`** (no document, verified uncovered — the only state offered a tick) · **`Waiting for goods from supplier`** / **`Sending not confirmed`** / **`Completed`** with the PO No (the same `documentState` words Purchase Orders prints) · **`{n} POs`** when several documents carry the line (the mapping is in `Purchase order details`) · **`Already on a PO`** with the covering PO No(s) — titled with the door's own sentence `purchasingRefusal("already_on_po")` · **`Coverage not checked`** when the flag is absent (unknown is not yes) · **`—`** when the shelf fully answered the line · a setup blocker's own sentence (`Production days not set` · `{sku} has no transaction cost.` · `SKU not found`) with its fix. Never two words for one line; never `Nothing to buy here` as a second line; never `Open`. |
| `Ready Stock` cell on the actionable goods table (owner ruling 2026-09-18) | **`▸ Ready Stock · {n}`** — a button that opens that line's choosable Units under the line; **`0`** with no button when nothing is on the shelf; in flight **`Reading the stock register…`**; failed **`Ready Stock could not be read. Try again`** — never `0` for unknown. After `Choose Ready Unit`: the number chosen, with the chosen Unit IDs as the 11px second line. |
| The buy quantity | Appears only after ticking: the selection bar `{n} selected · {u} unit(s) · Issue {p} PO(s)` and the `Issue PO` preview, from the server's recomputation. Never a column on the goods table. |
