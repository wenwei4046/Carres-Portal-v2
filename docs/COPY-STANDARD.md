...
| Inbound register heads (owner ruling 2026-09-25) | **`PO Delivery Date` · `Supplier Delivery Date` · `PO No` · `Supplier` · `Item` · `Order Qty` · `Received Qty` · `Pending Delivery Qty` · `Goods Received Date` · `Receive`**; several goods print **`{n} items`** with the expansion arrow; an overdue arrangement prints **`Expected {date} · not received`** | Document (as a composite cell) · Receiving progress (as one cell) · Physical arrived Qty printed at 0 |
