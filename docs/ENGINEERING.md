...
| Live web bundle | **`index-Bj5Q2R3d.js`** from main tip `75b31502` (Orders S2.3, 2026-08-08) — carres-portal `739c2d1f` + carres-pos `7f82fc7e`, **both projects deployed before polling**, and all four canonicals converged **on the FIRST poll this time** (the ~20s index.html lag is variable, not fixed — poll, do not assume either way). **Live md5 == local build byte for byte** (`c5e9e541…`), `SERVICE_ROLE` **0**. **Verified in BOTH directions with a control:** S2.3's marker greps **0 → 1**, and S2.2's marker greps **1 on both** — which is what proves the predecessor was really read rather than 404ing into the SPA fallback (predecessor `078a3ca8`, a real 4.78 MB bundle). **No Worker deploy owed:** the only files between the live Worker's source commit `21b7e38e` and this tip are `apps/web` + docs. **NOT verified with eyes: the authenticated Orders screen** — the browser pane holds no operator session and typing a password is a red line, so the strip, the ▼ and the sort rest on 178 unit tests plus Chromium measurements at four widths. |
