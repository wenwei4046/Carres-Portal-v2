...
| Live web bundle | **`index-HGvgTGKG.js`** from main tip `b93d652b` (T6, 2026-08-06) — carres-portal `f0581d61` + carres-pos `28a2c3ca`, **both projects deployed before polling** and all four canonicals converged on the SECOND poll. Live md5 == local build (`856ddb27…`, 4,777,559 bytes), `SERVICE_ROLE` **0**. Marker on the DOWNLOADED live bundle: `fullyOnPo` **0 → 3** (the receipt flag reaching the grid), control `Yet to Order` present. *The predecessor `index-BT8sveya.js` could no longer be fetched from the canonical host (1,724 bytes = the 404 page), so the before-count is taken from the local predecessor build rather than a downloaded one — recorded rather than glossed.* |
| Live Worker | **version `88516ff6-1b80-445d-9bb8-71d5187818b5`** from main tip `b93d652b` (T6, 2026-08-06) — deployed `--env production`; wrangler echoed the `api.carresofficial.com` custom domain. `/health` **200 `{"ok":true}`**. **Owed by the md5 rule**: the dry-run bundle at the tip (`4bb2fac8…`) differs from the previous run, and the reason is real — `packages/shared/src/to-order.ts` gained the `already_on_po` gate in `validateIssuePlan`, which the issue route calls. Predecessor `9402d994` |
