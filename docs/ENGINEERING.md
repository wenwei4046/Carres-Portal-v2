...
| Live web bundle | **`index-VlKjOaSU.js`** from main tip `d50691cc` (Orders S3.3, 2026-08-08) — carres-portal `c6ddfe11` + carres-pos `d5f4c604`, both projects deployed before polling, all four canonicals converged. **Live md5 == local build byte for byte** (`dce69b14…`), `SERVICE_ROLE` **0**. Verified in BOTH directions with a control: `Actions…331` **0 → 1**, `Actions…253` **1 → 0**, and S3.2's `headerContent` **4 on both**. **Three deploys in a row on this tip** (S3.1 `46e38221` · S3.2 `c6144aaf` · S3.3 `c6ddfe11`) — convergence took 2, 3 and 1 polls respectively, which is the spread the row already warns about. **No Worker deploy owed:** every file since `21b7e38e` is `apps/web` + docs. **NOT verified with eyes: the authenticated Orders screen.** |
