...
<<<<<<< HEAD
| Migration tail | `0336`. **Verify against the tracker before numbering.** Measured 2026-08-10 on all three sources and they now AGREE: repository `0336`, every branch `0336`, tracker `0336` (`name` = `0336_an_approved_request_can_be_taken_back`, applied 07:45 MYT). The 11-file gap that opened when `0326`–`0336` were applied while living only on `stage3-preflight-merge` closed when that branch merged the same day. **DO NOT TAKE A NUMBER FROM THIS ROW — re-measure at the moment you write the file.** This row said "next free is `0337`" when it was written on 2026-08-10; by the next morning `0337_governed_batch_purchase_issue` and `0338_old_orders_cannot_issue_purchase_orders` had both shipped. A tail measured yesterday is not a number you can hold: this repository lands several migrations a day, and the gap between reading this row and writing your file is where the collision lives. Note for whoever reads this next: the tracker carries BOTH a timestamp `version` (`20260810074513`) and a `name` (`0336_…`) — **query the NAME**, because the timestamp cannot be compared against a file number. |
| Live web bundle | **`index-5owGjDhD.js`** from main tip `277a0fd9` (stage-3 merge + the rental copy fixes, 2026-08-10) — both apexes converged on the SAME bundle on the first poll, `SERVICE_ROLE` and `service_role` both **0**. Forward direction proven on the DOWNLOADED bundle: `until finance approves the rental` **1**, `no monthly price yet` **3**, with controls `Delivery address` **6** and `T&C accepted` **1** proving the file was really read rather than a 1.7 kB SPA fallback. **THREE THINGS THIS DEPLOY DID NOT PROVE, recorded so nobody reads more into it than was measured:** (1) the REVERSE direction — no predecessor bundle hash was captured before deploying, so nothing was shown to go 1 → 0; (2) local-build md5 vs live md5 was not compared; (3) the `/principal?tab=rental-setting` padding fix is **not provable from the bundle at all** — `px-9 py-8 pb-14` greps **32** because it is the standard page-padding class every sibling tab uses, so it needed eyes on the page, not a grep. Pages deployment ids were not captured at deploy time. Predecessor bundle `index-GJ7-CZe6.js` from `ff539f99` (Orders S2.5 · Expansion, 2026-08-08) — carres-portal `d47b5806` + carres-pos `64ce03ff`, both canonicals converged on the FIRST poll. **Live md5 == local build byte for byte** (`71cbb0be…`), `SERVICE_ROLE` **0**. Verified in BOTH directions with a control: S2.5's `Show items in SO-` **0 → 1** and `order-items-panel` **0 → 1**, while S3.3's `w:331` stays **1** and the retired `w:253` stays **0** — so the deploy carries the new card WITHOUT having quietly re-shipped an older width. **No Worker deploy owed:** every file since `21b7e38e` is `apps/web` + docs. **NOT verified with eyes: the authenticated Orders screen** — there is no `apps/web/.env.local` in any worktree, so a signed-in local page cannot be reached, and the 42px gutter + the eight unchanged widths were measured instead in real Chromium at 1130px against the kit's own colgroup (recorded in `orders/MASTER.md` §3 · S2.5). |
| Live Worker | **version `a09da410-7ee6-40d7-a3a9-44b0d2d3b998`** from main tip `277a0fd9` (2026-08-10, deployed by yhatjuuni@gmail.com — every prior deployment on this Worker is limchaichiew@gmail.com's, so the account is shared). **Owed and paid, and the ownership was PROVEN not assumed:** the change touched `packages/shared/src/blockers.ts`, `apps/api/src/routes/orders.ts:20` imports `isProceedBlockerCode` from it, and rather than reason about that, the bundle was built to disk (`--dry-run --outdir`) and `rental_not_approved` greps **1** inside it — the new code is demonstrably in the Worker artifact. §6's md5 idiom does not work as written on wrangler 3.114.17: `--dry-run` alone prints only the upload size and no hash, so `--outdir` plus a grep of the emitted `index.js` is the check that actually answers the question. `/health` **200 `{"ok":true}`** on BOTH canonicals, `api.carresofficial.com` custom domain and the `0 1 * * *` cron re-bound. Predecessor `fa58a642-6003-4497-8529-1d7a4d38f8f5` from main tip `a7daf937` (Orders D9, 2026-08-08) — **owed and paid**: D9 changed no `apps/api` file, but `apps/api/src/routes/operation/order-control.ts:11` imports `bookingConfirmGate`, whose answer changed, which is exactly the case §6 warns about. Deployed `--env production`; `/health` **200 `{"ok":true}`**, `api.carresofficial.com` custom domain and the `0 1 * * *` cron both re-bound. Predecessor `88516ff6-1b80-445d-9bb8-71d5187818b5` |
=======
| Migration tail | **`0338_old_orders_cannot_issue_purchase_orders`** (`20260811084500`). `authenticated` and `anon` both lack EXECUTE on the historical order-level Issue RPC; governed Batch Purchase retains EXECUTE. **Verify against the tracker before numbering.** |
| Live web bundle | **`index-DUNp7gpt.js`** from main tip `4c9884eb` (Card 3 — remove Old Orders Issue PO, 2026-08-11) — deployments carres-portal `8fc8b2be` + carres-pos `2108836d`; all four canonicals returned this bundle for three consecutive rounds. Local SHA-256 `18010108…`, 5,283,581 bytes. Authenticated production proof: Old Orders still loaded 85 rows and its remaining queues/actions, with zero `Issue PO`/`Raise PO` entries; normal Batch Purchase loaded with the governed Issue control. Predecessor/new bundle comparison removed `Raise consolidated POs` 1→0 and preserved `Remind suppliers` 1→1. |
| Live Worker | **version `8a1cb61e-d1cd-45ec-b313-1701fe3ce786`** at 100%. The retired order-level Issue route is absent; production role simulation returned `42501 permission denied` before the historical RPC body could execute. Governed `purchasing_issue_pos_batch(jsonb)` remains executable by `authenticated`. |
>>>>>>> ddedfe65b9a2223cef1faf29452849178a2ba994
