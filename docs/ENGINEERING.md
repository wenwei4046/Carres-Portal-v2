...
| Live web bundle | **`index-CWj-SnZa.js`** from main tip `de783d46` (Sales Order production cutover, 2026-08-10) — carres-portal `a4a10141` + carres-pos `9a02fbf5`, all four canonicals converged. **Live md5 == local build byte for byte** (`aaa64f32…`), `SERVICE_ROLE` **0**. Verified in BOTH directions against the predecessor fetched from its own deployment URL (`71a1b554…`), with a control: `/operation/old-orders?order=` **0 → 1**, `Old Orders (temporary)` **0 → 3**, `operation-orders-control` **0 → 2**, `/operation/orders?order=` **1 → 0**, while `Sales Orders · Sales Order — Carres` stays **1**. **Verified WITH EYES, signed in, on production at 1440 and 1130** — 64 checks, 0 failures, 0 console errors, using the repo's own seeded `operation-test@x.com` fixture; the "no `.env.local`, so no authenticated screen" limitation recorded here since 2026-08-08 is solved by driving the live origin instead of localhost. **A rollback to the predecessor build was deployed and proven to bite, then rolled forward** (`docs/SALES-ORDER-CUTOVER.md` §EVIDENCE). **No Worker deploy owed:** the card changed no `apps/api` file. ⚠️ The predecessor bundle contained **no copy of `OperationOrdersControl` at all** — Stage 1 left it reachable only through a `typeof` annotation, so it was tree-shaken out; this deploy is +110,114 bytes, which is that page returning. |
| Live Worker | **version `a09da410-7ee6-40d7-a3a9-44b0d2d3b998`**, deployed 2026-08-10 09:27 UTC (author `yhatjuuni@gmail.com`), carrying `8f9e412f` — *a row that is not there is 404, not 500*. `/health` **200 `{"ok":true}`** on `api.carresofficial.com`. Predecessor `fa58a642-6003-4497-8529-1d7a4d38f8f5`. **Re-measured 2026-08-10 with `wrangler deployments list --env production`; the row this replaces named `fa58a642` as live and was two deploys stale.** |
