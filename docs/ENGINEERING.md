...
| Live web bundle | **`index-CXU3d1wL.js`** from main tip `406181d2` (T1, 2026-08-06) — carres-portal `4aaffa3e` + carres-pos `83047a91`; **pos.carresofficial.com lagged one poll and all four canonicals converged on the second** (the documented cache lag). Live md5 == local build (`4a1a0f3a…`, 4,773,846 bytes), `SERVICE_ROLE` **0**. Both directions on DOWNLOADED bundles, predecessor fetched from its own deployment URL `c97866db` (its 4,770,082 bytes match the previous row): `kit-table-group-` **0 → 1** (the aligned order line's ☑) · `bg-kit-canvas"` **3 → 4** (To Order's root joins the other tabs) · `flex flex-col bg-kit-slate-3` **1 → 0** (the old grey root — its old count of exactly 1 identifies it as To Order's own). Controls: `Yet to Order` **1/1** · `Required By` **1/1** |
| Live Worker | **version `5e3d6c96-6498-4e29-9032-17f020c4527d`** from main tip `406181d2` (T1, 2026-08-06) — deployed `--env production`; wrangler echoed the `api.carresofficial.com` custom domain + the 09:00-MYT cron. `/health` **200 `{"ok":true}`**. **Owed by the md5 rule, not the changed-file list**: the dry-run bundle at the main tip (`55b5a1c1…`) differs from the dry-run at the predecessor's source commit `cdf9a456` (`4deda464…`), and the only code diff between them is `packages/shared/src/to-order.ts` (+18 lines of word mirrors) — a different bundle is a deploy owed. Predecessor `df3f23df` |
