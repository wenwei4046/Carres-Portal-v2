...
| Live web bundle | **`index-DLbYeDo7.js`** from main tip `aed7e5a4` (T2, 2026-08-06) — carres-portal `3458384d` + carres-pos `cf1051b4`; **all four canonicals converged on the FIRST poll.** Live md5 == local build (`c560b814…`, 4,775,949 bytes), `SERVICE_ROLE` **0**. Both directions on DOWNLOADED bundles, predecessor fetched from its own deployment URL `4aaffa3e` (its 4,773,846 bytes match the previous row): `po-rail-call-day-` **0 → 1** and `po-rail-call-overdue` **0 → 1** (the CALLS calendar) · the inline rail-row class string **3 → 1** (the page's two inline copies deleted for the shared `workspace-rail.tsx` recipe; the one left is the recipe's own). Controls: `po-rail-state-all` **1/1** · `Yet to Order` **1/1**. This deploy also carried `fbdb78bd` (header `_headers` + DealerPos rental dealerId), which was merged but undeployed |
| Live Worker | **version `3264d196-3bb8-498f-b196-bade552cab2b`** from main tip `aed7e5a4` (T2, 2026-08-06) — deployed `--env production`; wrangler echoed the `api.carresofficial.com` custom domain + the cron. `/health` **200 `{"ok":true}`**. Owed by the changed-file list itself (`apps/api/src/routes/operation/pos.ts` — the ready-date route now computes `p_new_eta` with the shared `arrivalFromReadyDate` and passes it to the RPC). Production RPC signature verified live after deploy: exactly ONE `purchasing_record_ready_date`, 4-arg (`p_po_id, p_new_date, p_reason, p_new_eta`) — 0325's shape. Predecessor `5e3d6c96` |
