...
| Migration tail | **`0337_governed_batch_purchase_issue`** (`20260810162700`), tracker and schema re-proved after real rollback/redeploy. **Verify against the tracker before numbering.** |
| Live web bundle | **`index-BhZkHeM-.js`** from main tip `0e521a34` (Batch Purchase — Legacy Issuability Parity, 2026-08-11) — redeploys carres-portal `a02b9392` + carres-pos `b8615f70`; all four canonicals returned this bundle for three consecutive rounds. Local SHA-256 `43b82143…`, 5,298,629 bytes. Authenticated production proof: normal Batch Purchase loaded and opened the governed review; SO-1254 retained `blocked_delivery_date` through reload and Clear restored normal mode; SO-1300 retained two supplier documents and open-PO explanation, with two document-level procurement-partner choices and three line-level transaction-cost decisions; Free of Charge exposed a separate mandatory-reason field; no Issue was submitted. Old Orders loaded independently with 85 rows and its legacy queues. |
| Live Worker | **version `6c5c65c0-653d-40e6-a50f-d7efe225aa13`** at 100%, restored after the real rollback proof. `/health` **200 `{"ok":true}`** on `api.carresofficial.com`. Rollback proved predecessor `552c1e43-0a34-441e-95dc-b9ac24a14a52` at 100%, then convergence returned to `6c5c65c0…`. Production crafted-RPC matrix: 9/9 laws passed; successful normal/FOC writes were deliberately rolled back; mixed two-document failure kept PO count 24→24. |
