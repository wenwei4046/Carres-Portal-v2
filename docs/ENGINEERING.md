...
| Migration tail | **`0347_a_voided_payment_is_not_money`** (Card 4 closing slice, applied 2026-08-13 — **and it is in the TRACKER as `0346_a_voided_payment_is_not_money`, version `20260813034523`**). Two lanes took 0346 in the same minute: Card 3's `an_appointment_names_the_carrier_it_was_made_with` (`20260813034521`) is the repository's 0346 and landed on `main` first, so the payments file renumbered to 0347 — red line 6 forbids altering a committed migration, and the applied tracker row keeps its own name. **The repository now holds one 0346 and one 0347; the tracker holds two rows named 0346.** 0347 adds a partial unique index on `order_payments.receipt_no` and rewrites `payment_record` / `payment_void` to stamp `payment.received` / `payment.voided` into `ops_activity_log` in the same transaction as the money — **proven by nine probes as the real operation/principal users inside two aborted transactions; ledger 0 rows, Σ `orders.paid` RM 55,350.00 and 0 `payment.*` activity rows, identical before and after.** Predecessor `0346` (Card 3). **DO NOT TAKE A NUMBER FROM THIS ROW — re-measure at the moment you write the file**, and take the MAX of the repository tail, every branch and the tracker. The collision above is what happens when two lanes read this row instead of measuring: it was written and stale inside two seconds. |
| Migration tail, previous | **`0339_one_purchase_order_creation_authority`** (Card 4B, applied 2026-08-11). Revokes EXECUTE on `operation_create_po`, `operation_create_pos_batch` and `_operation_create_po_inner` from `public`/`anon`/`authenticated`; revokes direct INSERT on `purchase_orders` + `purchase_order_lines` from `anon`/`authenticated` and drops the two INSERT policies. `purchasing_issue_pos_batch(jsonb)` keeps EXECUTE and still reaches the inner writer as its `postgres` DEFINER — **proven by running a valid governed batch as the `authenticated` operation user (minted `PO-2055`) inside a transaction that was then aborted; live counts unchanged at 24 POs / 38 lines.** Predecessor `0338` (Card 3). **DO NOT TAKE A NUMBER FROM THIS ROW — re-measure at the moment you write the file.** This repository lands several migrations a day and this row has been stale within hours, twice. Take the MAX of the repository tail, every branch and the tracker; in the tracker query the **`name`** column (`0339_…`), because its `version` is a timestamp (`20260811…`) that cannot be compared against a file number. |
