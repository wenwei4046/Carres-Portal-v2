...
| Migration tail (2026-09-13, Payment/Delivery ownership seam) | **`0499_one_responsible_operation_person_per_order_the_contact_carries_four_identities`** — APPLIED to production by its own lane after a rolled-back probe (tracker row `20260913150119`, PR #1283). A second lane wrote a colliding `0499` (`…_a_contact_names_its_responsible_person_its_acting_person_and_its_recorder`, PR #1284) for the same seam; it was CLOSED unmerged and never applied — the probe it ran on production rolled back cleanly. The applied 0499 replayed on the full chain (497 of 502, the five known failures only) and its ownership chain is proven by `apps/api/src/test/collection-owner-responsibility.integration.test.ts` (7 cases; skips without `CARRES_TEST_DATABASE_URL`). **Next free number: `0500`** — re-measure before writing. |
| Migration tail (branch note 2026-09-14, RE-MEASURED after merging `main`) | **`0493_exception_evidence_is_a_record_with_its_line`** on `build/receiving-grn-redesign`. The 2026-09-13 note on this row was measured before `main` moved 36 commits and is superseded. Measured 2026-09-14 against the merged tree: repository tail on `main` = **0504**, MAX across every ref = **0506**, and **`0493` is UNCLAIMED** — `main` jumps `0492 → 0494`, so the file keeps its number and `pnpm ci:migrations` validates 511 filenames with no two sharing one. ⚠️ **The number now sorts BEHIND eleven migrations that reached `main` after it**, so apply order was checked rather than assumed: every object `0493` creates, replaces or drops (11 of them) was compared against all of `0494`–`0504` and **the intersection is empty** — the two pre-existing functions it replaces (`warehouse_receipt_validate_lines`, `receiving_validate_session_extras`) are in neither `0500`s 78-function null-role rewrite nor any other later file, so applying `0493` after them reverts nothing and the relative order is immaterial. ⚠️ **Measured on production 2026-09-14: `0500`, `0502` and `0503` are MERGED TO `main` AND ABSENT FROM `supabase_migrations.schema_migrations`** (tracker runs `…0499`, `0501`, `0504`; only 32 of 568 SECURITY DEFINER functions carry `0500`s `app_role() is null` guard). That is another lane apply debt, reported and NOT touched here; it does not gate `0493`, whose own door gates on `receiving_require_post_authority()` and fails closed. `0493` is **NOT applied** to production — measured the same day: `receiving_line_evidence`, `receiving_line_evidence_add` and `receiving_media_kind_of_path` all resolve NULL. Proven on a real in-process PostgreSQL (`@electric-sql/pglite`, `apps/api/src/test/receiving-line-evidence.test.ts`, 15 cases). **A full-chain replay on real PostgreSQL could NOT be re-run on this machine** — no Docker, no Supabase CLI, no `psql`, and the `embedded-postgres` Windows PostgreSQL 18 binaries crash on launch (`initdb.exe` and `postgres.exe --version` both exit `0xC0000186`, a native DLL-init failure). Re-measure before push and before apply. |
| Migration tail | **`0472_the_reservation_door_is_one_door_again`** — measured 2026-09-10. **The three numbers DISAGREE and that is the headline.** Repository tail on `main` = **0469**, MAX across every branch = **0472**, tracker tail (numeric prefix) = **0458**. ⚠️ **`0461`–`0469` ARE MERGED TO `main` AND ABSENT FROM `supabase_migrations.schema_migrations`** (the Finance ledger set plus `0469_a_reversal_and_its_contra_are_both_counted`, measured 2026-09-10 alongside the long-known `0454` scar). Nothing in this session depended on them — `0442`/`0443`/`0444`/`0453` are all present, which is what `0471` stands on — and NOTHING here fixed them: they are reported, not touched, because Finance is another lane's work. ⚠️ **`0469` WAS A LIVE COLLISION, CAUGHT BY CI:** `0469_a_reversal_and_its_contra_are_both_counted` reached `main` in PR #1208 while this branch already held `0469_a_reserved_unit_names_the_sales_order_line`. This branch renumbered to **`0471`/`0472`** — the one that had NOT reached main moved — and its four tracker rows were renamed to match the file, exactly as the `0454`/`0455` pair was handled the day before. **`0455`, `0459`, `0460` and `0470` are gap numbers nothing claims.** ⚠️ **Seventeen numbers are claimed by TWO files each** — `0038 0039 0055 0165 0166 0204 0206 0232 0233 0239 0241 0242 0255 0267 0398 0417 0424` — so `ls | tail` is meaningless. **DO NOT TAKE A NUMBER FROM THIS ROW — re-measure at the moment you write the file, and again right before you push**, taking the MAX of the tracker, the repository AND EVERY BRANCH: `git branch -a --format='%(refname)' | while read r; do git ls-tree --name-only "$r" supabase/migrations/; done`. Query the tracker's **`name`** column; its `version` is a timestamp that cannot be compared to a file number. `pnpm ci:migrations` catches a same-number collision in CI, but only after the file is committed — as it did here. See `docs/carry-forwards.md` → `0454-issue-action-merged-to-main-but-absent-from-the-tracker`. |
| Migration tail, current work | **`0474`** (the Purchasing Approver duty; measured 2026-09-11 immediately before the push: `0473_the_refusal_names_the_unit_that_stopped_it` reached `main` mid-build, so this file moved from `0473` to `0474` — the one that had NOT landed is the one that moves). It creates `purchasing_approver_gate()` — which resolves `purchasing_approver` through `workspace_resolve_duty`, the Shared Duty Resolver Staff & Duties actually writes — and re-creates `purchasing_decide_request` on it. **The first draft gated on `org_position_duties` and was wrong:** that is the HR POSITION duty table, which Staff & Duties never writes, so an assignment could never have reached the door. **The re-created body was diffed line by line against the live `prosrc` before commit: the ONLY difference is the gate call on line 2** — the check that catches a hand-retyped apply. It changes no RLS policy, adds no write policy, writes no row at all and assigns the duty to nobody: the `ops_manager` rung keeps deciding while `workspace_resolve_duty('purchasing_approver')` answers `not_assigned`, so applying it changes no behaviour on the day it lands. |
| Migration tail, previous | **`0473`** (SO Batch Purchase Ready Stock). **`0471` applied 2026-09-10** via the governed MCP path in four parts (tracker rows `0471_a_reserved_unit_names_the_sales_order_line_part1`…`part4`): `ops_stock_items.reserved_order_line_id`, the SQL `stock_match_key` twin, `so_line_remaining_requirement`, the line-validating `ops_stock_pool_draw`, the atomic `so_batch_reserve_ready_units`, and the binding exposed on both register views. All six function bodies reconciled `md5(prosrc)` against the committed file. No RLS policy changed and no write policy was added; the backfill bound only what could not be wrong and left ambiguous rows NULL. **`0472` applied 2026-09-10 (tracker `20260910105452`), AFTER all three production surfaces reported the merge SHA `dd7b6a0b`** — it dropped `0471`'s deliberate 7-argument `ops_stock_pool_draw` delegator, and `pg_proc` now carries exactly ONE `ops_stock_pool_draw`, the 8-argument door naming `p_order_line_id` (measured 2026-09-11). **`0473` redefines `so_batch_reserve_ready_units` only**, so every refusal re-raises with the failing Unit in DETAIL as `unit_id=<uuid>`, keeping the original SQLSTATE and refusal word. It is **safe in either apply order**: a bundle that does not parse the detail prints the sentence it printed before, and a bundle that does, reading a pre-0473 refusal, simply names no Unit. No delegator, no deploy window. |
...
| Live web bundle | **`main` tip `1c8cd56c`** (Delivery Orders — the submission names its own trip, #1238), served by `erp.carresofficial.com`, `pos.carresofficial.com`, `carres-portal.pages.dev` and `carres-pos.pages.dev`, with the Worker on the same SHA — all five canonical surfaces read 2026-09-11. Asset `index-8z1zsJvf.js`, 6,485,292 bytes. **Proved in BOTH DIRECTIONS against its own predecessor** `index-rFH559OB.js` (6,474,604 bytes, `e280f3f7`), downloaded from the apex before the deploy: `Driver submission` 0→1 · `Showing only:` 0→1 · `Videos` 0→3 · `signed-document` 0→2 · `Proof Status` 1→0 · `Delivery photo saved` 1→0, and the CONTROL `Delivery Orders` 6→6 — the control is what proves the predecessor was really read rather than a 1.7 kB SPA fallback. **AUTHENTICATED walk, not a public probe:** signed in as an Operation account, `/operation/delivery-orders` served the eleven approved headings in order, both live documents measured exactly 38px, `WORK TO DO` counted 1/0/0 and `DOCUMENT STATUS` read `All (2)` — matching the database exactly (2 delivery orders, 0 recorded results, 0 ledger entries, 0 signed DO files). |
| Live web bundle, previous | **`main` tip `64a16a9e`** (Manual Purchase settled register, #1217), served by `erp.carresofficial.com`, `pos.carresofficial.com` and `carres-portal.pages.dev` — read 2026-09-11. Its predecessor **`2aff9964`** (SO Batch Purchase Ready Stock — the act names its Units and a stale tick is dropped, #1215) was walked on the same day at `index-k6XPVHZ0.js`, 6,322,071 bytes: **AUTHENTICATED, not a public probe** (`feedback_verify_with_an_authenticated_read`) — signed in, the computed body font came back `Inter, "DM Sans", system-ui` so the stylesheet had applied, SO Batch Purchase drew 26 Sales Orders and an expanded row drew the `Ready Stock` section stating `No stock on the shelf matches this order.` **Measured the same round: of those 26 proceeded Sales Orders, 25 are offered no Unit at all and one (SO-1209) is offered three, every one `No item line needs it`** — today's shelf answers nothing the Register still has to buy. That is a fact about the data, not the code, and it is why the RESERVE journey cannot be walked on a live Register row without inventing an order. |
| Live Worker | **source commit `64a16a9e`**, reported by `GET https://api.carresofficial.com/health` → `{"ok":true,"commit":"64a16a9e…"}`. **Read the commit from `/health`, not from this row and not from a document.** Both Ready Stock doors were exercised AUTHENTICATED against this exact Worker and again against its predecessor `2aff9964`: the read answered 200 with real arithmetic for SO-1322 (`JAGER-SS qty 1 · Ready Stock 1` — Unit `id-vyf051985` — `To purchase 0`, its three other available JAGER-SS Units correctly `No item line needs it`), and a deliberately mismatched pick answered **422 `{code: "unit_does_not_match_line", itemId: "426067bf-…"}`** — the governed word AND the Unit it is about, which is `0473` reaching the browser. The refusal wrote nothing: the Unit is still `free` and `ops_stock_pool_usage` gained no row. ⚠️ A Supabase access token expires mid-walk; reload the page and re-read the session before calling a 401 a regression. |
