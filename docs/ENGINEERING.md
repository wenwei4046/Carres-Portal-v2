...
| Migration tail (2026-09-26, Manual Purchase line configuration) | **`0591_a_manual_purchase_line_is_configured_like_a_sales_line`** — on branch `build/purchasing-one-card-grammar` (PR #1672), NOT yet applied. Numbered from the MAX across every branch: `0588`–`0590` exist on other branches, so this file took `0591`. Adds `purchase_demands.attrs jsonb` and re-creates the ONE create door and the ONE resubmit door with the per-line configuration (signatures unchanged); the issue route copies it into `purchase_order_lines.attrs`, which the PO PDF already prints. Replayed on the kept full-chain database. **Next free number: `0592`** — re-measure across every branch before writing. |
| Migration tail (2026-09-26, Purchasing per-line supplier answer) | **`0587_a_supplier_answer_is_recorded_per_goods_line`** — APPLIED 2026-09-26 through the governed `apply_migration` path after a rolled-back production probe (`probe_0587_rolled_back_do_not_track`, no tracker row). Tracker row `20260926084721`, `md5(statements[1])` = file md5 `1f7ef2f3e8050b819a0482f358b517ad`; all seven redefined function bodies reconciled against the replayed full-chain database (Purchasing MASTER §5.7 production record). Tracker tail and repository tail now agree at `0587`; **next free number: `0588`** — re-measure across every branch before writing. ⚠️ Auto-mode denies `apply_migration` as a protected production write; the apply went through only after the session was switched to manual (`default`) mode so the owner received the prompt. |
