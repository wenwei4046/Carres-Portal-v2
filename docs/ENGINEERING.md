...
| Live web bundle | **`index-DMtpHU5R.js`** from main tip `15a78ba1` (Sales Orders Reference Implementation — Stage A, 2026-08-10) — carres-portal `6ad09b64` + carres-pos `b3885d0a`; all four canonicals converged. **Live md5 == local main build byte for byte** (`13f59b90…`). Authenticated production validation at 1920 · 1440 · 1130 measured DestinationHeader 44px · Work Toolbar 45px · grid header 31px / 10px 700 · parent row 33px / 11px · footer 22px. One destination identity, Search and toolbar; zero Sales Orders tabs; outer work surface `clientHeight == scrollHeight` with overflow hidden. At 1130 the 846px toolbar had 0 clipped children and the grid owned 306px horizontal overflow. Server Search returned 2 matching rows; Customer filter 74 → 1 → 74 after Clear; Phone column survived reload; expansion opened; XLSX downloaded; 0 browser console errors. **No Worker deploy owed:** diff from the live Worker's source commit through this tip changes no `apps/api` or `packages/shared` file; live Worker remains `a09da410`. Stage B not started. |
