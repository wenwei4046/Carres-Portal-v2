...
| Migration tail (branch note 2026-09-13) | **`0499_a_contact_names_its_responsible_person_its_acting_person_and_its_recorder`** on `build/payment-collection-owner-seams` — measured 2026-09-13 before writing: repository tail on `main` = **0498**, MAX across every branch = **0498** (the Receiving lane holds **0493**), tracker tail `0498`. Replayed on the full chain (`scripts/dry-run-migrations.mjs`, 497 of 502 applied — the five known failures only) and proven on that database by `apps/api/src/test/collection-owner-responsibility.integration.test.ts`. Applied to production only through the governed rolled-back probe → apply path recorded in the closure. |
