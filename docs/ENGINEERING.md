...
| Migration tail, current work | **`0474`** (the Purchasing Approver duty; measured 2026-09-11 immediately before the push: `0473_the_refusal_names_the_unit_that_stopped_it` reached `main` mid-build, so this file moved from `0473` to `0474` — the one that had NOT landed is the one that moves). It adds ONE `org_duties` row (`purchasing_approver`), creates `purchasing_approver_gate()` and re-creates `purchasing_decide_request` on it. **The re-created body was diffed line by line against the live `prosrc` before commit: the ONLY difference is the gate call on line 2** — the check that catches a hand-retyped apply. It changes no RLS policy, adds no write policy, writes no business row and assigns the duty to nobody: the `ops_manager` rung keeps deciding until Jess assigns the new duty in `Workspace → Staff & Duties`, so applying it changes no behaviour on the day it lands. |
