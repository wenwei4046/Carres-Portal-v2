...
| Migration tail (branch note 2026-09-13) | **`0493_exception_evidence_is_a_record_with_its_line`** on `build/receiving-grn-redesign` — measured 2026-09-13 after rebasing onto `650abf39`: repository tail on `main` = **0491** (`0490`/`0491`, Delivery Card 14, #1270), MAX across every branch = **0492** (`build/delivery-15-loan-offer`, open PR #1271), open PR #1234 holds **0480**; so this branch took **0493** (its first build had taken `0490` before Card 14 merged — renamed before any push, never applied anywhere). NOT applied to production under the 2026-09-13 instruction (non-production verification only); proven on a real in-process PostgreSQL (`@electric-sql/pglite`, `apps/api/src/test/receiving-line-evidence.test.ts`, 15 cases). Re-measure before push and before apply. |
