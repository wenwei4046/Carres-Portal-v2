...
| Migration tail, previous | **`0471`** (SO Batch Purchase Ready Stock, applied 2026-09-10 via the governed MCP path in four parts, tracker rows `0471_a_reserved_unit_names_the_sales_order_line_part1`…`part4`). It adds `ops_stock_items.reserved_order_line_id`, the SQL `stock_match_key` twin, `so_line_remaining_requirement`, the line-validating `ops_stock_pool_draw` and the atomic `so_batch_reserve_ready_units`, and exposes the binding on both register views. **All six function bodies reconcile `md5(prosrc)` against the committed file** — the check that catches a hand-retyped apply. It changes no RLS policy and adds no write policy. Its backfill is additive and bound both existing reservations to the correct line of their own order; ambiguous rows were left NULL rather than guessed. **`0471` deliberately keeps a 7-argument `ops_stock_pool_draw` delegator so neither the pre-deploy nor the post-deploy bundle ever calls a missing function; `0472` DROPS it and must be applied only after production reports the merge SHA.** |
