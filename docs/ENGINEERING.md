...
| Live web bundle | **`index-D7G2BM2C.js`** from main tip `cdf9a456` (D2, 2026-08-06) — carres-portal `c97866db` + carres-pos `611ee5fe`; **two canonicals lagged and all four converged on the fourth poll** (the documented cache lag — one poll cannot tell a lag from a split, which is why it is polled until it converges). Live md5 == local build (`34a34cee…`, 4,770,082 bytes), `SERVICE_ROLE` **0**. Both directions on DOWNLOADED bundles: `receive-line` **2 → 1** and `Goods arrived at the warehouse` **2 → 1** — **and each survivor was identified rather than assumed**: the remaining `receive-line` is `data-testid="partner-receive-lines-table"` (a different page, whose testid merely CONTAINS the string) and the remaining `Goods arrived at the warehouse` is the column header `… (GRN)`, which stays. Controls: `po_skus` **1/1** (D1's key) and `Check in` **6/6** (the ruled word, still on the PO row's hand-over link) |
| Live Worker | **`df3f23df-7253-43ba-bf42-82268bbf3464`** from main tip `cdf9a456` (D2, 2026-08-06) — deployed `--env production`; wrangler echoed the `api.carresofficial.com` custom domain + the 09:00-MYT cron. `/health` **200 `{"ok":true}`**. **Owed and unarguable**: D2 DELETES `POST /operation/orders/:id/receive-line` from `routes/operation/order-control.ts`, so leaving the old Worker up would leave the bypass live. Predecessor `78e44123`, read from `wrangler deployments list` |
