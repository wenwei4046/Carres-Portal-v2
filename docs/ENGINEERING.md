...
| Live web bundle | **`index-k6XPVHZ0.js`**, 6,322,071 bytes, from `main` tip **`2aff9964`** (SO Batch Purchase Ready Stock — the act names its Units and a stale tick is dropped, 2026-09-11). `erp.carresofficial.com`, `pos.carresofficial.com` and `carres-portal.pages.dev` all served `__carres_deploy.json` = `2aff9964` in the same round. **Measured through an AUTHENTICATED session, not a public probe** — `feedback_verify_with_an_authenticated_read`. The page was walked signed in: the computed body font came back `Inter, "DM Sans", system-ui` so the stylesheet had applied, SO Batch Purchase drew 26 Sales Orders, and an expanded row drew the `Ready Stock` section, which stated `No stock on the shelf matches this order.` **Measured the same round: of those 26 proceeded Sales Orders, 25 are offered no Unit at all and one (SO-1209) is offered three, every one of them `No item line needs it`** — today's shelf answers nothing the Register still has to buy, which is a fact about the data, not about the code, and it is why the RESERVE journey could not be walked on a live Register row without inventing an order. |
| Live Worker | **source commit `2aff9964`**, reported by `GET https://api.carresofficial.com/health` → `{"ok":true,"commit":"2aff9964…"}`. **Read the commit from `/health`, not from this row and not from a document.** Both Ready Stock doors were exercised AUTHENTICATED against this Worker: the read answered 200 with real arithmetic for SO-1322 (`JAGER-SS qty 1 · Ready Stock 1` — Unit `id-vyf051985` — `To purchase 0`, its three other available JAGER-SS Units correctly `No item line needs it`), and a deliberately mismatched pick answered **422 `{code: "unit_does_not_match_line", itemId: "426067bf-…"}`** — the governed word AND the Unit it is about, which is 0473 reaching the browser. The refusal wrote nothing: the Unit is still `free` and `ops_stock_pool_usage` gained no row. Predecessor `dd7b6a0b`. |
