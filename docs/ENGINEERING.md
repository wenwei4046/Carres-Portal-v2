...
| Live web bundle | **`index-BpnvUU5a.js`** from main tip `ec433a81` (Batch Purchase — Sales Order scoped entrance, 2026-08-10) — carres-portal `56d44f4a` + carres-pos `7d55be90`; all four canonicals converged. **Live md5 == local main build byte for byte** (`791bb06c…`, 5,292,942 bytes), `SUPABASE_SERVICE_ROLE_KEY` 0. Authenticated production proof: `/operation/to-order?so=` survives reload; SO-1317 normal issuable; SO-1300 spans 2 supplier×category proposals; SO-1287 explains open-PO cover + issued history; SO-1315 explains the existing no-delivery-date block; Clear removes only `so` and restores the normal rail/sheet; a scoped Issue emitted the unchanged `POST /api/operation/purchase/to-order/issue` arrangement and was deliberately aborted before creating data; the live endpoint recomputed and rejected an invented build 409 `nothing_to_issue`; Old Orders remained 200 and named `Old Orders (temporary)`. Card 2 not started. |
| Live Worker | **version `552c1e43-0a34-441e-95dc-b9ac24a14a52`**, deployed 2026-08-10 14:48 UTC (author `limchaichiew@gmail.com`), carrying the scoped GET read model from `1160ab11`. `/health` **200 `{"ok":true}`** on `api.carresofficial.com`. Predecessor `a09da410-7ee6-40d7-a3a9-44b0d2d3b998`. |
