...
| Live web bundle | **`index-CMEMMgoB.js`** from main tip `29e81044` (T1.1, 2026-08-06) — carres-portal `c9148bc5` + carres-pos `86da5014`. **BOTH Pages projects need their own deploy**: `pos.carresofficial.com` is served by `carres-pos`, and after ten minutes of polling it was still on the previous bundle because only `carres-portal` had been pushed — the earlier row's *"lagged one poll"* note described a cache lag that was really a missing second deploy. All four canonicals now converged. Live md5 == local build (`d455473d…`, 4,776,493 bytes), `SERVICE_ROLE` **0**. On DOWNLOADED bundles, predecessor `index-DLbYeDo7.js`: `bg-kit-slate-2` **6 → 0** (the phantom token, gone from production) · `Ready Stock` **2 → 3** (the new column header) · `border-t-kit-slate-6` **5 → 6** (the white, ruled order line) |
| Live Worker | **version `885b5eda-7f00-4048-9ecd-7b58679e3322`** from main tip `29e81044` (T1.1, 2026-08-06) — deployed `--env production`; wrangler echoed the `api.carresofficial.com` custom domain. `/health` **200 `{"ok":true}`**. **Owed by the md5 rule**: the dry-run bundle at the main tip (`7f9063a7…`) differs from the dry-run at the predecessor's source commit `aed7e5a4` (`b889826f…`), the only code diff being `packages/shared/src/to-order.ts` (+1 word mirror). Predecessor `5e3d6c96`
