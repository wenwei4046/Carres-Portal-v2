...
| Live web bundle | **`index-CTbZKTLN.js`** from main tip `a7daf937` (Orders D9, 2026-08-08) — carres-portal `391b7283` + carres-pos `ee4cfe1d`, **both projects deployed before polling**. All four canonicals converged **on the second poll** — `pos.carresofficial.com` was the straggler, which is this project's normal pattern: the hashed assets go live immediately, the `index.html` that points at them lags ~20s. **Live md5 == local build byte for byte** (`074656ac…`), `SERVICE_ROLE` **0** and `service_role` **0**. D9's marker greps **both ways on the DOWNLOADED bundles**: `name:"Pillow"` · `test:/pillow/` · `"Pillow"?4:` go **0 → 1**, and the chained form it replaced — `return"Pillow"` · `"Pillow"?3:` · `if(/topper/.test` — goes **1 → 0**, with `to_reserve` (4) · `protector\|protect` (1) · `foot rest\|ottoman` (1) identical in both as the control. The predecessor was fetched from **its own deployment URL** (`078a3ca8`) and is a real 4,780,147-byte bundle, not the 1.7 kB SPA fallback that greps a clean 0 for everything. **NOT verified with eyes: the authenticated Orders screen** — the browser pane holds no operator session and typing a password is a red line, so D9's behaviour rests on 2231 shared + 2707 web tests and a production SQL re-measurement (20 orders that could never fail a stock check → 0). |
| Live Worker | **version `fa58a642-6003-4497-8529-1d7a4d38f8f5`** from main tip `a7daf937` (Orders D9, 2026-08-08) — **owed and paid**: D9 changed no `apps/api` file, but `apps/api/src/routes/operation/order-control.ts:11` imports `bookingConfirmGate`, whose answer changed, which is exactly the case §6 warns about. Deployed `--env production`; `/health` **200 `{"ok":true}`**, `api.carresofficial.com` custom domain and the `0 1 * * *` cron both re-bound. Predecessor `88516ff6-1b80-445d-9bb8-71d5187818b5` |
