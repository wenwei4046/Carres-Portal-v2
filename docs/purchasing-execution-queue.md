...
**THE ONE BUSINESS QUESTION, AND IT MUST GO TO LOO BEFORE ANY SCHEMA IS DRAWN — PO-2032 IS
MIXED.** It has three lines: **one to Carres Klang and two to AL Sungai Buloh.** So one
document is on BOTH paths at once, and *"which path is this PO on?"* has no answer for it.
Either fulfilment is confirmed **per LINE** (each line finishes by its own path, and the PO
completes when all its lines are finished — matching 0311's own per-line destination grain),
or a mixed PO is refused at issue. **Do not guess: ask once, with PO-2032 on the screen.**
