...
| P5 | 🔨 CLAIMED 2026-08-05 — `claude/p5-real-po` · **the chain HAS now been walked end to end on production** (PO-2052 · PO-2053 · PO-2054, 3 receipts, 1 claim SC-1014 closed, first `expected_ready_date` and first `balance_delivery` promise ever written). **Nothing was fixed — 9 findings recorded**, ranked by money. The three that cost real money: **① `finance_ap_aging()` is DEAD on production** (selects `po.delivery_partner_id`, renamed by 0052 → `procurement_partner_id`; raises 42703 for every caller including service role, so Finance → Accounts Payable has never loaded since 0063 shipped) · **② received goods do not attach to the customer they were bought for** (PO-2052 carried `so_refs [1209]`, the units landed `status='free'`/`reserved_ref=null`, SO-1209 holds **0** reserved units and `order_supplier_threads` is **0 rows database-wide**, so `threads_advanced` is always 0 — goods bought for one customer sit in the free pool where P10's Reserve offers them to anyone) · **③ the card's own gate #1 has no home** — a second identical PO for the same customer line and quantity was created with no refusal (proved in a rolled-back transaction; the only protection is the route recomputing coverage, a read-then-write with no lock). **The UI half is OWED**: no browser session was available and a password may not be typed, so every finding is a database/route measurement, not a screen. | — |
