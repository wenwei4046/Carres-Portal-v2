...
阶段用词(全 portal 统一,list NEXT 对齐):Balance Placed下单→Confirmed确认单→Paid收齐(类别名=Balance 不用 Money);Stock PO raised→ETA set→Goods ready;Logistic Assigned→Booked(partner 已联系客人并约好 slot)→Delivered。NEXT verb=完成当前 pending 阶段的动作(Order PO/Chase supplier/**Assign logistic**/Chase logistic/Record payment/Confirm——C-vocab 2026-07-19:assign=我们派 carrier 的动作,booked=partner 约好 slot 的状态,"Book logistic" 死词)。(“Record payment” verb 属 List 阶段 §14,详情页现阶段不加。)
...
**QUEUES C-vocab 定稿(2026-07-19,Jess 挑 C 案)**:QUEUES 行名 = NEXT 动词——全 portal 一套词(QUEUES ↔ NEXT 列 ↔ 详情页 Chase Now),数字天生对上。行序:**Overdue(结果,追谁看该行 NEXT)· Owing RM · Order PO(红)· Chase supplier(琥珀;备货窗内该行 NEXT 变红)· Assign logistic · Chase logistic** + Follow-up / For Jess;zero-hide 不变;点行 = nextFilter,chip "Next: X"。**死词:To book · Waiting stock(状态词退去 FILTERS)· No logistic(该队列 = Assign logistic)· NEXT 旧词 "Book logistic"(出现条件其实是没派车 → 改 Assign logistic)**。词法拍板:**assign = 我们派 carrier 的动作;booked = partner 跟客人约好 slot 的状态**(Delivery 列 booked <date>/not booked 保留)。PO-day banner 同词("PO day — Order PO n · Chase supplier m");cron task 文案同步。TEAM rail 另加:未入池员工灰显 "· joins on first login"(Khor Yee 8/1 首登自动入池分单,提前上榜不提前分单);通用账号(logistics@,`OPS_GENERIC_EMAILS`)永不自动入池、不上 TEAM 榜——round-4 "generic 不自动入池" 的意图落到代码(旧代码只挡管理层,logistics@ 被登录会吃单)。

**Chase 分工定稿(2026-07-19,Jess 挑 C)——按对象分线,one voice per counterparty**:**货 = PO 值日者**(Order PO + Chase supplier 两条队列是值日者的活;supplier WhatsApp 每月只有值日者一个声音;PIC 要货期内部问值日者,不戳供应商;月底交接 = open PO 清单)。**送 + 钱 = PIC**(Chase logistic + Owing 是各 PIC 点自己头像 chip 的清单;纪律:用 bulk Chase 按 counterparty 合并、固定时段发;NETS 7/30 上 portal 后送的线进系统)。**上线节奏**:SH & LC 从 **Wed 22 Jul** 开始用——Jess 10:30 会议先 brief(投屏 `apps/web/public/brief-ops-22jul.html` 一页纸),brief 完才开闸;deploy 排在 brief 之前(周二晚/周三早),首个真 PO day = Thu 23 Jul(Shasha 值日)。**`?poday=1`** = 管理层专用预览:任何一天在 Orders URL 加此参数即见 PO-day banner + duty 徽章(0236 未 live 时用第一个 pool 成员顶位,标 "· demo";员工与正常 URL 完全不受影响)——brief 现场演示用。

## 14.7 Master import 对账 + 补 line 门(2026-07-18 夜定案 — Jess 挑 A 案;BUILT `048fe32`,migration 0237 已 apply,web/API 待 deploy)
**背景**:0214 锁死 AutoCount/Master re-import = create-only,旧单永不悄悄动 → sheet 有、portal 没有的 line(例 TCF0544/SO-1138 客户两张 sofa 只进了一张)没门可补。**18 Jul Master 全量 re-import 已跑**(pre-check 全中:15 created · 259 stock lines · 60 balances · 1 storage fee · Owing RM55,758→**52,137** 与预测一分不差;12 unmatched 全是 sofa 行)。**A 案 = Import 顺手补**:import result 页新增「Sheet has it — portal doesn't」tick-list → 勾选 → Append(POST `/api/operation/orders/append-missing-lines`,dryRun 检测 / commit 服务端**重检**后走 **0237 RPC** `append_autocount_order_lines`:只准 operation/principal + 只准 autocount 单,raw sku + unit_price 0 + source_po,RPC 内写 order_history/audit + items_edited=true,anon revoked)。
**防重复的关键 = `clean` 旗**:候选 = 该行 PO 在自己单的任何 line 上都不存在;**默认打勾只给 clean**(= 单上每条 portal line 的 PO 都在 sheet 里有 → 真加货);单上存在 sheet 没有的 line(换 model / PO 重开 / line 掉 PO)→ 默认不勾 + 行下亮出 portal 那条(琥珀)。真数据 sweep:13 候选、唯一 clean = SO-1138 ✓,0 误勾。**Master 过时四单**(TCF0545/46/47/50:portal 是新版 AutoCount,sheet 是旧 model/旧 PO)→ 该改 sheet 不是补 portal。**FIX DATA 挂账**:SO-1104(TCF0529)line 无 source_po + 名字漂移(Modulo AM9038)· SO-1163(CR1203)6 条 line 全无 source_po · SO-1113 = TYCF0475 typo 鬼单(TH5119 实际住这) · SO-1112/1149 DSL9038 同 PO/2606-045 双算。CR0629 storage RM200 = staff 填错(Jess:照 import,staff 在 portal 改;⚠️ 下次 re-import sheet 没改会冲回 200)。CR1123 RM2,248 不在 sheet,owing 照挂。
...
1. 详情页(feat/orders-drawer):布局✓·Items §9✓·Balance §10✓·Delivery §12✓·Storage §11✓(rev21 2026-07-18)·**Loan 定稿✓(2026-07-19,`f24065b6`,未 deploy)**。Loan = "ON MISSION" grounded-card 语言(Jess 从她的行程 sample 拍板;白/飘/全 pill 方向作废):每张 loaner = grounded 白卡(icon header · `BORROWED · <供应商>`/`Warehouse · Klang` · 品名 · loaned 日期右上)+ labeled 行(Condition/Category · Loan DO 内嵌链接 · At customer 等待/✓Collected+Collect-swap · **Return by** 日期+tone);**pill 全退**,状态靠绿✓/琥珀行托底/红行托底。供应商 **return-by** 从 notes(`expected return YYYY-MM-DD`)读回、现形+催(`in Nd`琥珀 / `overdue Nd`红),真列=1B。**仓库挑货 = 有什么借什么(不配客户尺寸,"got what we borrow what")**,全信息(品名·状况·库龄·**原始 PO**·free 数,同款合并)·展示品优先·新货压后+sellable 软提醒;**sofa** 借出前勾"能进得去"。Portal 通用设计 DNA(此后新 UI 都照)= grounded 卡 + icon 分类通道 + labeled 行 + pills-retired。
   **Loan 完整清单(2026-07-19,全部 push 到 `feat/orders-drawer`,⚠️ 未 deploy —— deploy 交一个人管、只 deploy 分支不 deploy main)：**
   ...
   - **ON LOAN 送货单文件 ✅**(feat/loan-note,2026-07-19,Jess A):Loan 卡右上角 **🖨 Loan note** → `renderLoanNotePdf` 开 A4 PDF(letterhead + 客户 + 借件/状态/来源/收回时机 + 4 条 bailment 条款 + 双签名),客户交件时签。号码 = `docNumber` 方案(见下),**不用** `ops_loan_note_seq`(流水会泄单量)。头部 Order 行现带 CR/TCF ref。
   - **⭐ 文档编号方案 `docNumber()`(`packages/shared/src/doc-number.ts`,Jess 2026-07-19 锁定,全 portal 通用)**:格式 `前缀-DDMMYY-NNNN[-字母]`。前缀=单据类型(LN loan note · RC receipt · DO · INV);DDMMYY=出单日;**NNNN=4 位尾号,从「订单 id」FNV-1a hash 推导,不是计数器** → ①单量保密(纸上数不出你做几单)②补印必同号(签过的纸对得上)③**同一订单所有单据共用尾号**(SO-1197 的 LN/RC/DO 都收在 `1905`)。尾号非唯一键,SO 号才是;同日跨单撞号是 cosmetic,4 位近乎零。改单/重发加 `-B`/`-C`(现操作员手动;自动记 rev 要存字段=1B)。`ops_loan_note_seq` 保留不用。
   - **Loan 物流腿剩下的**:IN 腿(送货那天「Collected back」自动绑送货同趟)· 还供应商腿(软还款期✓,差「搭供应商送新货那趟」的 `supplier_return_ref`)。`loan_note_no`/`loan_note_signed_at` 列存在但暂不写(号码现是 docNumber 推导 client-side;真存 = 1B)。
2. **1B ✅**(feat/loan-note,2026-07-19):**wa.me 直发+存号**(customer chase 用存的 order phone 开 `wa.me/<intl>?text=` 预填,一点即发、绝不自动发;supplier/logistic 走群链接无预填保持复制;clipboard 桌面回退;`waLink` 已存在,现补 `?text=`)· **last_chased_at ✓**(每次 chase 已 stamp,header「chased Xh ago」)· **⋮真动作 = chase 变真 wa.me 动作 ✓**。**OCR 收据 = Jess 挑 C 暂缓**(要么付费云 OCR〔账号+预算+secret〕要么重依赖 Tesseract〔+2-4MB·准度差〕;省的只是打字、钱要准必须人复核 → 不值,量大了再上)。 3. List §14(§14.5 staff LIVE·§14.6 集合开 PO A 案 BUILT〔0236 待 apply〕·§14.7 import 补 line BUILT〔0237 已 apply 待 deploy〕—— 平行 session 已建)。 4. Bulk bar。 5. AL 调货引擎(专门 chat)。
