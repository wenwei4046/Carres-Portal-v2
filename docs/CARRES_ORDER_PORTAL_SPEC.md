...
**rev26 定稿(2026-07-19,Jess「all design floating in the air」top-to-toe,option B,commit `59e2f88`)**:病根 = 上半五步 spine 已立「单栏·节点｜chip」语法,下半 Collect/Extension 还用旧 `FieldRow` 左标签｜右内容两栏 + 一句 prose 读数 → 整卡飘。修:①**Collect / Extension → 带 band header 的贴齐子卡**(header + 单栏 body,零 FieldRow 左右甩);两组件加 `chrome` prop(`bare`=子卡单栏;默认 `fieldrow` 保 Delivery-tab Postponed 复用 + 旧 storage 面板不变)。②**Extension 已延期读数 prose → chips**(`postponed → 31 Jul 26 · Renovation · free from 9 Jul 26 · 1/1 used`)。③**步骤④ Fee 拆三行去拥挤**(day N+公式 / RM+unpaid / No storage escape),不再一行五件平级 soup。④**所有展示日期 → `fmtDateShort`(「31 Jul 26」无星期)**,守 rev25 date law。⑤**步骤3/5 原生 `<input type=date>`(16/07/2026 · dd/mm/yyyy 空盒)→ `EditableDateChip`**(读=格式化 chip,点击才露 picker;Ends 空态=「follows delivery」软 pill,杀空盒飘)。live-verified #1101+#1103。Jess UI LAW 再固化:**一张卡从头到脚只走一套 step 语法,子区也要用同一语法(band header + 单栏),不许下半退回 FieldRow 两栏**。
