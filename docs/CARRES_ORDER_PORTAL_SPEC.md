...
**步骤布局(rev23 定稿,Jess V2 修订;rev22 的右侧状态句已被 rail 取代)**:
- **左 rail(230px,竖向从头到尾)= 状态词**(§8 marks):Assigned → **Stock ready n/m**(只读,amber 圆点写 n/m,sub=落后类目+ETA+"open Items ›" 跳 Items;guardrail:Delivery 永不加 stock 动作)→ Booked → Delivered;**delivered 单 rail 收成一行 "Delivered ✓"**。rail 下方 = next-action 卡(阶梯词 + chase by 日期·−Nd 输入 + 真按钮 **Remind(ghost+bell)/ Chase(box+message-circle)** → copyChase logistic + last_chased_at 章)。状态只画一次(band 无 pill)。
- **右表单(620px)= 动作词 band**:`1 ASSIGN LOGISTIC · you`(Logistic+Apply 建议;**Customer deadline 只读** "auto · AutoCount"——import 的 New-Delivery-Date 所有权归 import;**Postponed?** = 正式一次性 0196 extension〔快照原日期、动 storage、第二次要 principal〕,已延显示 "→ 新日期 (postponed)")› `2 CALL CUSTOMER · NETS — keyed by us for now`(**Before-the-call 简报卡**:各类目 n/m+status 用 Items 同一 stockCats、尾款用 Balance 同一 balanceDue〔无 total 不显钱〕;Logistic ETA〔NETS 回报,今代填,未来 partner view〕;Customer confirmed 手动勾=纯记录)› `3 NOTES · auto-dated`(**自动盖日期流水**,append 进原 customer_request 列一行一条最新在上,零 migration,list tooltip/导出照读;客人随口改期进这里,别烧 Postponed)。
...
**Staff auto-assign(2026-07-18 Jess 拍板 B,推翻 05-14 "不做分工"——换血后新人需要明确责任)**:每单一个 owner;新单自动派给最闲在职 operation 账号(只在进单时分,**绝不半路自动换人**);人人看全表(不加 RLS 隔离);任何人可一键改派;辞职=停用账号时一键平分/指定其手上 open 单;可选 "Rebalance now"(预览后执行)。统计中性 + 新人 ramp 标记(不比烂)。**待办**:migration 加 `ops_order_control.assigned_staff`(+assigned_by/at,additive、不动 RLS,migration 文本先给 Jess 过目)→ list 行 owner 头像 + STAFF facet(可点)→ 停用账号转移流程。Housekeeping:samantha@ 停用、Ching(7/20 入职)+ 8/1 新人账号要建。
