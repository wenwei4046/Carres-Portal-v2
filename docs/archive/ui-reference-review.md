...
|---|---|---|
| **KIT-CONSOLIDATION** | The single card that updates `docs/UI-KIT.md` from the frozen set. | ✅ **DONE 2026-07-29, card D0.6.** All five principles are in the law — **R1 → §8.0 · R2 → §0.4 · R3 → §0.5 · R4 → §0.3 · R5 → §10.1** — each with a `Rule · Enforcement · Status · Evidence` row, and a grep for each ratified sentence returns exactly one hit outside this file. **From this moment `docs/UI-KIT.md` is the source and this file is the record of where the reasoning came from.** Findings, and the two that were reported rather than acted on, are in [`docs/ui-kit-execution-queue.md`](ui-kit-execution-queue.md) under D0.6. |
| **Worklist floorplan** | R1's finding: Fiori would call `ACTION-FLOW-STANDARD`'s "Open My Work" a Worklist, and that page does not exist. | **Parked by the PM, 2026-07-28. STILL PARKED — deliberately NOT carried into §8.0.** D0.6 wrote the closed-catalogue rule and did not add a fifth floorplan, which is what the parking ruling requires: a gap is a governance event, not a licence to draw. **No design discussion may be started from it.** |
| **Implementation observations** | R2's three `localStorage` sites; R4's stale machine mirror. | **CARRIED 2026-07-29, both, and in opposite ways.** R4's mirror was **corrected** — `design-standard.ts` declares `UI-KIT 2026-07-27`, drops its claim to outrank the law, and no exported value moved (proved: 0 `hex:` lines and 0 colour literals in the diff). R2's three sites were **named in §0.4 and not touched** — they are what its Build-Guard rule will fail on at D1, and one of them (`hiddenCols`) is a feature somebody asked for, so removing them is Jess's call, not a guard's. |
| **Documentation debt** | `docs/ui-reference/` does not exist while `UI-KIT` §11 cites screenshots in it. | **Classified by the PM, 2026-07-28** as documentation debt. **NOT carried by D0.6** — it was named out of scope on that card and is still open. |
