...
### ✅ SHIPPED 2026-08-04 — and DEFECT 2 WAS BLOCKED BY A RULE, NOT BY MISSING CODE

**The card's premise for defect 2 was stale in the same way P8's was, and measuring first is
what turned a UI card into a one-gate migration.** *"The payload carries `sku` · `qty` ·
`destinationId` · `requiredBy` · `remark` and no `purpose`"* was true of the CLIENT and
false of the wire: the route already sent `p_purpose`, hardcoded, because
**`purchasing_create_demand` refused every other value BY NAME** (`purpose_not_enabled`) —
Jess, 2026-08-03, *"V1 buys READY STOCK only… Display begins at the Dealer/Sales portal and
Office at a future internal request workflow."* **0319 wrote that refusal deliberately so
that *"the day one is approved this gate is the only thing that changes"*, and 0323 is that
day.** Loo ruled the four the CHECK already held; **the CHECK is not widened, and no value is
added, removed or re-ordered.**

**`Spare Parts` and `Other…` are NOT offered, and that is the card's own Done-when rather
than a narrowing.** Both are ruled WORDS in `TO_ORDER_WORDS`; **neither has ever had a
database value.** Offering six when the store can record four would mean the operator's
choice is refused by name on two of them — *a demand records the Source the operator chose*
would be false — which is the exact disease 0322 had to repair on the pool's reasons, where
one list lived in four places and only two moved. Naming `other` here would also be a
migration ruling on a business question nobody has asked: other than what, recorded where?

**DEFECT 1 IS BIGGER THAN THE CARD MEASURED, and production says so: FIFTEEN SKUs share the
model name `Booqit`, not four** — every one a `part` variant, every one Ohana. The card saw
four because the picker caps at 8 results. `railItemLabel` prints the model plus a size
letter and a part has no size, so fifteen different products rendered one word. **Verified on
the deployed page**: searching `Booqit` returns 8 rows, all reading `Booqit` in Model, each
separated only by its own code (`5539-1A(LHF)` … `5539-2B(LHF)`).

**THE STOCK NUMBERS ARE ZERO ON LIVE DATA AND THAT IS THE CORRECT ANSWER, not a wiring
fault.** `free` is P10's rule EXTRACTED into `readFreeStock` and called by both surfaces —
two callers of one body, so a second count is structurally impossible. The register holds
**975 free units** and every one is keyed off a free-text warehouse name while the catalog
uses codes: measured, `stockMatchKey("B1201F-Q")` is `b1201f|Q` and
`stockMatchKey("Breeze FirmCare-B1201F-Q")` is `breezefirmcareb1201f|Q`. They do not match,
which is **P10's own measured finding** (*"total offered = 0"*) reproduced — so the picker
shows exactly the number the grid would offer, which is what the Done-when asks for.

**TWO DEPARTURES, both stated rather than slipped in.** ① The picker's second column is
**`Model`**, not the card's `Item`: the FIELD this picker belongs to is already labelled
`Item`, so `Item` as a header would put one word on two things in one dialog (C6's own
finding, where two blocks both read `Actions`). `Model` is the ruled word the GRID uses for
this exact value. ② It renders through the kit's **`DataTable`**, not a hand-rolled table —
which is why **check-design is 8367, identical category for category to `origin/main`**,
proved by linting a DETACHED WORKTREE at main. A hand-rolled one measured **+1 in category G**
(§0.1), and the guard's own escape (`not-a-list-page`) covers only the page SHELL, which
genuinely does not belong inside a modal.

**THE SUPPLIER IS SHOWN AND NEVER CHOSEN — Jess's ruling survives intact.** The dialog prints
`Supplier: Ohana` on pick; the RPC derives it from the SKU, there is no parameter to override
it, and a route test asserts a smuggled `supplierId` reaches nothing. `Required By` and
`Destination` were not touched.

**A LATENT HOLE IN THE PAGE'S OWN SOURCE SCAN, found and fixed.** The JSX comment stripper ran
FIRST, and its leading brace-then-whitespace opener starts at ANY brace followed by a block
comment — so a function body opening with a doc comment let it delete **5,868 characters of
live code**, including both demand doors, while the test went on passing. That is the silent
shrink P14's own note records having already been paid for once. Stripping ordinary block
comments first cannot do it: the match begins at the comment. **Caught by the guard's own
create-door assertion, which failed on its own.**

**Verified BEFORE the merge and AFTER the apply.** The **negative control ran first and
FIRED** — the live pre-0323 function refused `warranty` with `purpose_not_enabled`, *"purpose
warranty is not available yet"*. Then the sanity block plus **11 behavioural assertions** in a
rolled-back transaction (all four stored; `spare_parts` and `other` refused by NAME rather
than by a raw CHECK; unknown sku · qty 0 · bad destination still refused; the default still
`ready_stock`), **rollback proved total**. Applied THROUGH the mechanism — tracker row
`20260804114009` — and the applied body reconciles **byte-identical to the COMMITTED file**
(`md5(prosrc)` `5fecede1…`, 2329 chars, computed from `git show HEAD:…` rather than the
working copy). 6 of 6 post-apply, rolled back.

**The number came from rule 17's widened form and all three measurements were taken**:
tracker tail `0322` · repository tail `0322` · every branch's tail `0322` → 0323.

**SEVEN negative controls, each a real edit verified applied, each fired**: free counts
reserved → api 2 · route re-hardcodes the purpose → api 1 · the two unstorable words made
offerable → shared 3 · the SKU column removed → web 5 · the supplier fact removed → web 1 ·
the comment-stripper order restored → web 1. **One control did NOT fire and the CONTROL was
fixed rather than kept**: widening the Model column does not remove the SKU, so it was
measuring nothing.

**Live effect today, measured**: 200 SKUs offered · 5 withheld for having no supplier ·
Carres Klang · 975 on hand · 975 free · **0 reserved**, so On Hand and Free read the same
number on every row until the first reservation exists. `purchase_demands` holds ONE row and
**0 non-ready-stock demands** — the Source starts recording from the next demand typed.

**VERIFIED IN A REAL BROWSER ON PRODUCTION at 1280×800**, which is the half jsdom structurally
cannot do: six fields (`Item · Qty · Reason · Destination · Required By · Remark`), five
columns (`SKU · Model · On Hand · Reserved · Free` at `143 · 143 · 71 · 62 · 57` inside a
512px modal), the listing **not** scrolling sideways, page horizontal scroll **0**, rows
**40px**, **0 clipped cells**, `Supplier: Ohana` on pick with no control inside it, and the
Reason list holding exactly `Ready Stock · Display · Warranty · Office`. **The first attempt
measured a 0×0 viewport** — the documented pane-not-displayed condition — and every width in
it was an artifact; only `resize_window` made the numbers real.

**A 401 on the new route proves nothing and it is recorded that way**: auth runs before
routing, so `/demand/pick-items` and a nonsense path both answer 401 (measured side by side).
What IS proved is the version serving **100%** built from the tip that contains it, plus the
deployed page reading it with a live operator session.

**Five things reported, not fixed.** ① **`Source` vs `Reason` vs `Purpose`** — one concept,
three spellings across §0A's frozen field list, this mirror and Jess's own comment. The label
reads `Reason` because that word is already in `TO_ORDER_WORDS` and the six option words are
named after it; `Source` has never been on a screen, so spelling it would be the new word the
card forbids. **Jess's to rule.** ② **`On Hand` vs `On hand`** — the Stock register's page
title and its sidebar tab disagree; another lane's file. ③ **The live `purchase_demands` table
carried NO comment** although 0319's file sets one — hand-apply drift, and 0323 restores it.
④ The register's free-text SKU vocabulary is why every stock number reads 0; that is a data
question, not this card's. ⑤ `Spare Parts` and `Other…` now have a screen that deliberately
cannot offer them — if the business wants either, it needs a CHECK value and its own card.
...
### ✅ SHIPPED 2026-08-04 — PR #609 `dc1908c5` · **no migration · no api** · web `index-b8xJQC_M.js`

**THE MEASUREMENT CAME FIRST AND IT CHANGED THE PLAN.** Production at 1440×900: table
1094px, content ~345, `Model` alone **470px — 43%** for `Sonic S` (46.4). Every width is
now measured in a real browser against the app's own stylesheet, against **the worst string
the column CAN hold** — supplier names read out of `suppliers`, model names out of
`product_models`, never off the six rows that happen to be on screen.

| column | worst content | +16 pad | header floor | width | ratio |
|---|---|---|---|---|---|
| Supplier | `Carres Internal` 90.8 | 106.8 | 101.6 | **111** | 1.22× |
| Qty | `999` 24.2 | 40.2 | **50.8** | **55** | header-bound |
| Model | `Mattress Protector SS` 134.3 | 150.3 | 90.7 | **155** | 1.15× |
| PO No. | `Yet to Order` + `Cancel` 143.0 | 159.0 | 94.7 | **163** | 1.14× |

**THE `+4` IS A SUB-PIXEL GUARD AND IT WAS PAID FOR ONCE ALREADY.** `PO No.` shipped its
first build at exactly 159 — 143.0 of content in 143 of box — and the browser **ellipsized**
it. `scrollWidth === clientWidth` reported it fitting while the screen said `Yet to Ord…`:
text metrics are fractional, box widths round, and a column sized to the exact measurement
is one rounding away from a lie. **The eye is the last check, not the first.**

**THE KIT HAD TO CHANGE, and the card's own rule is the reason.** In `table-fixed` the
browser hands spare width BACK OUT over the columns unless something `auto` is there to
take it — so *"size a column to its content"* is a number the browser immediately
overrides, and ❌ *let one column absorb the leftover width* is unenforceable without a
mechanism. `DataTable` gains ONE optional prop, `sizing="content"`, plus a trailing
**FILLER**. **It is not a column**: it never enters `columns`, so it cannot be sorted,
filtered, hidden or reordered, it carries no word, it is `aria-hidden`, and the header row
a screen reader hears is still the four business words. Passing nothing keeps D0.5c's
markup byte for byte, which is why Purchase Orders and Receiving are untouched.

**`sizing` TAKES NO DEFAULT VALUE — `undefined` IS fill.** The kit's own §10.1 guard
forbids defaulting a prop to a string and caught `sizing = "fill"` on the first run. It was
right: the default must be the ABSENCE of the caller's choice, like `order` and `widthPct`.

**`Model` IS SIZED FOR A STRING THAT DOES NOT EXIST YET, deliberately.** Protectors are 0
today, but the rail's Pillow and Mattress Protector rows are Jess's planned-ahead
placeholders — *"the rows exist so the layout never moves again"* — so sizing Model to
today's `Sonic S` would move the layout the day they arrive.

**MEASURED AT 1440, 1280 AND 1024 IN A REAL BROWSER, then again on production.** Every
column at exactly its ruled width (`42 · 32 · 111 · 55 · 155 · 163`), **0 clipped cells**,
rows **40px**, page horizontal scroll **0** at all three. Truncation is detected over
`td, th, td *, th *` — **the ellipsis lives on nested `truncate` spans, and a `td`-only
scan is exactly how the first pass missed the `PO No.` clip above.**

**REPORTED, NOT SOFTENED.** At 1024 with the sidebar EXPANDED the listing region scrolls
**20px** (region 540, table 558); on production, with the operator's own collapsed rail, it
is **0**. The only way to remove it is to squeeze a column below its content, which is the
one thing this card forbids — it costs a scroll, the alternative costs a truncated word.

**Flush**: the 16px side gutters and the kit's `rounded-t-card` are gone, and the grid
wrapper's `rounded-card overflow-hidden` went with them. Live, the sheet's right edge is
**1388** and the right rail's left edge is **1388**. **Both sibling pages were opened and
checked on production**, because the radius is shared: Purchase Orders still renders its
nine frozen minimums (`96 · 87 · 83 · 94 · 135 · 135 · 140 · 206 · 192`) and Receiving
`88 · 92 · 104 · 150 · 291`, both with no filler and rows at 40px. **Reported and NOT
mine**: Purchase Orders' expand `<td>` clips 40px of button into 33px of column — the
predecessor bundle measured the identical **35 / 40 / 33**.

**Expand keeps its ONE job** (P10's meaning, untouched) and **no inline second line was
added** — rule ③ is an exception left unused, because no fact needed one.

**A TEST THAT ASSERTED THE OLD LAW WAS REWRITTEN, NOT DELETED.** `the grid's width system`
asserted *"every column is a PERCENTAGE and the set sums to 100"* — and summing to 100 IS
the instruction "stretch to fill". The tests do not prove the widths are right; jsdom has no
layout. They prove nobody quietly puts the old system back.

**Both directions proved on DOWNLOADED bundles, with controls**: `table-filler` **0 → 3**,
`155px` · `163px` · `111px` each **0 → 1**, and going the other way `rounded-t-card`
**1 → 0** — while `Yet to Order` is **1 in BOTH** and `to-order-cancel-` **5 in BOTH**, the
markers proving the predecessor `index-C3XUGs8p.js` was really read, fetched from its OWN
deployment URL `b38e7e41` (4,765,187 bytes, a real bundle rather than the 404 page a grep
reads as a clean 0 for everything). **All four canonicals FLAPPED for ~2 minutes** and
`wrangler pages deployment list` settled it (`dc1908c` the newest Production/main writer on
BOTH projects) before they converged — **one poll cannot tell a lag from a split**.

Gates: web tsc **0** · shared **2129/2129** · the two suites **121/121** · full web suite
**2485 passed / 16 pre-existing** (§17.7), zero new · **check-design 8366 against main's
8367, proved by linting a DETACHED WORKTREE at `origin/main`** — identical category for
category except **E 1693 → 1692**. Four negative controls, each a real edit verified
applied, each fired: **2 · 2 · 1 · 1**.
...
> **Written from a top-to-toe review Loo asked for on the LIVE page, 2026-08-05.** Every
> number below was measured in a real browser at 1280, not estimated. He ruled the fix
> (option A) after seeing them.

### THE MEASUREMENTS — this is the whole card

```
viewport            1280
  rail   200  ·  listing 568  ·  panel 400

the register's table   1203px wide   inside 568px   → 635px off-screen
the expand's header    1171px wide   inside 568px   → Qty · Destination · Received unreachable
the expand's Description column        787px        → flex-1 eats everything and evicts the rest

and, at the same moment, on one screen:
    the expanded row   PO-2038
    the right panel    PO-2032        ← TWO DIFFERENT PURCHASE ORDERS
```

**The three columns off-screen are `Customer Delivery`, `Expected Arrival` and
`Current Action`.** So with the panel open — its default state — **the page cannot show the
column it exists for.** Q1's risk order and Q8's new word are both invisible.

**AND THE Q1b CLOSURE WAS WRONG, which is this manager chat's error.** Q1b was closed with
*"Q7 deletes COMPACT_KEYS, so the columns never hide again."* They no longer hide — **they
scroll off the right edge, which is worse**: the compact set at least chose which five
survived and had an honesty guard, and a horizontal scrollbar chooses nothing and warns
nobody.

### THE STRUCTURAL FIX — Loo's option A, and it is one state, not a rule

**A purchase order is looked at in ONE of two ways, never both:**

| | what it is for |
|---|---|
| **the right panel** | read it · print it · talk to the supplier |
| **the row expand** | change its lines — destinations, the two dates, the note |

```
click a row            → the PANEL opens on that PO          (the row does not expand)
click the row's  ⌄     → the row EXPANDS and the panel CLOSES → the list gets its full 968px
collapse the expand ⌃  → the panel comes back, same PO
the panel's  ✕         → closes it; clicking any row brings it back
```

**Make it ONE selected id and ONE mode.** `{ poId, mode: "panel" | "expand" }` — **two
different purchase orders must be structurally unrepresentable**, exactly as Q5 made "one
expand at a time" a single id rather than a Set. A rule that says *keep them in step* is a
rule somebody will break; a state that cannot hold two is not.

### The four other repairs, all agreed by Loo

**Ⓐ The `›` toggle leaves the shell header and becomes a `✕` on the panel.**
It is the PANEL's control, not the page's — Gmail, GitHub, Linear and Fiori all put close at
the panel's own top-right. **And it is a dead end today**: `openPo` sets only the URL param,
so once the panel is closed, clicking a row does nothing and the operator is stuck. With the
✕, closing then clicking any row brings it back, and **no "show" button is needed at all.**

**Ⓓ The expand must live inside the VISIBLE width, not the table's `min-w-[1205px]`.**
The expanded cell spans every column and inherits that minimum. It should be pinned to the
visible viewport of the scroller (`position: sticky; left: 0`) and sized to it, and
`Description` must take what is LEFT rather than `flex-1` eating 787px and evicting three
columns. **Measure the result in a real browser; jsdom has no widths.**

**Ⓔ `Print PDF` moves up beside the PO number, and the `DOCUMENT` band is deleted.**
Measured: `PO-2038` 100.8 + gap 8 + `Print PDF` 83.8 = **192.6px inside 368px usable** — 175px
spare. The band held one button; its title and hairline cost ~30px for nothing. The object's
identity and its actions belong together at the top (Fiori Object Page · BC document page ·
GitHub · Linear).
**The three Communication controls do NOT move up** — measured `Print PDF` + `Copy message` +
`Open WhatsApp group` = **385.1px, which does not fit 368px** — and Loo froze that band's home
on 2026-08-03: *"Communication starts from the DOCUMENT, never from the register."*

**Ⓔ The items listing comes back to the panel, READ-ONLY** *(this was card Q9, absorbed here)*.
Loo: *"为什么你把我 right panel 里面的 listing 拿走？give me back."* Q5 removed it to obey
rule 1; using it, clicking a row opened a panel that named the supplier, the dates and the
communication and **never said what was ON the purchase order**. Rule 1 is already narrowed in
`PURCHASING-INFORMATION-MODEL.md` §12.7.5: **a fact may be READ in two tiers and WRITTEN in
only one.** No dropdown, no date field, no `Save`, no `⋮` in the panel — editing stays in the
expand, so rule 3 (one editing surface) is untouched.

### DONE WHEN — on production, with your own eyes, quoting the numbers

- **The panel and the expand can never show two different POs.** Try to make them; say what
  the state made impossible.
- With a row expanded, the listing is **968px** and `Customer Delivery`, `Expected Arrival`
  and `Current Action` are all on screen. Measure and quote.
- `PO-2032` expanded shows all six of its expand columns — `#` · `SO No.` · `Description` ·
  `Qty` · `Destination` · `Received` — **with nothing off the right edge.**
- Clicking a row after closing the panel with `✕` brings the panel back.
- The panel shows `PO-2032`'s 3 items **without expanding the row**, and that block has no
  editable control — asserted by a test.
- `Print PDF` sits beside the PO number; the `DOCUMENT` heading greps zero.
- Negative controls, each a real edit with its count: let the id and the panel diverge · make
  the panel's items editable · put `Description` back to `flex-1`.

### MUST NOT

❌ change any column, width or token on the REGISTER (Q7's nine are measured and frozen) ·
❌ move editing into the panel · ❌ move the Communication controls up · ❌ add a word —
every string here already ships · ❌ touch Receiving, Orders or To Order · ❌ hold the card
for a design round (§13.2).

### REPORTED, NOT IN THIS CARD

**The rail's five buckets: three read 0 and always will** (`Ready to Receive` · `Completed` ·
`Cancelled`) — this tab has no receiving door, so nothing can move them. 200px of permanent
navigation, three fifths of it silent. **Hiding a zero would also hide a filter the operator
may want**, so it is a word-and-behaviour decision for Loo, not a repair.

### ✅ SHIPPED — what actually landed (2026-08-05)

**PR #613 merge `547e7a84` · no migration · no api · web `index-BO-R9Kej.js` — DEPLOYED**
(carres-portal `e6c8e4c6` + carres-pos `35f5180c`, both `--branch=main`; all four canonicals,
live file md5-identical to the local build `6ef4a422…` at 4,768,607 bytes, `SERVICE_ROLE` **0**).
**No Worker deploy was owed and it was measured against the LIVE WORKER'S OWN SOURCE COMMIT**
— which is NOT the one §17.1 named: the row still said `3b87b0ab` from `f2517f99`, and
`wrangler deployments list` says the Worker serving 100% is **`620bc69a` from `d460d899`**
(P15's, deployed the day before). Against the real one, `git diff d460d899..HEAD -- apps/api
packages/shared supabase/migrations` is **EMPTY**. Measuring against the stale row would have
made this card deploy another lane's api half for no reason. `/health` **200 `{"ok":true}`**.

**THE DEFECT WAS REPRODUCED ON PRODUCTION BEFORE ANYTHING WAS TOUCHED, and the card's own
numbers came back exactly.** At 1280: rail 200 · listing 568 · panel 400 · the register's table
**1203px inside 557px visible → 648px off the right edge**; the expand's own header **1171px**
with `Description` at **787px**. Then, live: clicking `PO-2032` while `PO-2038` was expanded
left **the panel on PO-2032 and the expand on PO-2038** — the exact pair the card names.

**ONE STATE, and it is what makes the two-PO screen unreachable.** `{ poId, mode }` through one
reducer (`nextPoView`): the id lives in `?po=` and there is only ever one of it, so the panel
and the expanded set read the SAME id. Expanding a different row MOVES the selection with it —
that is the whole mechanism, and it is why no rule saying *keep them in step* was written.

**VERIFIED IN A REAL BROWSER ON PRODUCTION, every step of the card's flow, in one run:**

```
start                  panel PO-2032   expand —
expand PO-2038         panel —         expand PO-2038      ← never two POs
collapse               panel PO-2038   expand —            ← the PO worked on, not the stale one
panel ✕                panel —         expand —   listing 968
click PO-2031's row    panel PO-2031   expand —            ← the dead end is gone
```

**Ⓓ MEASURED, with `PO-2032` expanded at 1280 — all six expand columns on screen, nothing cut
off:** listing **968**, scroller visible **957**, the record **935px wide ending at 952**, page
horizontal scroll **0**, **0 clipped elements** inside it.

```
#  17–33 (16)   SO No. 41–105 (64)   Description 113–664 (551)
Qty 672–712 (40)   Destination 720–880 (160)   Received 888–952 (64)
```

Scanned rows still **40px**. At 1920 the listing is 1608, the register scrolls **0**, and the
record is 1575px ending at 1592.

**`position: sticky` WAS TRIED AND MEASURED NOT TO WORK, so it is not left in as a class that
does nothing.** A probe carrying `sticky left-0` inside the expansion `<td>` scrolled straight
off (left **−123** at `scrollLeft` 400) while the identical probe one level up, outside the
table, stuck at the scrollport edge (**260**). A sticky element whose containing block is a
table cell does not hold horizontally. It costs nothing today: expanding CLOSES the panel, so
the record fits the 957px visible width without any scrolling at all.

**Ⓔ the panel says what is ON the PO again, read-only** — `PO-2032` shows its 3 items
(`Booqit 1B(LHF)` · `2A(RHF)` · `CNR`, SO-1211, TOTAL 3 / 0) **without expanding the row**, and
the block holds **0** buttons, inputs, selects or textareas. `Print PDF` sits beside the PO
number; the `DOCUMENT` heading greps **0** on the panel.

**Gates.** web tsc **0** · page suite **89 → 98** · full web suite **16 pre-existing (§17.7),
zero new** — the first full run showed 19 and a second showed exactly 16, the extra three being
load-timing flakes in the two files that already time out · **check-design 8365 against 8366 on
`origin/main`, proved by linting a DETACHED WORKTREE at main** (never a stash — memory's own
rule). The one category that moved is **I 303 → 302**: a finding REMOVED, because the print
button is now spelt once. **Three negative controls, each a real edit verified applied**: let
the id and the panel diverge → **11** fail · make the panel's items editable → **1** ·
`Description` back to `flex-1` → **1**. **Both directions proved on DOWNLOADED bundles with two
controls**: `po-panel-items` · `po-panel-close` · `po-panel-title` · `100cqi` · the grid
template **0 → 1** each; going the other way `po-workspace-toggle` **1 → 0** and
`Show purchase order` **2 → 0**; while `po-work-` **1/1** and `Print PDF` **2/2** in BOTH — the
markers proving the predecessor `index-BiJsXPig.js` was really read rather than 404'd.

### REPORTED, NOT FIXED

1. **The card's Done-when asks for three columns on screen at 968 and only ONE of them gets
   there. Measured, not estimated.** With the expand open the listing is 968 and the visible
   width 957, while Q7's nine columns come to a frozen 1168 (1205 with the kit's control
   column) — so **248px still scrolls**. `Customer Delivery` ends at **806** and IS now on
   screen (it was not before); `Expected Arrival` ends at **1012**, 55px short, and
   `Current Action` at **1204**. The card's MUST NOT forbids touching those widths, and Q7's
   own ruling is that the listing region scrolls sideways when the nine do not fit. All nine
   are on screen from about 1450px of listing width up — at 1920 the register scrolls 0.
2. **The panel's last column says `Received`, not the pre-Q5 `Recv`.** Both strings shipped
   before Q5; `Received` is the newer of the two, it is the EXPAND's own word one tier away,
   and the panel's tracks were measured for it. One fact, one word — a departure from the
   card's sketch, made deliberately and recorded here rather than slipped in.
3. **§12.7.5 rule 1's note still points at "Card Q9"**, which was absorbed. Corrected in place
   to Q10 — the rule itself is unchanged, it was already narrowed by Loo on 2026-08-04.
4. **§17.1's api-bundle row was STALE and it nearly cost a wrong deploy** (see above). It now
   names `620bc69a` from `d460d899`.
...
> **Loo, 2026-08-04, looking at the live page: *"为什么你把我 right panel 里面的 listing
> 拿走？give me back."*** Q5 moved the items grid out of the panel and into the row expand.
> He asked for that split himself; using it, it is wrong. **CLAUDE.md §13.2 is his own rule
> and it governs here: real operator feedback overrides theoretical discussion, and a design
> found worse after deployment is refactored, not defended.**

### What Q5 did, and why it read well and worked badly

Q5's own comment records it: *"the items grid, the per-line destination and the ops remark →
the expand's line rows. **Nothing was deleted and nothing is in two places** — rule 1 (nothing
appears in two tiers) and rule 3 (one editing surface)."*

**The measured cost, which the rule did not predict:** clicking a row now opens a panel that
says who the supplier is, where it goes and what has been communicated — **and never says what
is ON the purchase order.** To see the items the operator must ALSO expand the row. **Two
actions where there was one, on the most-asked question about a PO.**

### Build

**Put the items listing back in the right panel, READ-ONLY.**

```
PO Issued          1 Aug 26                         PO-2038
Delivery To        Carres Klang
Supplier           Nice Future
Customer Delivery  4 Aug 26

ITEMS                                    ← back
#   SO NO.     DESCRIPTION       QTY   RECV
1   SO-1203    L1201S K            1      0
    TOTAL                          1      0

DOCUMENT · COMMUNICATION · COMMUNICATION HISTORY     ← unchanged
```

- **Read-only. No dropdown, no date field, no `Save`, no `⋮`.** Editing stays in the expand,
  and that half of Q5 is not reversed: **rule 3 — one editing surface — still holds**, and two
  editable copies of one PO line will eventually disagree.
- The panel shows what is ON the PO. The expand is where it is CHANGED.
- The per-line `Destination`, the ops remark and the two date fields **stay in the expand
  only** — those are the editable ones.

### The rule that changes, and it must be written down

**`PURCHASING-INFORMATION-MODEL.md` §12.7.5 rule 1 — *"nothing appears in two tiers"* — is
narrowed, not deleted.** Correct it in place, under Loo's name:

> **A fact may be READ in two tiers. It may be WRITTEN in only one.**
> Rule 1 was written to stop two sources of truth; two READERS of one source were never the
> danger. What is forbidden is two places that both write, and two places that could disagree.

### DONE WHEN

- On production, clicking `PO-2032` shows its 3 items in the right panel **without expanding
  the row**. Say what you saw.
- The panel's items block has **no editable control at all** — asserted by a test.
- The expand is unchanged: still editable, still one PO at a time.
- Negative control: make the panel's items editable → the read-only test fails. Quote it.

### MUST NOT

❌ move editing back into the panel · ❌ remove or change the expand · ❌ add a word that is
not already on the page (the column headers `#` · `SO NO.` · `DESCRIPTION` · `QTY` · `RECV` ·
`TOTAL` all shipped before Q5 and come back exactly as they were) · ❌ change any column,
width or token · ❌ hold the card for a design round.
...
**Lane: PURCHASING · the BUYER's side. MIGRATION: probably yes — the fulfilment record.
NOT a Receiving screen (§1's role-anchor rule).**

**THE RULE IS ALREADY FROZEN AND THIS CARD ONLY BUILDS IT.** Loo, 2026-08-05, written into
[`PURCHASING-WORKING-FLOW.md`](PURCHASING-WORKING-FLOW.md) §9 and
...
- goods into a Carres warehouse → **no button**: posting the Receiving Session IS the
  confirmation.
- goods **straight to the customer** → **Operation confirms explicitly**, because no
  Receiving Session can ever exist for them.
- fulfilment carries its own record — **who · when · which path** — and may never be
  expressed by writing `received_qty` or by reusing a Receiving status.

**MEASURED LIVE, 2026-08-05 — one real purchase order is stuck today.** `purchasing_destinations`
holds `Carres Klang` (warehouse-linked, 24 POs) · `AL Sungai Buloh` (address only, **2 PO
lines**) · `HOUZS` (address only, 0). Both of those lines are on **PO-2032**, and nothing can
ever receive them, so `received >= ordered` is unreachable and the PO can never read
`Completed`.

**THE GRAIN IS SETTLED AND IT IS NOT A QUESTION — PER LINE.** *"本来就是一项一项"* (Loo,
2026-08-05, answering a chat that had put it to him as a choice). A line finishes by its own
path; the PO finishes when all its lines have. **A MIXED purchase order is normal** — PO-2032
has one line to Carres Klang and two to AL Sungai Buloh — so **nothing in this build may ask
*"which path is this PO on?"*, and a mixed PO is never refused at issue.**

**TWO PATHS, AND THE SECOND ONE COVERS SELF-COLLECTION:**
...
**Loo asked on 2026-08-05 what *"顾客自己去厂里拿"* belongs to, and it is the second path.**
Fulfilment asks one question — *did the supplier hand the goods over?* — and both answer it
identically. **Who carries the goods is a DELIVERY fact and already has a home**; the only
real difference is EVIDENCE (a carrier reports back, a self-collection is reported by the
factory), which rides the record's note. **Build ONE confirm act for the second path, not
two**, and do not add a `self_collect` path value.

**`which path` is a STORED value of the fulfilment record, not a re-derivation of the
destination** — a destination can be edited afterwards, and the record must keep saying how
the goods actually went.

**Free measurement the builder should not re-derive:** the path is already readable with no new
column — a destination with `warehouse_id is null` can never produce a Receiving Session
(0307's constraint makes it either a warehouse or a plain address). What is missing is the
RECORD of the confirmation, not the ability to tell the paths apart.
...
**Must NOT.** ❌ write `received_qty`, or any receipt, to close a direct-ship line — that is the
one thing the frozen rule exists to forbid · ❌ build it as a Receiving screen · ❌ invent a
word: every string needs a COPY-STANDARD row first · ❌ decide the mixed-PO question in code.
...
| **Q14** | ✅ **SHIPPED 2026-08-05 — one supplier date, one door** (**no migration · no api · no wire change · no new word**; web only, and the Receiving FORM is untouched). Q12 named the defect and deliberately left it: `POST /pos/:id/tomorrow-delivery` had TWO doors on TWO tabs and `balance-date` had ONE, on the wrong tab. **THE ORDER WAS THE WHOLE CARD and it was kept: the balance door LANDED on the register first, then both call buttons came off Receiving, then the modal was deleted** — reversing it would have left `Confirm balance delivery date` with no surface anywhere in the portal. **THE PLACEMENT WAS DECIDED BY A PRODUCTION MEASUREMENT, NOT BY TASTE.** The obvious home is the expand's item rows, and it is wrong: `docRowsOf` deals ONE PO line out across every sales order it covers, and **7 of 21 open POs carry more than one SO, holding 18 of the 35 lines** — so a door on the item row would have given one fact two to five buttons, which is this card's own defect one tier down. It is rendered from the ENGINE's array instead (`balanceDeliveryCallsOf` walks `po.lines`), so **one door per line is structural rather than a rule somebody keeps**; the negative control that renders it per item row fires **3**. **The door landed on the queue row that was already there** — Loo's own words on `Confirm ready date` the same day, *queue and door in one place* — so no block moved and the expand Q5 froze was not redesigned. **The tomorrow call keeps its statement and gains NO trigger**: its door is `SupplierDateForm` a line above, and a second one would rebuild what this card tore down (control fires **1**). **ZERO NEW WORDS**: the five ruled strings are `order-action-words`' (`Record balance date` · `Balance date recorded`), the line's fact is `poReceivingProgress().pendingLabel` — R1's shipped sentence, exactly as the retired modal printed it — and free text reuses this surface's own live `Remarks` rather than importing the modal's `Note (optional)`, so one page does not spell one thing two ways. **A FAULT THE CONSOLIDATION WOULD HAVE CREATED WAS FIXED WITH IT, and it was the deleted hook's own docblock that named it**: `useRecordTomorrowDeliveryMutation` re-read the ORDERS queries because a `delayed` answer opens Delay planning on every customer order the PO covers, while the surviving `useRecordSupplierDate` re-read only the PO list — so consolidating onto the narrower door would have left a recorded delay showing a stale Orders board. The wider invalidation moved up to the survivor. **The modal and the dead hook are DELETED, not left unrendered** — a component with no callers is the `ops_order_control.balance` disease in its other form — and `ReceivingWorkspace` loses its `calls` prop, its `answerFor`/`onAnswer` state and three imports, asserted by a source scan. **A SOURCE SCAN, NOT A RENDER TEST, IS WHAT GUARDS THIS** (R8 · C1 · P13 · Q7's lesson): the whole `apps/web` tree is read from disk and callers are COUNTED — each endpoint now has exactly one, and both are the register's. **That scan failed on its own first run and the fix is the durable part: my own prose tombstone in `queries.ts` names the retired hook, so scanning for the bare word counted a COMMENT as a caller** — the needle carries a trailing `(`, because a call and a definition both have one and a sentence about either does not. **Live effect today: NONE, measured — 0 of 35 PO lines have received anything, so 0 balance calls are open**; what changes is that the action can be closed at all, from the tab that owns it. Gates: web tsc **0** · build clean · register suite **101 → 110** · Receiving **23 → 26** · full web suite **16 failed / 2592 passed (2608)** against a **DETACHED CONTROL WORKTREE at `origin/main` `4dd2e315`** measuring **16 / 2580 (2596)** — the 16 diffed **test name for test name and IDENTICAL**, so zero new and +12 tests · **check-design 8323 against the control's 8331, every category equal or LOWER** (`G` hand-rolled table/input/select/overlay **686 → 685**, the modal's own overlay; `O` −7), nothing up. **Five negative controls, each a real edit verified on disk, firing 3 · 5 · 1 · 1 · 1.** **Reported, not fixed**: the two supplier-call doors had **ZERO test coverage anywhere in the repo** before this card — a write onto the supplier promise ledger guarded by nothing, which is why the second door survived unnoticed · `purchasingActionDone` for both calls now has no consumer, because this surface's three existing doors (`SupplierDateForm` · `ReadyDateField` · `LineRow`) fire no toast and adding one for a fourth would be the inconsistency · the balance endpoint takes free text as `reason` while the register's date form keeps `reason` for the countable category — a wire asymmetry this card may not touch · Receiving's `Current Action` column can now only say `Check in` (§13.3 predicted it), and **deleting that column is the Receiving module's call, reported to them, not this card's** | — |
| **Q15** | ⬜ **a purchase order whose goods never touch a Carres floor can be finished** — Loo's `Fulfilment ≠ Received` ruling, 2026-08-05, already written into `PURCHASING-WORKING-FLOW.md` §9 and `RECEIVING-INFORMATION-MODEL.md` §1.1/§1.2; **this card only builds it.** `PO Complete` is the SUPPLIER's word, `GRN` and `Inventory` are Receiving's, and fulfilment gets its OWN record (who · when · which path) — never `received_qty`, never a Receiving status. Warehouse path gains **no button** (posting the Session IS the confirmation); direct-to-customer needs Operation, because no Session can exist. **Measured live: PO-2032 is stuck today** — 2 of its 3 lines go to `AL Sungai Buloh`, a destination with no warehouse, so `received >= ordered` is unreachable. **The grain is SETTLED — per LINE** (*"本来就是一项一项"*), so a MIXED PO like PO-2032 is normal and is never refused at issue. **TWO paths only**, and the second covers *"顾客自己去厂里拿"* — fulfilment asks *did the supplier hand the goods over?*, which a self-collection answers exactly as a carrier delivery does; who carries the goods is a DELIVERY fact, and the only difference is evidence, which rides the record's note | — |
