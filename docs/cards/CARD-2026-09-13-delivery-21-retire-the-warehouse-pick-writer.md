# DELIVERY — CARD 21 · The legacy warehouse-pick writer is retired

> Module **DELIVERY** · Card **21** · authored 2026-09-13 under the Delivery final-convergence
> takeover (owner mission of 2026-09-13, closing 🟡 note (3) of Delivery MASTER §16 after
> Cards 08–18). Steps use checkbox syntax for tracking. **Surface owned by this Card:** the three
> legacy SQL doors below, the two API routes that call one of them, and the Old Orders drawer's
> `Transfer to ready` menu item. **May not touch:** the exact-Unit register, `stock_balances`'
> derivation triggers (0366), any Receiving, Outbound or Delivery writer, any gate.
...
**Migrations required:** one — `0499_the_warehouse_pick_writer_is_retired.sql` (numbered from
...
- [ ] Migration 0499: drop the three functions; assert the 0366 trigger is armed; rolled-back production probe with the negative control (a hand-written `reserved` still refused)
...
- [ ] PR → CI → merge → apply 0499 through the governed path → deploy → production verification (functions absent, trigger armed, routes 404) → Delivery MASTER §16 closure
