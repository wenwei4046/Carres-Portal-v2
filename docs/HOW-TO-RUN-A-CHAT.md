...
>
> **量数字和截图是两件事，不要混（2026-08-05 实测更正）：**
>
> | | 要什么 | 靠不靠得住 |
> |---|---|---|
> | **JS 量宽度 / 颜色 / 文字** | `preview_start` + `resize_window` | ✅ **一定可以**，这是你的主要工具 |
> | **截图** | 还要那个浏览器窗格**真的显示在画面上** | ⚠️ 窗格没显示就会超时 |
>
> **所以「量不到」永远不是可以接受的结论 —— 量一定量得到。**
> **「截不到图」有时候是真的**，那就照实写「宽度已量，照片欠着」，
> 不要因为截不到图就连量都不量。
