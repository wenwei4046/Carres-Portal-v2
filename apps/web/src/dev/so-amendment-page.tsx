...
type DCol = { key: string; label: string; align?: "left" | "right" | "center"; nowrap?: boolean; width?: string; min?: string };
  ...
  return (
    <div className="overflow-x-auto">
      <table aria-label={label} className="w-full border-collapse text-body">
        <thead><tr className="border-b border-kit-slate-5">{cols.map((c) => <th key={c.key} style={{ ...(c.width ? { width: c.width } : {}), ...(c.min ? { minWidth: c.min } : {}) }} className={`whitespace-nowrap px-2 py-2 text-label font-medium text-kit-slate-11 ${al(c.align)}`}>{c.label}</th>)}</tr></thead>
        <tbody>
          {rows.map((r) => [
...
/* ── the page ────────────────────────────────────────────────────────── */
/** The approved Sales Order amendment page, rendered inside the REAL Portal shell by
 *  so-amendment-shell-preview.tsx (dev only). Not production code. */
export default function SoAmendmentPage() {
  ...
  const { setPane, pdfError, retry: retryPdf } = usePdfCanvases(showPdf ? `rev-${docVersion.rev}-${docVersion.signedAt ?? "u"}-${current.rev}` : null, () => renderReview(pdfData));
  ...
  if (PAGE_STATE !== "ready")
    return (
      <div className="flex h-full min-h-0 flex-col bg-kit-slate-2">
        <SalesOrderTabs identity={CASE.so} />
        {PAGE_STATE === "loading" ? (
  ...
  /* The REAL object header (SalesOrderTabs): ← Sales Orders · SO number · customer · actions · the
     portal's own icons, then the view row — the same one-implementation header production uses. */
  ...
  const VIEWS = ["Order", "Revisions", "History", "Order Route"];
  const navigation = (
    <nav aria-label="Sales Order views" className="flex h-full items-stretch gap-1">
      {VIEWS.map((view) => {
        const active = tab === view;
        const locked = editing && view !== "Order";
        return (
          <button key={view} type="button" role="tab" aria-selected={active} disabled={locked}
            onClick={() => { setTab(view); setViewRev(null); }}
            className={`relative px-3 text-body disabled:opacity-40 ${active ? "font-semibold text-base-900 after:absolute after:inset-x-2 after:bottom-0 after:h-0.5 after:bg-kit-blue-9" : "text-base-600 hover:text-base-900"}`}>
            {view}
          </button>
        );
      })}
  ...
  const orderCards = (src: Version, readOnly: boolean) => {
    ...
    const itemCols: Column<Line>[] = [
      ...
      { key: "code", label: "Item Code", width: "130px", cell: (l) => l.kind === "goods" && !edit ? <ReadOnlyValue editable><span className="break-words">{skuOf(l)}</span></ReadOnlyValue> : <span className="break-words">{skuOf(l)}</span> },
      ...
      { key: "unit", label: "Unit (RM)", width: "120px", align: "right", numeric: true, cell: (l) =>
          edit && !l.removed && l.kind === "goods"
            ? <div className="min-w-[80px]"><Input id={`unit-${l.id}`} aria-label={`Unit price ${l.name}`} type="number" min={0} step="0.01" value={l.unit} onChange={(e) => patch(l.id, { unit: Math.max(0, Number(e.target.value) || 0) })} /></div>
      ...
      <>
        ...
        <Panel title="Items" padding="none">
          <DocTable label="Items" cols={[
              /* Minimum widths from the real shell (1440 with the menu shown left the code 50px and the
                 description 90px): a column never crushes; below the minimums the table scrolls in its box. */
              { key: "no", label: "#", width: "28px" }, { key: "code", label: "Item Code", min: "104px" }, { key: "desc", label: "Description", min: "130px" },
  ...
  const order = (
    <div className="flex flex-col lg:h-full lg:min-h-0 lg:flex-row" data-testid="so-split">
      ...
      <aside className="min-w-0 border-t border-kit-slate-5 bg-kit-slate-3 px-4 py-4 lg:min-h-0 lg:w-1/2 lg:overflow-auto lg:border-l lg:border-t-0" aria-label="Sales Order document">
        ...
        <div className="mx-auto max-w-[700px]">
          {pdfError && <div className="mb-3 flex items-center justify-between gap-2 rounded-card border border-kit-red-6 bg-white px-3 py-2 text-body text-kit-slate-12" data-testid="pdf-error"><span>The document could not be drawn. {pdfError}</span><Button size="sm" variant="neutral" onClick={retryPdf}>Try again</Button></div>}
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-kit-slate-2" data-so-theme="trial">
      {simBanner}
      <SalesOrderTabs identity={CASE.so} customer={current.form.name} right={headerRight} navigation={navigation} docTitle={`${CASE.so} — Carres`} />
