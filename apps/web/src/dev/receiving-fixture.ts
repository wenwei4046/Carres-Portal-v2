/**
 * RECEIVING PREVIEW FIXTURE — DEV ONLY.
 *
 * The seeded identity, the card fixture and the fetch stub shared by the two
 * Receiving dev entries: `receiving-preview.tsx` (the page alone, in a
 * shell-shaped frame) and `receiving-shell-preview.tsx` (the page inside the
 * REAL Portal shell — PortalSidebar, OperationApp routing, ModuleHeader).
 * Importing this module installs the stub. Fixture data only — no production
 * record is read or written here.
 */
import { buildGrnRegisterView } from "@carres/shared";
import { useAuth } from "@/lib/auth";

/** ?page=duties | report | warehouse — defaults to the Receiving register. */
export const PAGE = new URLSearchParams(window.location.search).get("page") ?? "receiving";
