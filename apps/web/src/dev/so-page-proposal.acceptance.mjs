...
t = flat(await form(p));
let r = await revs(p);
ok(/Rev 2 · Current .*? Not signed/.test(r) && /Rev 1 .*? Signed Fri, 21 Aug 2026 10:05/.test(r), "Revisions: Rev 2 (correction) not signed, Rev 1 keeps its signature");
await p.close();
...
t = flat(await form(p));
ok(t.includes("Qty: Mattress 1 · Accessory 1"), "Rev 2 applied");
...
t = flat(await form(p));
ok(t.includes("Rev 2") && t.includes("Qty: Mattress 1 · Accessory 1") && !t.includes("Signed by"), "View version Rev 2: complete order, no signature card");
