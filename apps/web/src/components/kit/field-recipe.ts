...
/** Everything a control shares: surface, hairline, radius, type, focus. */
export const CONTROL_BASE =
  "w-full bg-white text-body text-kit-slate-12 rounded-control border " +
...
export function controlClass(error: boolean, shape: "single" | "multi"): string {
  return [
    CONTROL_BASE,
    error ? CONTROL_BORDER.error : CONTROL_BORDER.rest,
    shape === "single" ? CONTROL_SINGLE_LINE : CONTROL_MULTI_LINE,
  ].join(" ");
}
