...
export default function SalesOrderCard({ orderId, label }: { orderId: string; label: string }) {
  ...
  const goods = (o.order_lines ?? []).map((l) => `${lineName({ sku: l.sku })} ×${l.qty}`);
  const money = moneyOfOrder(o);
  const balance = !money.known
    ? "Value not recorded"
    : money.outstanding > 0
      ? `RM ${RM.format(money.outstanding)} · not paid`
      : "RM 0.00 · paid";
  ...
  return (
    <WorkSection className="shrink-0 px-3 py-2 min-[768px]:px-4" data-testid="work-sales-order" aria-label="Sales Order">
      <div className="flex items-center justify-between">
        ...
        <Link to={`/operation/orders/so/${encodeURIComponent(orderId)}`} className="text-[12px] leading-4 text-kit-slate-11 underline underline-offset-2 hover:text-kit-slate-12" data-testid="work-sales-order-open">
          Open {label}
        </Link>
      </div>
      <dl className="mt-1.5 grid grid-cols-[92px_minmax(0,1fr)] gap-x-2 gap-y-1">
