 ...
 * `operationProcurement.test.tsx`. Per-channel behavior (Sofa pickup-flight
...
let tabHookState: {
  data: operationPosListResponse | undefined;
  isLoading: boolean;
  isError: boolean;
  error: Error | null;
...
vi.mock("@/lib/queries", async () => {
  ...
  return {
    ...
    useOperationSuppliers: () => suppliersHookState,
...
function setLoaded(pos: operationPoListRow[]) {
  tabHookState = {
    data: { pos },
    isLoading: false,
    isError: false,
...
beforeEach(() => {
  ...
  // GET /api/operation/pos/:poId/threads for own_logistics suppliers. Any
...
describe("ProcurementTabContent — list rendering + filter chips", () => {
  ...
  it("Pickup action chip excludes already-received POs (sup_status=delivered + status=received)", () => {
    setLoaded([
      ...
      // Genuinely awaiting operation action — partner just delivered to wh
      // but operation hasn't pressed Receive yet.
  ...
  it.each([
    ...
    (supStatus, poId) => {
      setLoaded([
        makePo({
          id: poId,
          sup_status: supStatus as operationPoListRow["sup_status"],
        }),
      ]);
      render(wrap(<ProcurementTabContent slug="nice-future" />));
    ...
    (_label, poId, supStatus, status) => {
      setLoaded([
        makePo({
          id: poId,
          sup_status: supStatus as operationPoListRow["sup_status"],
          status: status as operationPoListRow["status"],
        }),
      ]);
      render(wrap(<ProcurementTabContent slug="nice-future" />));
  ...
  it("PoDetailModal Print button fetches JSON + renders @react-pdf client-side", async () => {
    ...
    await waitFor(() => {
      expect(apiFetchSpy).toHaveBeenCalledWith(
        "/api/operation/pos/PO-2074/print-data",
      );
    });
    await waitFor(() => {
