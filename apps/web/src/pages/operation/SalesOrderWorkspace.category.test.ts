...
describe("categoryWord — the catalog answers, the parser only fills gaps", () => {
  it("the headline: a sofa the keyword list cannot see prints SOFA when the catalog says so", () => {
    ...
    expect(categoryWord({ sku: MISSED })).toBe("OTHER GOODS");
  ...
  it("a native SKU prefix still names kinds the classifier has no branch for", () => {
    ...
    // for both, so dropping this rung would print OTHER GOODS over a guarantee.
  ...
  it("null means asked-and-silent and falls through exactly as before", () => {
    ...
    expect(categoryWord({ sku: MISSED, category: null })).toBe("OTHER GOODS");
  ...
  it("`Other goods` survives — an unrecognised line is never called an Accessory", () => {
    ...
    expect(categoryWord({ sku: "ZZZ-NOTHING-RECOGNISES-THIS" })).toBe("OTHER GOODS");
  });
