...
const LANE = [
  ...
  "pages/operation/components/PoIssueEvidence.tsx",
...
describe("Purchase Orders · the Register, object and send surface share one dictionary", () => {
  const register = visibleSource("pages/operation/purchase-orders/PurchaseOrdersPage.tsx");
  const send = visibleSource("pages/operation/components/PoIssueEvidence.tsx");

  it("uses the official date and document words", () => {
    expect(register).toContain('label: "PO Issued"');
    expect(register).toContain(">Official PO<");
    expect(register).not.toMatch(/>Official document</);
    expect(register).not.toMatch(/label: ["'`]PO Date["'`]/);
  });

  it("uses the governed PO identity instead of printing Version 1", () => {
    expect(send).toContain("purchaseOrderIdentity(po.id, version)");
    expect(send).not.toMatch(/· Version \$\{version\}/);
  });
});
