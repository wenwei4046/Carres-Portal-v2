...
import type {
  operationReservedDrilldownResponse,
...
let drilldownHookState: {
  data: operationReservedDrilldownResponse | undefined;
  isLoading: boolean;
  isError: boolean;
  error: Error | null;
  refetch: ReturnType<typeof vi.fn>;
...
describe("OperationWarehouse page", () => {
  ...
  it("14. 'Movement log' button calls setTab('movements')", () => {
    setLoaded();
    const setTab = vi.fn();
    render(wrap(<OperationWarehouse setTab={setTab} />));
  ...
  it("23. ⚙ Threshold button per row opens SetThresholdDialog prefilled (T42-pass3-C1)", () => {
    ...
    // the warehouse page so operation users can configure low/high WITHOUT
