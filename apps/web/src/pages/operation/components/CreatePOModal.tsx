...
/**
 * CreatePOModal — proto NewPODialog (operation-screens.jsx lines 791-1026).
 *
 * Lets operation raise a Purchase Order. SKUs are auto-routed to suppliers via
...
export default function CreatePOModal({ prefill, onClose }: Props) {
  ...
  // on the button's onClick → refetch(). Source: GET /api/operation/stock-alerts
  // (T18 route → RPC `operation_stock_alerts()` migration 0054).
  //
  ...
  /**
   * T22 — Pre-populate the lines list from `operation_stock_alerts`. Mirrors
  ...
  async function suggestFromAlerts() {
    try {
      ...
      //
      // T42-pass3-C3 — `operation_stock_alerts()` returns one row per
          ...
          return (
            ...
            >
              ...
              >
                ...
                {needsPartner && (
                  <div>
                    <div className="label mb-1.5">operation partner *</div>
                    <select
                      value={partnerFor(g.supplier)}
                      onChange={(e) =>
                        setPartner(g.supplier.id, e.target.value)
                      }
                      aria-label={`operation partner for ${g.supplier.name}`}
                      className={INPUT_CLS}
                    >
                      {partners.length === 0 && (
