import type { operationLowStockRow } from "@/lib/queries";

/**
 * Low Stock side card — SKUs with `available <= 1` across all warehouses.
 *
 * Mirrors `reference/proto/operation-dashboard.jsx` `StackedLayout` rows
  ...
  return (
    <div className="bg-white border border-base-200 rounded-md overflow-hidden">
      ...
      <div className="px-[18px] pb-3.5">
        {rows.length === 0 ? (
          <div className="text-meta text-base-500 text-center py-4">
            All SKUs healthy.
          </div>
        ) : (
          rows.map((row) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`font-mono text-label font-semibold ${tone}`}
                >
                  {available} units
                </span>
              </div>
