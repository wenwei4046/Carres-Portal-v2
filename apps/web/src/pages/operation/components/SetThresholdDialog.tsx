 ...
 * OperationWarehouse page (T21 wires it; T20 is component-only).
 *
 * Wired against:
 ...
 *
 * Cache invalidation: blasts `qk.operation.warehouse()` so the warehouse page
 * refetches the row + threshold pill, plus `["operation", "stock-alerts"]`
...
export default function SetThresholdDialog({
  ...
  const save = useMutation({
    mutationFn: (input: { low: number | null; high: number | null }) =>
      apiFetch(
        `/api/operation/warehouses/${warehouseId}/skus/${encodeURIComponent(sku)}/threshold`,
        {
          method: "POST",
