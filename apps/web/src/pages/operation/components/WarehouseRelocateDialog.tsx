 ...
 * Reject branch lands the PO in `customer_rejected` sup_status. operation
 * picks an alternate warehouse; the wrapped RPC `operation_relocate_warehouse`
 * resets the inbound leg so the supplier (or partner) can re-route the goods.
 *
 * Wired against:
 *   - GET  /api/operation/warehouse              — flat list for the picker
 *   - POST /api/operation/pos/:id/relocate-inbound — { new_warehouse_id }
 *
 ...
 * rather than fold this into the more elaborate `Modal`+`useOperationWarehouse`
...
export default function WarehouseRelocateDialog({
  ...
  const { data: warehouses } = useQuery({
    ...
    queryFn: () => apiFetch<Warehouse[]>("/api/operation/warehouse"),
  });

  const relocate = useMutation({
    mutationFn: () =>
      apiFetch(`/api/operation/pos/${poId}/relocate-inbound`, {
        method: "POST",
