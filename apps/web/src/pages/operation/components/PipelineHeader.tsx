...
/**
 * 2026-05-10 — Pipeline header for `/operation/orders[/:stage]`.
 *
 ...
 * path is exactly `/operation/orders`). Each chip is a `<Link>` so right-click
 * → "Open in new tab" Just Works and the URL is shareable.
 *
 * Sub-filter row: `All / Running late / Last 24h` chips + search input. All
 * three chips + search are wired client-side in `OperationOrders.tsx`:
...
export interface PipelineHeaderProps {
   ...
   * computes these from the same `useOperationOrders` query so chip badges
  ...
  activeStage: OperationStage | null;
...
const STAGE_LABELS: ReadonlyArray<{
  /** URL slug — matches the operationStage enum value. */
  slug: OperationStage;
