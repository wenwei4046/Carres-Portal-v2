...
export interface DraftAddon {
  ...
  /** Free-form attrs jsonb. For disposal addons: { size, sizes } — see above. */
...
export function step2Valid(d: WizardDraft): boolean {
  ...
  for (const a of d.addons) {
    ...
    if (isDisposalAddon(a.key) && disposalUnitSizes(a).some((s) => !s)) return false;
  }
  return true;
}

/** Returns the first failing disposal addon's display label, or null when
 *  every disposal unit has a size. Used by the wizard footer to surface a
...
export function step2FirstDisposalIssue(d: WizardDraft): string | null {
  for (const a of d.addons) {
    if (!isDisposalAddon(a.key)) continue;
