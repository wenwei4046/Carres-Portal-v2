...
export default function DealerLeaderboard({ stats, setTab }: Props) {
  ...
  return (
    <div className="bg-white border border-base-200 rounded-md">
      ...
      <div className="py-2">
        {stats.length === 0 ? (
          <div className="px-[18px] py-7 text-center text-[12px] text-base-500">
            No dealers yet.
          </div>
        ) : (
          stats.map((d, i) => (
            ...
            >
              ...
              <div className="min-w-0">
                <div className="text-[13px] font-semibold flex items-center gap-2">
                  ...
                  {d.status === "suspended" && (
                    <span className="text-[9px] px-1.5 py-px bg-base-200 rounded-full text-base-600">
                      SUSPENDED
                    </span>
                  )}
                  {d.status === "pending" && (
                    <span className="text-[9px] px-1.5 py-px bg-primary/10 rounded-full text-primary">
                      PENDING
                    </span>
                  )}
                </div>
              ...
              <div className="font-mono text-[13px] font-bold text-base-900">
