...
/**
 * LogisticsMovements — HQ stock-in/out history with KPIs + period chips +
 * by-month grouping + 5-filter row + CSV export.
 *
 * Mirrors `reference/proto/logistics-screens.jsx` `LogisticsMovements`
 * (lines 109-407):
 ...
 *   - All 5 filters + period are sent to GET /api/logistics/movements (M4.3)
 ...
 *   - LogisticsWarehouse passes `{ sku, warehouseId }` when the user clicks
 *     the "Movement log →" button or a SKU row. We accept the `initialFilters`
 *     prop and seed our filter state once on mount; LogisticsApp routes the
 *     prefill through.
 */

interface Props {
  /** Optional one-shot prefill from LogisticsWarehouse (sku and/or warehouseId). */
...
export default function LogisticsMovements({ initialFilters, setTab, clearInitialFilters }: Props) {
  ...
  // Build the filter object passed to useLogisticsMovements. Skip from/to when
  ...
  const movementsQ = useLogisticsMovements(filters, {
