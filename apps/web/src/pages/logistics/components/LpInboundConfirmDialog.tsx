...
/**
 * LpInboundConfirmDialog — Logistics-side 代按 (proxy) for HoOKkA Sofa
 * pre-flight inbound confirmation. Mirrors the Sofa SOP v2 (sops.ts):
 * supplier marks PO `ready_confirm_sent`, and Logistics-Procurement (LP)
 ...
 *
 * Wired against:
 *   - POST /api/logistics/pos/:id/lp-accept-inbound  (Task 29 — landed)
 *   - POST /api/logistics/pos/:id/lp-reject-inbound  (Task 31 — pending)
 *
...
export default function LpInboundConfirmDialog({
  ...
  const accept = useMutation({
    mutationFn: () =>
      apiFetch(`/api/logistics/pos/${poId}/lp-accept-inbound`, {
        method: "POST",
        body: JSON.stringify({}),
      }),
    onSuccess: () => {
  ...
  const reject = useMutation({
    mutationFn: () =>
      apiFetch(`/api/logistics/pos/${poId}/lp-reject-inbound`, {
        method: "POST",
        body: JSON.stringify({}),
      }),
    onSuccess: () => {
