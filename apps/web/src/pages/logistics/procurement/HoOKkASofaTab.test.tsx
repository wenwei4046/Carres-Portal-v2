...
let posListState: LogisticsPoListRow[] = [];
...
vi.mock("@/lib/queries", async () => {
  ...
  return {
    ...
    useProcurementTab: (slug: string) => {
      ...
      const data: LogisticsPosListResponse = { pos: posListState };
      return {
        data,
        isLoading: false,
        isError: false,
    ...
    useLogisticsSuppliers: (): { data: SuppliersListResponse } => ({
      data: { suppliers: [SUPPLIER_HOOKKA] },
    }),
    useLogisticsWarehouse: (): { data: WarehouseListResponse } => ({
      data: {
...
function makeSofaPo(
  overrides: Partial<LogisticsPoListRow> = {},
): LogisticsPoListRow {
  return {
    id: "PO-SOFA-001",
