...
/**
 * LogisticsMovements — covers the M5 Task 5 plan list (~15 tests).
 *
 * Same vi.mock(@/lib/queries) pattern as LogisticsWarehouse.test.tsx — each
 * test sets the mocked hook return states before rendering. The
 * `useLogisticsMovements` mock captures the filter argument so we can assert
...
vi.mock("@/lib/queries", async () => {
  ...
  return {
    ...actual,
    useLogisticsMovements: (filters: MovementsFilters) => {
...
describe("LogisticsMovements page", () => {
  ...
  it("15. loading state shows the movements-skeleton; error state shows Retry that calls refetch", () => {
    ...
    const { rerender } = render(wrap(<LogisticsMovements />));
    ...
    rerender(wrap(<LogisticsMovements />));
  ...
  it("19. breadcrumb '← Warehouse' calls setTab('warehouse')", () => {
    setLoaded([]);
    const setTab = vi.fn();
    render(wrap(<LogisticsMovements setTab={setTab} />));
