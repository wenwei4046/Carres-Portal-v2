...
const INVOICE: InvoiceRegisterRow = {
  ...
  orders: {
    ...
    // Loaded, and empty — which is NOT the same as never loaded (0448).
    order_payments: [],
  },
};
...
describe("§5 — a likely duplicate must be inspected before the money is recorded", () => {
  ...
  it("names the earlier payment and keeps the door shut until it is inspected", async () => {
    ...
    expect(warn).toHaveTextContent("This customer already has a payment that looks the same.");
    expect(warn).toHaveTextContent("Only a Payment Approver can record it after this.");
...
describe("§5 (0448) — the tick asks, the server decides", () => {
  function showWith(inv: typeof INVOICE) {
    const qc = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    render(
      <QueryClientProvider client={qc}>
   ...
   *  list at all, the old page compared against nothing, found nothing, and
   *  looked exactly like a clean order. */
  it("says so, and shuts the door, when the earlier payments could not be read", async () => {
    const unknown = { ...INVOICE, orders: { ...INVOICE.orders!, order_payments: undefined } } as typeof INVOICE;
    showWith(unknown);
    fireEvent.change(screen.getByLabelText("Transfer slip"),
      { target: { files: [new File(["slip"], "slip.jpg", { type: "image/jpeg" })] } });
    fireEvent.click(screen.getByRole("button", { name: "Review payment" }));
  ...
  it("sends the acknowledgement to the server, which is what actually decides", async () => {
    const withEarlier = {
      ...INVOICE,
      orders: {
        ...INVOICE.orders!,
        order_payments: [{
          id: "p-old", receipt_no: "RC-080926-0001", amount: 500,
          paid_on: "2026-09-08", voided_at: null, reference: null, method: "bank",
        }],
      },
    } as typeof INVOICE;
    showWith(withEarlier);
    ...
    expect(state.mutate.mock.calls[0][0]).toMatchObject({ duplicateAck: true });
  });

  it("an ordinary payment carries no acknowledgement at all", async () => {
    showWith(INVOICE);
    fireEvent.change(screen.getByLabelText("Payment amount"), { target: { value: "123" } });
    fireEvent.change(screen.getByLabelText("Transfer slip"),
      { target: { files: [new File(["slip"], "slip.jpg", { type: "image/jpeg" })] } });
    ...
    fireEvent.click(screen.getByRole("button", { name: "Record payment" }));
    await waitFor(() => expect(state.mutate).toHaveBeenCalled());
    expect(state.mutate.mock.calls[0][0].duplicateAck).toBeUndefined();
  });
});
