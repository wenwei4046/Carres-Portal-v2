...
describe("POST /api/principal/dealers/:id/terms", () => {
  it("200 — updates credit limit + payment terms, returns dealer", async () => {
    const { sb, rpcCalls } = buildSb({
      rpcResults: {
        dealer_set_terms: {
          data: {
    ...
    const jwt = await makeJwt("principal");
    const res = await app.fetch(
      new Request(`http://t/api/principal/dealers/${DEALER_ID}/terms`, {
        method: "POST",
        headers: { Authorization: `Bearer ${jwt}`, "Content-Type": "application/json" },
        body: JSON.stringify({ creditLimit: 75000, paymentTerms: "NET 60" }),
      }),
      env,
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as { dealer: { credit_limit: number; payment_terms: string } };
    expect(body.dealer.credit_limit).toBe(75000);
    expect(body.dealer.payment_terms).toBe("NET 60");
    expect(rpcCalls[0]?.name).toBe("dealer_set_terms");
    expect(rpcCalls[0]?.args).toEqual({
      p_dealer_id: DEALER_ID,
      p_credit_limit: 75000,
      p_payment_terms: "NET 60",
    });
  });

  it("422 — invalid paymentTerms rejected by zod", async () => {
    const { sb, rpcCalls } = buildSb({});
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    vi.mocked(userClient).mockReturnValue(sb as any);
    ...
    const res = await app.fetch(
      new Request(`http://t/api/principal/dealers/${DEALER_ID}/terms`, {
        method: "POST",
        headers: { Authorization: `Bearer ${jwt}`, "Content-Type": "application/json" },
        body: JSON.stringify({ creditLimit: 50000, paymentTerms: "NET 90" }),
      }),
      env,
    );
    expect(res.status).toBe(422);
