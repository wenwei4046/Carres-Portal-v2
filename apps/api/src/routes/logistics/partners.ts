 ...
 * panel beneath.
 *
 * Auth: logistics-only via inline guard (mirrors orders.ts pattern). RLS
 * `partners_read` already lets HQ roles SELECT (0002_rls.sql:157), so the
 * user JWT is enough — no service_role.
 */
const logisticsPartnersRouter = new Hono<AppEnv>();

logisticsPartnersRouter.use("*", async (c, next) => {
...
logisticsPartnersRouter.get("/", async (c) => {
...
export default logisticsPartnersRouter;
