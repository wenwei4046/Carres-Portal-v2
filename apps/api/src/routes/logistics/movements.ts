 ...
 *
 * Pattern: matches sibling logistics/orders.ts list endpoint (zod safeParse on
 * c.req.query() + supabase chain).
 */
const logisticsMovementsRouter = new Hono<AppEnv>();

// Inline logistics-only guard — fast 403 before any Supabase round-trip.
logisticsMovementsRouter.use("*", async (c, next) => {
...
logisticsMovementsRouter.get("/", async (c) => {
...
export default logisticsMovementsRouter;
