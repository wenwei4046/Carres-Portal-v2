...
describe("GET /api/logistics/movements", () => {
  ...
  it("returns 200 with rows + limit on default filters (period=30d cutoff applied)", async () => {
    ...
    const res = await app.fetch(
      new Request("http://t/api/logistics/movements", {
  ...
  it("filters by warehouseId when query param provided", async () => {
    ...
    const jwt = await makeJwt("logistics");
    await app.fetch(
      new Request(`http://t/api/logistics/movements?warehouseId=${wh}`, {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("filters by category=mattress as SKU prefix `mattress:%`", async () => {
    ...
    const jwt = await makeJwt("logistics");
    await app.fetch(
      new Request("http://t/api/logistics/movements?category=mattress", {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("filters by category=bedframe / sofa as SKU prefixes", async () => {
    {
      ...
      const jwt = await makeJwt("logistics");
      await app.fetch(
        new Request("http://t/api/logistics/movements?category=bedframe", {
          headers: { Authorization: `Bearer ${jwt}` },
        }),
        env,
      );
    ...
    }
    {
      ...
      const jwt = await makeJwt("logistics");
      await app.fetch(
        new Request("http://t/api/logistics/movements?category=sofa", {
          headers: { Authorization: `Bearer ${jwt}` },
        }),
        env,
      );
  ...
  it("filters by sku partial match (ilike)", async () => {
    ...
    const jwt = await makeJwt("logistics");
    await app.fetch(
      new Request("http://t/api/logistics/movements?sku=carres-cloud", {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("filters by kind=in (and kind=out)", async () => {
    {
      ...
      const jwt = await makeJwt("logistics");
      await app.fetch(
        new Request("http://t/api/logistics/movements?kind=in", {
          headers: { Authorization: `Bearer ${jwt}` },
        }),
        env,
      );
    ...
    }
    {
      ...
      const jwt = await makeJwt("logistics");
      await app.fetch(
        new Request("http://t/api/logistics/movements?kind=out", {
          headers: { Authorization: `Bearer ${jwt}` },
        }),
        env,
      );
  ...
  it("filters search across ref + note via .or() with ilike interpolation", async () => {
    ...
    const jwt = await makeJwt("logistics");
    await app.fetch(
      new Request("http://t/api/logistics/movements?search=DO-77310", {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("period=7d sets .gte(occurred_at, ~7 days ago)", async () => {
    ...
    const jwt = await makeJwt("logistics");
    await app.fetch(
      new Request("http://t/api/logistics/movements?period=7d", {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("period=all skips occurred_at filter entirely (no .gte / .lt called)", async () => {
    ...
    const jwt = await makeJwt("logistics");
    const res = await app.fetch(
      new Request("http://t/api/logistics/movements?period=all", {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("period=custom with from + to applies .gte(from) AND .lt(to)", async () => {
    ...
    const res = await app.fetch(
      new Request(
        `http://t/api/logistics/movements?period=custom&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,
        { headers: { Authorization: `Bearer ${jwt}` } },
      ),
      env,
    );
  ...
  it("period=custom missing from → 422 invalid_query (no Supabase round-trip)", async () => {
    ...
    const jwt = await makeJwt("logistics");
    const res = await app.fetch(
      new Request("http://t/api/logistics/movements?period=custom&to=2026-05-01T00:00:00Z", {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("invalid period value → 422 zod rejection", async () => {
    ...
    const jwt = await makeJwt("logistics");
    const res = await app.fetch(
      new Request("http://t/api/logistics/movements?period=bogus", {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("search regex rejects shell/sql metacharacters → 422", async () => {
    ...
    const res = await app.fetch(
      new Request(
        ...
        `http://t/api/logistics/movements?search=${encodeURIComponent("%' OR 1=1 --")}`,
        { headers: { Authorization: `Bearer ${jwt}` } },
      ),
      env,
    );
  ...
  it("LIMIT 200 always applied regardless of filters", async () => {
    ...
    const jwt = await makeJwt("logistics");
    await app.fetch(
      new Request("http://t/api/logistics/movements?period=all&kind=in", {
        headers: { Authorization: `Bearer ${jwt}` },
      }),
      env,
    );
  ...
  it("returns 401 without Authorization header", async () => {
    const res = await app.fetch(new Request("http://t/api/logistics/movements"), env);
