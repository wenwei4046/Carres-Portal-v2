 ...
 *
 * Two halves: what SQL knows (`commission_run_state`: existing run, unattributed
 * count, pending adjustments) and what only the engine knows (does everybody who
 * sold actually compute to a figure). Combined here by the shared pure function.
 */
hrRunsRouter.get("/state", requireHr, async (c) => {
  ...
  const checks: ReadinessCheck[] = commissionReadiness({
    report,
    unattributed: dbState.unattributed,
