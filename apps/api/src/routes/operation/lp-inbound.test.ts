...
describe("POST /api/operation/pos/:id/lp-accept-inbound", () => {
  it("calls lp_accept_inbound_delivery RPC for operation caller (代按)", async () => {
    const sb = {
    ...
    const res = await app.fetch(
      new Request("http://t/api/operation/pos/PO-100/lp-accept-inbound", {
        method: "POST",
  ...
  it("admits principal role (carry-forward route-mount-middleware-leak fix)", async () => {
    ...
    // here is ["operation","principal","partner"]; the post-fix per-route
    // requireOperation guard on operationPosRouter no longer leaks.
...
describe("POST /api/operation/pos/:id/lp-reject-inbound", () => {
  it("calls partner_reject_customer with empty p_reason default", async () => {
    ...
    const jwt = await makeJwt("operation");
    const res = await app.fetch(
      new Request("http://t/api/operation/pos/PO-100/lp-reject-inbound", {
...
describe("POST /api/operation/pos/:id/relocate-inbound", () => {
  it("calls operation_relocate_warehouse RPC", async () => {
    const sb = { rpc: vi.fn().mockResolvedValue({ data: { po_id: "PO-100", new_warehouse_id: "00000000-0000-0000-0000-0000000000a2" }, error: null }) };
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    vi.mocked(userClient).mockReturnValue(sb as any);
    const jwt = await makeJwt("operation");
    const res = await app.fetch(
      new Request("http://t/api/operation/pos/PO-100/relocate-inbound", {
        method: "POST", headers: { Authorization: `Bearer ${jwt}`, "Content-Type": "application/json" },
        body: JSON.stringify({ new_warehouse_id: "00000000-0000-0000-0000-0000000000a2" }),
      }), env,
    );
    expect(res.status).toBe(200);
    expect(sb.rpc).toHaveBeenCalledWith("operation_relocate_warehouse", {
      p_po_id: "PO-100",
