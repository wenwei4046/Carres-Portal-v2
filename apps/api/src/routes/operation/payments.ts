...
paymentsRouter.get("/", async (c) => {
  ...
  const { data, error } = await sb
    ...
    .select(
      "id, so, status, operation_stage, customer_name, customer_address, delivery_date, delivery_date_tbd, delivered_at, source_ref, order_lines(sku, qty), order_payments(amount, kind), ops_order_control(balance, payment_status, storage_from, storage_fee_override, balance_due_date, storage_collected_at, storage_waiver_status, extension_original_date)",
    )
