 ...
 *   POST /po-pay            RETIRED by 0477 — answers 410; suppliers are
 *   POST /po-schedule       paid with a Payment Voucher
...
// 0477 — ONE door pays a supplier: the Payment Voucher. These two routes wrote
// a payment and flipped purchase_orders.pay_status with no ledger entry; the
// database functions behind them are revoked from every signed-in user. They
// answer 410 Gone so an old tab or script learns where the door moved.
const RETIRED_PO_PAY = {
  error: "gone",
  code: "use_payment_voucher",
  message:
    "Suppliers are paid with a Payment Voucher now. Open Finance → Payment Vouchers to pay this supplier's bills.",
  path: "/finance/payment-vouchers",
};
financePaymentsRouter.post("/po-pay", requireFinance, (c) => c.json(RETIRED_PO_PAY, 410));
financePaymentsRouter.post("/po-schedule", requireFinance, (c) => c.json(RETIRED_PO_PAY, 410));
