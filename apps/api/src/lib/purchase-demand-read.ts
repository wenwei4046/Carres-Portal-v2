...
export async function readFreeStock(sb: ReturnType<typeof userClient>): Promise<{
  ...
  stockQtyById: Map<string, number>;
  /** 0469 — the same units, with the facts a Ready Stock table has to print. */
  ...
  try {
    ...
    /**
     * ── THE AUTHORITATIVE INVENTORY READ (0469) ───────────────────────────
     *
...
export async function loadToOrder(
   ...
   *              as the answer for pre-0469 rows only, keyed as it always was.
   *
  ...
  for (const l of demand) {
    ...
    /* 0469 — the EXACT binding first. This line's own units, not a share of
