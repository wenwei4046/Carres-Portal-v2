...
/* ─── rpc mock — stubs pwp_claim_code + pwp_release_codes ─────────────────────
 * `claimResponses` is consumed IN ORDER per pwp_claim_code call: a `row` (the
 * claimed pwp_codes row) → success, `null` → not-claimable (NULL), or an `error`
 * → server_error. Every rpc call is recorded so a test can assert the release
 * rollback fired with the right codes. */
interface MockOpts {
...
/** A reward line carrying a P8c voucher claim on attrs.pwp. */
const codedLine = (over: { sku?: string; code: string; ruleId?: string; claimGroup?: string; unitPrice?: number }) =>
  line({
    sku: over.sku ?? "BED-1",
...
describe("claimPwpCodesForLines", () => {
  ...
  it("a valid claim → stamps USED via pwp_claim_code (rule + claimGroup asserted) + returns the ledger", async () => {
    ...
    expect(r.status).toBe("ok");
    if (r.status !== "ok") return;
    expect(r.claimed).toEqual([{ code: "PWP-1111AAAA" }]);
