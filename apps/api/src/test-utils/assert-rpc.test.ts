 ...
 *
 * Why these tests matter: the helper itself runs across ~14 operation test sites
...
describe("assertRpcCallShape", () => {
  it("happy path: matching keys passes without throwing", () => {
    const rpc = vi.fn();
    rpc("operation_warehouse_pick", { p_order_id: "o1", p_warehouse_id: "w1" });
    expect(() =>
  ...
  it("throws when a key is missing from the actual call", () => {
    const rpc = vi.fn();
    rpc("operation_attach_do_and_deliver", {
      p_order_id: "o1",
      p_do_number: "DO-1",
      // p_signed missing
    });
  ...
  it("zero-arg RPC with expectedArgKeys [] passes (call[1] is undefined)", () => {
    const rpc = vi.fn();
    rpc("operation_dashboard_summary"); // no second arg
    expect(() =>
      assertRpcCallShape(rpc, "operation_dashboard_summary", []),
    ).not.toThrow();
  });
  ...
  it("zero-arg RPC fails when expectedArgKeys is non-empty", () => {
    ...
    rpc("operation_dashboard_summary"); // no second arg
    expect(() =>
      assertRpcCallShape(rpc, "operation_dashboard_summary", ["p_order_id"]),
    ).toThrow();
  });

  it("checks every matching call when the same fn is invoked multiple times", () => {
    const rpc = vi.fn();
    rpc("operation_pick_warehouse", { p_order_id: "o1" });
    // Second call has the wrong shape — helper must catch it even though the first
    // call was fine.
    rpc("operation_pick_warehouse", { p_order_id: "o2", p_extra: "leak" });
    expect(() =>
