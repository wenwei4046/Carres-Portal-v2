-- =============================================================================
-- 0588_a_unit_problem_is_reported_through_the_issue_door.sql
...
comment on column public.ops_stock_items.sale_cleared_at is
  '0588: when an authorised person made a Damaged Unit available for clearance sale (owner ruling 2026-09-25). Null = not cleared; a hold or repair clears it again.';
...
comment on function public.unit_availability(text, boolean, text, text, boolean) is
  '0588 — THE availability arithmetic: status, repair flag, protection reason, condition AND the clearance fact. Nothing may re-derive this from a status alone.';
...
comment on function public.stock_unit_report_problem(uuid, uuid, text, jsonb, jsonb, jsonb, jsonb) is
  '0588: Report a problem on a Unit — records the Issue through issue_record_issue and applies the derived protective control in one transaction.';
...
comment on function public.stock_unit_make_available(uuid) is
  '0588: Make available for sale — refused while a problem is open, in repair, in transit or without a Site; resolves a claimless hold and clears a damaged Unit for clearance sale.';
