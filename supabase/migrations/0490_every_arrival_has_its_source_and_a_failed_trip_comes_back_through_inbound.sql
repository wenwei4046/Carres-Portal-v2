-- 0490 · Every arrival has its source, and a failed trip's goods come back through Inbound
-- 【DELIVERY】 CARD 14 · Journey legs and returned-goods handling — lands the Stock-owned
-- arrival-source draft (`supabase/drafts/arrival_sources_and_receiving.sql`, owed since
-- docs/stock/MASTER.md §13.9) as the numbered migration, with ONE addition for Delivery
-- MASTER §7: a `failed-delivery-return` may be bound to the Delivery Visit that failed
-- (`attempt_id`) instead of a Service Case, so returned goods enter Inbound as
-- `Check required` from the delivery result itself — never Ready Stock, never a
-- Logistics report substituting for the Warehouse's actual receipt (§4).
--
-- Every object is guarded with `if not exists` / `drop … if exists`; the draft ran
-- inside `begin … commit`, which the governed apply path supplies itself.
set search_path = public, pg_temp;

create table if not exists arrival_sources (
 id uuid primary key,
 ...
 -- 0490: the Delivery Visit whose failure sends the goods back (Delivery MASTER §7).
 attempt_id uuid references delivery_attempts(id),
 case_approval jsonb,
 ...
 check(kind <> 'customer-return' or case_id is not null),
 check(kind <> 'failed-delivery-return' or case_id is not null or attempt_id is not null),
 check(collection_date is null or collection_date <= expected_date),
 check(cancelled_at is null or length(btrim(cancel_reason))>0)
);
create table if not exists arrival_source_units (
...
create table if not exists arrival_source_events (
...
create index if not exists arrival_source_units_item on arrival_source_units(stock_item_id);
create index if not exists arrival_source_events_source on arrival_source_events(source_id,recorded_at,id);
...
drop policy if exists arrival_sources_read on arrival_sources;
...
drop policy if exists arrival_source_units_read on arrival_source_units;
...
drop policy if exists arrival_source_events_read on arrival_source_events;
...
drop policy if exists arrival_proofs_read on storage.objects;
...
drop policy if exists arrival_proofs_upload on storage.objects;
...
alter table warehouse_receipts add column if not exists arrival_source_id uuid references arrival_sources(id);
alter table warehouse_receipts drop constraint if exists receipt_has_one_source;
alter table warehouse_receipts add constraint receipt_has_one_source check ((po_id is null) <> (arrival_source_id is null));
create index if not exists warehouse_receipts_arrival_source on warehouse_receipts(arrival_source_id);

create or replace function arrival_source_gate() returns void language plpgsql security definer set search_path=public,pg_temp as $$
begin
...
create or replace function arrival_source_create(p_input jsonb) returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare
...
create or replace function arrival_source_handover(p_id uuid,p_input jsonb) returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
...
create or replace function arrival_source_plan(p_id uuid,p_input jsonb,p_cancel boolean default false) returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
...
create or replace function receiving_arrival_post(p_source_id uuid,p_input jsonb) returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
...
create or replace function receiving_arrival_void(p_receipt_id uuid,p_reason text) returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
