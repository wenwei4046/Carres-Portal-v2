-- =============================================================================
-- 0460_one_gate_writes_the_ledger.sql
-- FINANCE · GENERAL LEDGER — THE POSTING GATE
-- (build contract .claude/LEDGER-CONTRACT.md; rulings J/K/L/M/N)
--
-- 0459 gave the ledger a chart of accounts and a start line. This file gives it
...
begin
  ...
  if v <> 4 then
    raise exception '0460 sanity: expected the four ledger functions, got %', v;
  end if;

  if not exists (select 1 from pg_indexes
                  where schemaname = 'public' and indexname = 'gl_entries_one_active_per_source') then
    raise exception '0460 sanity: the one-active-per-source index is missing';
  end if;
  ...
  if has_table_privilege('authenticated', 'public.gl_entries', 'insert')
    ...
    raise exception '0460 sanity: a direct write door to the ledger survived';
  end if;

  if exists (select 1 from pg_policy
              where polrelid in ('public.gl_entries'::regclass, 'public.gl_entry_lines'::regclass)
                and polcmd <> 'r') then
    raise exception '0460 sanity: a non-SELECT policy exists on a ledger table';
  end if;

  if has_function_privilege('anon', 'public.gl_post(text,text,date,text,jsonb)', 'execute') then
    raise exception '0460 sanity: gl_post is callable by anon';
  end if;

  raise notice '0460 OK: one gate writes the ledger, and nothing else can';
end $sanity$;
