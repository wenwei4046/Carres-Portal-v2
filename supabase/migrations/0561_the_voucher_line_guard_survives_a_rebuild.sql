-- 0561 . THE VOUCHER LINE GUARD SURVIVES A REBUILD
--
-- A voucher line must say what it is for. 0536 wrote that guard, 0540 dropped
-- it while adding departments, 0552 put it back, and 0554 dropped it AGAIN
-- while replacing hardcoded account numbers: it rebuilt this function from a
-- body that predated 0552. Same function, same guard, third time.
--
-- This migration is 0554's own definition, unchanged except that the guard is
-- back in the line loop. Nothing else moves: no rate, no account, no grant,
-- no RLS policy, no row. `create or replace` keeps the existing ACL and the
-- signature is identical, so no grant is restated.
--
...
begin
  ...
  for v_line in select value from jsonb_array_elements(v_lines) loop
    ...
    -- 0536, restored by 0552, restored again by 0561: a line says what it is for.
    -- 0552 put this back after 0540 dropped it; 0554 then rebuilt this function
    -- to stop naming an account number as a literal and carried forward a body
    -- that predated 0552, dropping it a second time. The question is whether
    -- there is one character here that is not whitespace: a tab, a newline, a
    -- carriage return, a form feed and a non-breaking space are all refused,
    -- and '', a missing key and a JSON null all arrive here as ''.
...
-- character only and lets a single tab through.
do $sanity$
declare
  v_src text;
begin
  select prosrc into v_src
   ...
   where n.nspname = 'public' and p.proname = 'payment_voucher_save_draft';
  if v_src is null then
    raise exception '0561 sanity: payment_voucher_save_draft is not defined';
  end if;
  if position('line_needs_description' in v_src) = 0 then
    raise exception '0561 sanity: the voucher line guard is missing';
  end if;
  if position('!~ ''[^[:space:]]''' in v_src) = 0 then
    raise exception '0561 sanity: the guard is not the non-whitespace test';
  end if;
end
$sanity$;
