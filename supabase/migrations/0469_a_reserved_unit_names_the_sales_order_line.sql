-- 0469_a_reserved_unit_names_the_sales_order_line.sql
--
...
comment on column public.ops_stock_items.reserved_order_line_id is
  '0469: the Sales Order ITEM LINE this unit is reserved (and later sold) against. Written only by ops_stock_pool_draw with a validated line; cleared by ops_stock_release and ops_stock_reassign. `reserved_ref` names the order, this names the line — a Sales Order with two lines of one SKU needs both.';
...
comment on function public.stock_match_key(text) is
  '0469 — the SQL half of the portal''s ONE rule for linking `order_lines.sku` (catalog vocabulary) to `ops_stock_items.sku` (the warehouse''s own names). Kept identical to `stockMatchKey` in packages/shared by a contract test over the real SKU corpus. Zero collisions across 261 catalog SKUs, measured 2026-09-10.';
...
comment on function public.so_line_remaining_requirement(uuid, uuid) is
  '0469 — what a Sales Order item line still needs: ordered quantity less the Ready Stock Units bound to it (reserved or sold) less its non-cancelled purchase-order lineage. The same expression `soBatchOrderLineOutstandingQty` prints, so the reservation door and the Register cannot disagree.';
...
-- exist. So this migration leaves a DELEGATOR (§4c below) and `0470` removes it
...
begin
  ...
  -- ── 0469 · A CUSTOMER ORDER IS NAMED BY ITS LINE, NEVER BY ITS NUMBER ─────
  --
...
comment on function public.ops_stock_pool_draw(text, text, text, uuid, text, text, uuid) is
  '0469 DEPLOY-WINDOW DELEGATOR — forwards to the 8-argument draw door with no item line, so a browser or Worker still on the pre-0469 bundle keeps working while the deploy lands. It validates nothing of its own. REMOVED BY 0470 once production is verified; do not build on it.';
...
comment on function public.so_batch_reserve_ready_units(text, text, text, jsonb) is
  '0469 — SO Batch Purchase''s `Choose Ready Unit`: N exact Units, each named against the Sales Order ITEM LINE it answers, in ONE transaction. Delegates every guard and every ledger row to `ops_stock_pool_draw`; it adds only atomicity, so a race refuses the whole act by name instead of half-reserving it.';
...
BEGIN
  ...
  UPDATE ops_stock_items
     SET status       = 'free',
         reserved_ref = NULL,
         -- 0469: the line goes with the reference. The customer still owes the
...
BEGIN
  ...
  UPDATE ops_stock_items
     SET reserved_ref = p_new_ref,
         -- 0469: the reference moved without naming a line, so the OLD line's
...
begin
  ...
  -- delegator. 0470 asserts it is back to one.
  select count(*) into v_overloads
    from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'ops_stock_pool_draw';
  if v_overloads <> 2 then
    raise exception '0469 sanity: expected the door and its delegator, found %', v_overloads;
  end if;
