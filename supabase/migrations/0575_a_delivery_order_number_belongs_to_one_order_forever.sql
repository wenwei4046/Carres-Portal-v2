...
-- ── D. the proof ────────────────────────────────────────────────────────────
--
-- Catalogue reads only — pg_proc, pg_constraint, pg_class. No row is read or
-- written and no production row count is asserted (CLAUDE.md red line 8). The
-- BEHAVIOUR is proved separately, on a real Postgres running this whole chain:
-- `apps/api/src/test/delivery-order-number-0575.integration.test.ts`, nine
-- cases, which were run against a replay stopped BEFORE this file (the second
-- order was allowed to wear the first order's number) and again after it.
do $sanity$
declare
  v_src text;
  v_n   int;
  m     text;
begin
  -- ① THE ALLOCATOR AND ITS DOOR EXIST, AND ONLY ONE OF EACH.
  if to_regprocedure('public._delivery_document_number_draw(uuid)') is null then
    raise exception '0575 sanity: the draw is missing';
  end if;
  if to_regprocedure('public.delivery_document_number_draw(uuid)') is null then
    raise exception '0575 sanity: the gated door is missing';
  end if;
  select count(*) into v_n from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname in ('_delivery_document_number_draw', 'delivery_document_number_draw');
  if v_n <> 2 then
    raise exception '0575 sanity: expected exactly two number functions, found %', v_n;
  end if;

  -- ② THE POOL KEEPS ITS SHAPE RULES — the width is per series and fixed.
  if not exists (
    select 1 from pg_constraint
     where conrelid = 'public.delivery_document_numbers'::regclass
       and conname = 'delivery_document_numbers_shape'
  ) then
    raise exception '0575 sanity: the pool lost its DO/SDO shape rule';
  end if;
  if not exists (
    select 1 from pg_constraint
     where conrelid = 'public.delivery_document_numbers'::regclass
       and conname = 'delivery_document_numbers_period'
  ) then
    raise exception '0575 sanity: the pool lost the rule tying the number to its period';
  end if;
  if not (select relrowsecurity from pg_class where oid = 'public.delivery_document_numbers'::regclass) then
    raise exception '0575 sanity: the pool table has RLS disabled';
  end if;

  -- ③ THE DRAW KEEPS BOTH SERIES, THE REDRAW AND THE REFUSAL.
  select p.prosrc into v_src from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = '_delivery_document_number_draw';
  foreach m in array array['SDO', 'rental', 'delivery_order_numbers_used_up', 'on conflict', 'random'] loop
    if position(m in v_src) = 0 then
      raise exception '0575 sanity: the draw lost %', m;
    end if;
  end loop;

  -- ④ THE MATERIALISER REFUSES INSTEAD OF SKIPPING — the P0 itself.
  select p.prosrc into v_src from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'ops_delivery_orders_materialise';
  if position('delivery_order_number_taken' in v_src) = 0 then
    raise exception '0575 sanity: the materialiser can still take another order''s number silently';
  end if;
  if position('v_owner = new.id' in v_src) = 0 then
    raise exception '0575 sanity: the materialiser lost the same-order no-op the leg and trip doors rely on';
  end if;

  -- ⑤ AND THE LEGACY DISPATCH BACKSTOP NO LONGER INVENTS A NUMBER FROM THE SO.
  select p.prosrc into v_src from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'orders_auto_issue_on_dispatched';
  if position('_delivery_document_number_draw' in v_src) = 0 then
    raise exception '0575 sanity: the dispatch backstop does not draw from the pool';
  end if;
  if position('lpad(new.so::text, 6' in v_src) > 0 then
    raise exception '0575 sanity: the dispatch backstop still invents DO-<SO>';
  end if;
end
$sanity$;
