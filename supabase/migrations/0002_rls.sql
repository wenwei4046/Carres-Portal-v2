...
create or replace function public.app_dealer_id()
returns uuid
language sql stable security definer as $$
  select dealer_id from public.app_users where id = auth.uid()
$$;

create or replace function public.app_supplier_id()
returns uuid
language sql stable security definer as $$
...
-- -----------------------------------------------------------------------------
-- Note: the original reference defines a trigger on auth.users that auto-mirrors
-- new auth users into app_users. The Supabase MCP user lacks CREATE TRIGGER
-- privileges on schema auth, so we skip it here. Phase 1 seeds app_users
-- explicitly (see seed.sql). For self-signup → app_users mirror later, use a
-- Supabase Auth Hook (same pattern as 0004_auth_hook.sql).
-- -----------------------------------------------------------------------------
...
create policy catalog_write_internal on product_models  for all    using ((select public.is_internal())) with check ((select public.is_internal()));

create policy skus_read_all          on product_skus    for select using ((select auth.uid()) is not null);
create policy skus_write_internal    on product_skus    for all    using ((select public.is_internal())) with check ((select public.is_internal()));

create policy fabrics_read_all       on sofa_fabrics    for select using ((select auth.uid()) is not null);
create policy fabrics_write_internal on sofa_fabrics    for all    using ((select public.is_internal())) with check ((select public.is_internal()));

create policy addons_read_all        on addons          for select using ((select auth.uid()) is not null);
create policy addons_write_internal  on addons          for all    using ((select public.is_internal())) with check ((select public.is_internal()));

create policy floor_read_all         on floor_config    for select using ((select auth.uid()) is not null);
create policy floor_write_principal  on floor_config    for all    using ((select public.is_principal())) with check ((select public.is_principal()));

create policy warehouses_read_all       on warehouses   for select using ((select auth.uid()) is not null);
create policy warehouses_write_internal on warehouses   for all    using ((select public.is_internal())) with check ((select public.is_internal()));

-- app_users: self-read; principal full access
create policy app_users_self_read       on app_users for select using (id = (select auth.uid()) or (select public.is_principal()));
create policy app_users_principal_write on app_users for all    using ((select public.is_principal())) with check ((select public.is_principal()));

-- Dealers · outlets · salespersons
create policy dealers_internal_read on dealers for select using (
  (select public.is_internal()) or id = (select public.app_dealer_id())
);
create policy dealers_principal_write on dealers for all using ((select public.is_principal())) with check ((select public.is_principal()));

create policy outlets_scoped_read on outlets for select using (
  (select public.is_internal()) or dealer_id = (select public.app_dealer_id())
);
create policy outlets_dealer_write on outlets for all using (
  (select public.is_principal()) or dealer_id = (select public.app_dealer_id())
) with check (
  (select public.is_principal()) or dealer_id = (select public.app_dealer_id())
);
...
create policy suppliers_read on suppliers for select using (
  (select public.is_internal()) or id = (select public.app_supplier_id())
);
create policy suppliers_principal_write on suppliers for all using ((select public.is_principal())) with check ((select public.is_principal()));

create policy partners_read on delivery_partners for select using (
  (select public.is_internal()) or id = (select public.app_partner_id())
);
create policy partners_principal_write on delivery_partners for all using ((select public.is_principal())) with check ((select public.is_principal()));

create policy fleet_read on partner_fleet for select using (
  (select public.is_internal()) or partner_id = (select public.app_partner_id())
);
create policy fleet_partner_write on partner_fleet for all using (
  (select public.is_principal()) or partner_id = (select public.app_partner_id())
) with check (
  (select public.is_principal()) or partner_id = (select public.app_partner_id())
);

-- Stock
create policy stock_balances_read on stock_balances for select using ((select public.is_internal()));
...
create policy stock_movements_read on stock_movements for select using ((select public.is_internal()));
...
create policy orders_dealer_insert on orders for insert with check (
  ...
  or (select public.app_role()) in ('logistics','finance','bd')
  or dealer_id = (select public.app_dealer_id())
);
...
create policy po_scoped_read on purchase_orders for select using (
  (select public.is_internal())
  or supplier_id  = (select public.app_supplier_id())
  or delivery_partner_id = (select public.app_partner_id())
);
...
create policy po_scoped_update on purchase_orders for update using (
  (select public.is_principal())
  or (select public.app_role()) in ('logistics','finance')
  or supplier_id = (select public.app_supplier_id())
  or delivery_partner_id = (select public.app_partner_id())
);
...
create policy payments_scoped_read on payments for select using (
  (select public.is_internal())
  or exists (select 1 from orders o where o.id = payments.order_id and o.dealer_id = (select public.app_dealer_id()))
);
create policy payments_write_finance on payments for all using (
  (select public.app_role()) in ('finance','principal')
) with check (
  (select public.app_role()) in ('finance','principal')
);
...
create policy invoices_scoped_read on invoices for select using (
  (select public.is_internal())
  or exists (select 1 from orders o where o.id = invoices.order_id and o.dealer_id = (select public.app_dealer_id()))
);
...
create policy refunds_scoped_read on refunds for select using (
  (select public.is_internal())
  or exists (select 1 from orders o where o.id = refunds.order_id and o.dealer_id = (select public.app_dealer_id()))
);
...
create policy approvals_internal_read    on approvals for select using ((select public.is_internal()));
create policy approvals_internal_insert  on approvals for insert with check ((select public.is_internal()));
create policy approvals_principal_decide on approvals for update using ((select public.is_principal())) with check ((select public.is_principal()));

create policy audit_internal_read on audit_log for select using ((select public.is_internal()));
create policy audit_self_insert   on audit_log for insert with check ((select auth.uid()) is not null);

create policy inquiries_internal on inquiries for all using ((select public.is_internal())) with check ((select public.is_internal()));

create policy bank_lines_finance on bank_statement_lines for all using ((select public.app_role()) in ('finance','principal')) with check ((select public.app_role()) in ('finance','principal'));

create policy announcements_read            on announcements for select using ((select auth.uid()) is not null);
create policy announcements_principal_write on announcements for all using ((select public.is_principal())) with check ((select public.is_principal()));
