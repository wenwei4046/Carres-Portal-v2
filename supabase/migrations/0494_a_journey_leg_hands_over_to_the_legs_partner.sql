-- 0494 · A Journey leg hands over to the LEG's partner
-- 【DELIVERY】 CARD 14 · Journey legs — production-walk correction 2026-09-13
--
-- Found on the authenticated walk of SO-1362 (two legs, NETS then AL): the
-- handover door (0424/0440) resolved the goods-holder from the arrangement at
-- leg 0, so a leg document's `handed_over` refused with
-- `partner_holder_not_recorded` even though leg 1's arrangement named NETS.
-- The document carries its own scope since 0491 (`ops_delivery_orders.leg`);
-- the holder is the LEG's partner. Nothing else in the door changes; a
-- whole-order document (leg 0) resolves exactly as before.
