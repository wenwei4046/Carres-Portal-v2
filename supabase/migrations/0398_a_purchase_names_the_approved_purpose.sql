-- ---------------------------------------------------------------------------
-- 0398 · A purchase names the approved purpose it is for
-- (PURCHASING — CARD 03 · Manual Purchase left filter rail, Jess 2026-08-28)
--
-- The owner ruled the Manual Purchase purpose vocabulary: exactly
--
--   ready_stock              Ready Stock
--   showroom_display         Showroom Display
--   service_case             Service Case
--   internal_staff_purchase  Internal Staff Purchase   (Management included —
--                                                       there is no Management
--                                                       Purchase)
--   subsidiary_purchase      Subsidiary Purchase
--
-- NOTHING IS RELABELLED. The four pre-ruling values — display · warranty ·
-- office · spare_parts — are RETIRED, not renamed: an office-supplies buy was
-- never a staff purchase, and claiming old rows meant a word they were never
-- asked as is exactly the false relabelling the Card bans. Existing rows keep
-- their stored value and their original visible word; every DOOR now admits
-- only the approved five, refusing a retired value by the same name an
-- invented one gets (unknown_purpose). The CHECKs hold the union, because a
-- CHECK that forgot history would refuse the very rows it already guards.
--
-- WHAT THIS CHANGES
--   · purchase_requests / purchase_demands / purchasing_purpose_approval /
--     purchase_orders purpose CHECKs — widened to the nine (POs also keep
--     customer_sales).
--   · purchasing_purpose_approval — the four new purposes seeded, approval
--     required (the safe side, 0359's own default).
--   · purchasing_create_request / purchasing_create_demand /
--     purchasing_set_purpose_approval — gates narrowed to the approved five.
--   · purchasing_issue_pos_batch — 0380's body restated whole (a function is
--     replaced whole) with ONLY the purpose gate widened, so an approved
--     request created under the new vocabulary can become a purchase order
--     and a historical PO purpose stays valid.
-- ---------------------------------------------------------------------------
