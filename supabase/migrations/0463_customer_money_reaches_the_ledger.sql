-- =============================================================================
-- 0463_customer_money_reaches_the_ledger.sql
...
-- accounting truth. 0461 gave us a chart and a start line, 0462 gave us the
...
--
-- An earlier draft of this file copied both bodies out of 0351. Because 0463
...
comment on table public.gl_payment_account_map is
  'Payment method/channel -> the debit account customer money lands in (0463). A method with no row is UNMAPPED and its payments REFUSE to post; that is intentional. Never add a catch-all row.';
...
comment on function public.gl_account_for_payment_method(text,text) is
  'The account customer money of this method/channel debits (0463). NULL means unmapped — the caller must refuse, never substitute.';
...
-- ── 4d · seed the map against whatever chart 0461 actually created ───────────
...
begin
  ...
  if v_cash is not null then
    ...
    values ('cash','*', v_cash, 'Cash taken at the counter (0463 seed)')
  ...
  if v_bank is not null then
    insert into gl_payment_account_map(method, source_channel, account_code, note)
    values ('bank','*', v_bank, 'Bank transfer (0463 seed)')
    on conflict do nothing; v_seeded := v_seeded + 1;
    insert into gl_payment_account_map(method, source_channel, account_code, note)
    values ('cheque','*', v_bank, 'Cheque banked (0463 seed) — remap if a cheque-clearing account is added')
    on conflict do nothing; v_seeded := v_seeded + 1;
  end if;
  ...
  if v_card is not null then
    insert into gl_payment_account_map(method, source_channel, account_code, note)
    values ('card','*', v_card, 'Card terminal settlement (0463 seed)')
    on conflict do nothing; v_seeded := v_seeded + 1;
  end if;
  ...
  if v_gate is not null then
    insert into gl_payment_account_map(method, source_channel, account_code, note)
    values ('online','*', v_gate, 'Online gateway (0463 seed)')
    on conflict do nothing; v_seeded := v_seeded + 1;
    insert into gl_payment_account_map(method, source_channel, account_code, note)
    values ('online','stripe_checkout', v_gate, 'Stripe checkout (0463 seed)')
    on conflict do nothing; v_seeded := v_seeded + 1;
  end if;

  raise notice '0463 seed: % mapping row(s) written', v_seeded;
  if v_cash is null then raise warning '0463: payment method ''cash'' is UNMAPPED — cash payments will refuse to post until gl_map_payment_account names an account'; end if;
  if v_bank is null then raise warning '0463: payment methods ''bank'' and ''cheque'' are UNMAPPED — they will refuse to post until gl_map_payment_account names an account'; end if;
  if v_card is null then raise warning '0463: payment method ''card'' is UNMAPPED — deliberately not defaulted to the bank account; map it explicitly'; end if;
  if v_gate is null then raise warning '0463: payment method ''online'' is UNMAPPED — deliberately not defaulted to the bank account; map it explicitly'; end if;
  raise warning '0463: payment method ''other'' is UNMAPPED and stays that way — it is the unrecognised-method bucket and must never carry a default account';
end $seed$;
...
comment on table public.payments is
  'Legacy money table. The FOR ALL write policy was dropped in 0463 — customer receipts belong to order_payments via _customer_payment_post, and nothing in the application writes here through PostgREST.';
...
begin
  ...
  if v < 5 then
    raise exception '0463 sanity: expected the five ledger-bridge functions, got %', v;
  end if;
  ...
  select count(*) into v from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = '_customer_payment_post';
  if v <> 1 then
    raise exception '0463 sanity: % copies of _customer_payment_post', v;
  end if;
  if not exists (
    select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
     where n.nspname = 'public' and p.proname = '_customer_payment_post'
    ...
    raise exception '0463 sanity: the writer was rebuilt from a stale body — 0430''s method dictionary or 0449''s receipt snapshot is missing';
  end if;
  ...
  select count(*) into v from pg_proc p join pg_namespace n on n.oid = p.pronamespace
   where n.nspname = 'public' and p.proname = 'payment_void';
  if v <> 1 then
    raise exception '0463 sanity: % copies of payment_void', v;
  end if;
  ...
  if not exists (
    ...
    raise exception '0463 sanity: payment_void was rebuilt from a stale body — 0430''s required reason/approver gate or 0450''s allocation-following reversal is missing';
  end if;
  ...
  if not exists (select 1 from pg_policy where polrelid = 'public.invoices'::regclass
    ...
    raise exception '0463 sanity: the invoice void door is not UPDATE-only';
  end if;
  if not exists (select 1 from pg_policy where polrelid = 'public.refunds'::regclass
    ...
    raise exception '0463 sanity: the refund door is not INSERT-only';
  end if;
  ...
  if has_table_privilege('authenticated', 'public.payments', 'insert')
    ...
    raise exception '0463 sanity: a money table can still be written or deleted directly';
  end if;

  if has_table_privilege('authenticated', 'public.gl_payment_account_map', 'insert') then
    raise exception '0463 sanity: the payment account map is directly writable';
  end if;

  raise notice '0463 OK: customer money reaches the ledger, and the three legacy write doors are shut';
end $sanity$;
