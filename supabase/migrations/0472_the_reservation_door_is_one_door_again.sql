-- 0472_the_reservation_door_is_one_door_again.sql
--
-- ⭐ PAYING BACK 0471's ONE-DEPLOY DEBT.
--
-- 0471 §4c left a 7-argument `ops_stock_pool_draw` beside the real 8-argument
-- door so that neither the pre-0471 Worker nor the post-0471 one was ever
...
--
-- ⛔ APPLY ONLY AFTER PRODUCTION REPORTS THE 0471 SHA. Applied earlier, every
...
begin
  ...
  if v_overloads <> 1 then
    raise exception '0472 sanity: the draw door must be ONE function, found %', v_overloads;
  end if;
  ...
  if position('p_order_line_id' in v_args) = 0 then
    raise exception '0472 sanity: the surviving door does not name the item line (%)', v_args;
  end if;
  ...
  if not has_function_privilege('authenticated',
    ...
    raise exception '0472 sanity: the draw door lost its grant';
  end if;
end;
$sanity$;
