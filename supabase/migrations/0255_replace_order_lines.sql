-- 0255_replace_order_lines.sql
-- Order line EDIT (Loo 2026-07-25): the POS place lane re-configures an
-- existing item (bed gap/legs/options/specials, sofa compartments) via a
-- pencil on the My-orders drawer. The Hono route re-prices the replacement
-- through the SAME pipeline as add_order_lines (0231/0232, gifts stage
-- skipped); this RPC applies it atomically: delete the old row (or the FULL
-- exploded sofa group) + insert the re-priced replacement rows.
--
-- THE business rule (Loo): edits may only UP-SELL — the replacement total
-- (Σ unit_price × qty) must be ≥ the replaced rows' total, else 22023 with
-- detail 'downsell_blocked'. Enforced HERE (not only in Hono) so no client
-- can drop revenue.
--
-- Guardrail #4 (no silent money edits): order_history carries the full
-- BEFORE image of the replaced rows + old/new totals; audit_log gets
-- 'order.lines_replaced'.
--
-- Free / promo / bundle / combo rows are never editable — their prices are
-- minted by promo/bundle law, not the configurator ('line_not_editable').
-- A sofa group must be replaced WHOLE ('partial_sofa_group').
...
begin
  ...
  -- concurrent add / proceed on the same order (the append-only add RPC
  -- skips the lock; a replace can't afford to).
  ...
  -- Place lane exactly (same gate as add_order_lines 'direct'): status
  -- 'place', operation has NOT picked it up, and not an AutoCount import.
  ...
  -- Old rows: all must belong to THIS order; collect the before-image +
  -- total while checking editability.
  for v_old in
  ...
  loop
    ...
    if (v_old_attrs ? 'free_gift') or (v_old_attrs ? 'free_item')
  ...
  -- build explodes into per-compartment rows.
  ...
  -- validated; cheap defensive re-check). Replacement rows never carry
  -- free/promo markers — the configurator can't mint them.
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    if coalesce(v_line->'attrs', '{}'::jsonb) ? 'free_gift'
       or coalesce(v_line->'attrs', '{}'::jsonb) ? 'free_item'
       or coalesce(v_line->'attrs', '{}'::jsonb) ? 'pwp' then
      raise exception 'free/promo markers are not allowed on replacement lines'
        using errcode = '22023', detail = 'invalid_lines';
    end if;
  ...
  -- (authoritative), new total from the server-priced replacement.
...
REVOKE ALL ON FUNCTION public.replace_order_lines(uuid, uuid[], jsonb, jsonb) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.replace_order_lines(uuid, uuid[], jsonb, jsonb) TO authenticated, service_role;
