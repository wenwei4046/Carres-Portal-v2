 ...
 * (and other supplier-flow specs) advance HoOKkA POs through their state
 * machine; without a reset, re-running the spec hits "no pending POs"
 * because the seed inventory is exhausted.
 *
 * Resets all HoOKkA-supplier POs (supplier_id ='00000000-0000-0000-0000-0000000000e1')
 ...
 * POs assigned to HoOKkA (the test supplier); other suppliers' POs are
...
const SUPPLIER_ID_HOOKKA = "00000000-0000-0000-0000-0000000000e1";
...
async function main() {
  ...
  const { data: before, error: selErr } = await sb
    ...
    .eq("supplier_id", SUPPLIER_ID_HOOKKA);
  if (selErr) throw selErr;

  console.log(`HoOKkA has ${before?.length ?? 0} POs:`);
  ...
  console.log(`\nReset ${after?.length ?? 0} HoOKkA POs to sup_status='pending'.`);
}
